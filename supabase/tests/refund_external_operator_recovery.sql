...
set local role authenticated;
select throws_ok($$with current_receipt as (
 select public.admin_get_refund_authoritative_receipt_overview('bf400000-0000-4000-8000-000000000001') v
) select public.admin_queue_refund_receipt_completion(
 'bf400000-0000-4000-8000-000000000001',(v->'receipt'->>'id')::uuid,
 (v->>'expectedCaseVersion')::bigint,'bf700000-0000-4000-8000-000000000001',true,repeat('a',64)
) from current_receipt$$,'P4664',
 'Review the existing completion or unresolved message; do not send another',
 'An externally adopted SENT notice blocks completion queueing before any new message');
reset role;
select ok(not exists(select 1 from public.refund_receipt_completion_intents where refund_case_id='bf400000-0000-4000-8000-000000000001')
 and not exists(select 1 from public.refund_case_messages where refund_case_id='bf400000-0000-4000-8000-000000000001'),
 'Adopted-notice rejection creates no completion intent or customer message');
