-- Add a conservative refund adjustment review layer and include applied
-- refund adjustments in partner period settlement math.

create or replace function public.normalize_reporting_match_text(p_value text)
returns text
language sql
immutable
as $$
  select trim(regexp_replace(regexp_replace(lower(coalesce(p_value, '')), '[^a-z0-9]+', ' ', 'g'), '[[:space:]]+', ' ', 'g'));
$$;

create table if not exists public.reporting_machine_aliases (
  id uuid primary key default gen_random_uuid(),
  reporting_machine_id uuid not null references public.reporting_machines (id) on delete cascade,
  alias text not null,
  normalized_alias text generated always as (public.normalize_reporting_match_text(alias)) stored,
  alias_type text not null default 'refund_location'
    check (alias_type in ('refund_location', 'alternate_label', 'legacy_location', 'other')),
  ...
  constraint reporting_machine_aliases_alias_present check (length(trim(alias)) > 0),
  constraint reporting_machine_aliases_normalized_present check (length(trim(normalized_alias)) > 0)
);

create index if not exists reporting_machine_aliases_machine_idx
  on public.reporting_machine_aliases (reporting_machine_id);

create index if not exists reporting_machine_aliases_normalized_active_idx
  on public.reporting_machine_aliases (normalized_alias)
  where status = 'active';

create unique index if not exists reporting_machine_aliases_machine_alias_active_idx
  on public.reporting_machine_aliases (reporting_machine_id, normalized_alias)
  where status = 'active';

drop trigger if exists reporting_machine_aliases_set_updated_at on public.reporting_machine_aliases;
create trigger reporting_machine_aliases_set_updated_at
before update on public.reporting_machine_aliases
for each row execute function public.set_updated_at();

create table if not exists public.refund_adjustment_review_rows (
  id uuid primary key default gen_random_uuid(),
  import_run_id uuid references public.sales_import_runs (id) on delete set null,
  source text not null default 'sheet_export'
    check (source in ('sheet_export', 'api_payload', 'manual_csv')),
  source_reference text not null default 'refund-source-export',
  source_row_reference text not null,
  source_row_hash text not null,
  source_location text,
  normalized_source_location text generated always as (public.normalize_reporting_match_text(source_location)) stored,
  refund_date date,
  ...
  match_status text not null default 'needs_review'
    check (match_status in ('needs_review', 'matched', 'applied', 'ambiguous', 'unmatched', 'duplicate', 'invalid', 'ignored')),
  match_confidence numeric(5,4) not null default 0 check (match_confidence >= 0 and match_confidence <= 1),
  match_reason text,
  candidate_machine_ids uuid[] not null default '{}'::uuid[],
  matched_machine_id uuid references public.reporting_machines (id) on delete set null,
  matched_location_id uuid references public.reporting_locations (id) on delete set null,
  applied_adjustment_id uuid references public.sales_adjustment_facts (id) on delete set null,
  resolution_status text not null default 'unresolved'
    check (resolution_status in ('unresolved', 'approved', 'ignored', 'rejected')),
  reviewed_by uuid references auth.users (id) on delete set null,
  reviewed_at timestamptz,
  review_notes text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint refund_adjustment_review_rows_source_row_hash_present check (length(trim(source_row_hash)) > 0),
  constraint refund_adjustment_review_rows_source_row_reference_present check (length(trim(source_row_reference)) > 0)
);

create unique index if not exists refund_adjustment_review_rows_source_row_idx
  on public.refund_adjustment_review_rows (source, source_reference, source_row_reference);

create index if not exists refund_adjustment_review_rows_import_run_idx
  on public.refund_adjustment_review_rows (import_run_id, imported_at desc);

create index if not exists refund_adjustment_review_rows_match_status_idx
  on public.refund_adjustment_review_rows (match_status, resolution_status, imported_at desc);

create index if not exists refund_adjustment_review_rows_refund_date_idx
  on public.refund_adjustment_review_rows (refund_date desc);

create index if not exists refund_adjustment_review_rows_matched_machine_idx
  on public.refund_adjustment_review_rows (matched_machine_id, refund_date desc);

drop trigger if exists refund_adjustment_review_rows_set_updated_at on public.refund_adjustment_review_rows;
create trigger refund_adjustment_review_rows_set_updated_at
before update on public.refund_adjustment_review_rows
for each row execute function public.set_updated_at();

alter table public.reporting_machine_aliases enable row level security;
alter table public.refund_adjustment_review_rows enable row level security;

drop policy if exists "reporting_machine_aliases_super_admin_all" on public.reporting_machine_aliases;
create policy "reporting_machine_aliases_super_admin_all"
on public.reporting_machine_aliases
for all
using (public.is_super_admin((select auth.uid())))
with check (public.is_super_admin((select auth.uid())));

drop policy if exists "refund_adjustment_review_rows_super_admin_all" on public.refund_adjustment_review_rows;
create policy "refund_adjustment_review_rows_super_admin_all"
...
alter table public.sales_adjustment_facts
  ...
  add column if not exists refund_review_row_id uuid,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_adjustment_facts_refund_review_row_id_fkey'
      and conrelid = 'public.sales_adjustment_facts'::regclass
  ) then
    alter table public.sales_adjustment_facts
      add constraint sales_adjustment_facts_refund_review_row_id_fkey
      foreign key (refund_review_row_id)
      references public.refund_adjustment_review_rows (id)
      on delete set null;
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_adjustment_facts_match_status_check'
      and conrelid = 'public.sales_adjustment_facts'::regclass
  ) then
    alter table public.sales_adjustment_facts
      add constraint sales_adjustment_facts_match_status_check
      check (
        match_status is null
        or match_status in ('matched', 'applied', 'manual', 'legacy')
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sales_adjustment_facts_match_confidence_check'
      and conrelid = 'public.sales_adjustment_facts'::regclass
  ) then
    alter table public.sales_adjustment_facts
      add constraint sales_adjustment_facts_match_confidence_check
      check (
...
begin
  ...
  scoped_facts as (
    ...
    -- Sunze Order amount is the gross sales basis, then machine tax,
    -- configured paid-order fees, and applied refunds are deducted before split.
    select
      period.period_start,
      period.period_end,
      fact.id,
  ...
  unresolved_refund_review as (
    ...
    where review.refund_date between p_date_from and p_date_to
      ...
      and review.match_status in ('needs_review', 'ambiguous', 'unmatched', 'invalid', 'duplicate')
  ),
  warnings as (
    ...
    select jsonb_build_object(
      ...
      'severity', 'non_blocking',
      'message', 'Refund adjustment rows need admin review before they affect partner settlement.'
    ) as warning
