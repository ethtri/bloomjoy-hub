-- Scoped-admin training and partner-dashboard repair for issue #259.
--
-- Scoped Admin is an internal operations role, not a Plus member and not a
-- report_manager workaround. It should still get operator training and partner
-- dashboard access for partnerships fully covered by the scoped machine set.
...
begin
  ...
  is_admin_user := public.is_super_admin(p_user_id) or public.is_scoped_admin(p_user_id);
...
begin
  ...
  if not public.is_super_admin(actor_user_id)
    and not public.is_scoped_admin(actor_user_id)
  then
