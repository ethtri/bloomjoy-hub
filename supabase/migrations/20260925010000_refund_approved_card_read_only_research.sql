...
begin
  ...
  for candidate_id in
    ...
    where candidate.payment_method = 'card'
      ...
      and candidate.decided_by is not null
      ...
      and candidate.nayax_lookup_status = 'manual_exception'
      ...
      and not exists (
        select 1 from public.refund_nayax_lookup_candidates live
    ...
    order by candidate.nayax_lookup_finished_at, candidate.created_at, candidate.id
    limit p_limit
  loop
    ...
    if not found or c.payment_method <> 'card'
      ...
      or nullif(c.nayax_recommendation_policy_version,'') is null
      or c.nayax_recommendation_policy_version = 'manual-nayax-portal-v1'
      ...
      or exists (select 1 from public.refund_nayax_lookup_candidates live
        where live.refund_case_id = c.id
          ...
          and live.expires_at > statement_timestamp())
      or (public.refund_lifecycle_contract(c.id) -> 'lookup')
        @> '{"status":"results_expired","safeRetryEligible":true}'::jsonb is not true
    ...
    update public.refund_cases set nayax_lookup_retry_fact_version = c.deterministic_fact_version
      where id = c.id;
