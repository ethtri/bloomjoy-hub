-- Repair partner dashboard amount owed totals so all non-Bloomjoy payout
-- recipient shares are included. The original period-preview RPC only used
-- partner_share_basis_points, which misses the first configured payout
-- recipient stored in fever_share_basis_points.
