        ...
        jsonb_build_object(
          ...
          'nayaxLookupCandidates', coalesce((
            select jsonb_agg(
              ...
              order by
                coalesce((candidate.evidence_summary ->> 'match_confidence')::numeric, 0) desc,
...
comment on function public.admin_get_refund_operations_overview() is
  'Refund operations queue overview with provider/setup identifiers redacted and tokenized active Nayax candidates available for manager review.';

comment on table public.refund_nayax_lookup_candidates is
  'Private server-side Nayax lookup candidate tokens. Browser clients receive only tokenized evidence; raw provider transaction IDs stay server-side. Pilot candidates remain available for manager review for up to 24 hours by default.';
