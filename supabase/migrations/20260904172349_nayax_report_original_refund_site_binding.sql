-- #971: original-sale and refund-row sites are separate native namespaces.
-- Corroborate only an existing exact full-refund receipt; no new receipt/payment/mail.
create index if not exists nayax_report_observations_refund_identity_idx
  on public.nayax_scheduled_refund_observations(account_scope,provider_transaction_id);
    ...
    if inserted and case_id is not null then
      ...
      values(case_id,'nayax_scheduled_report_observed',case when receipt.id is not null
        ...
        jsonb_build_object('observation_digest',obs_digest,'file_digest',digest,'existing_receipt_id',receipt.id,
          ...
          'original_sale_site_id',case when receipt.id is not null then original_case.matched_nayax_site_id else null end,
          'linkage_source',case when receipt.id is not null then 'authenticated_report_explicit_original_id' else null end,'terminal_evidence_proven',false,'payload_redacted',true));
    end if;
  end loop;
