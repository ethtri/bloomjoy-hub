...
const resolveMessageType = (
  ...
  if (
    afterRow.status === "waiting_on_customer" &&
    beforeRow.status !== "waiting_on_customer"
  ) {
    return "more_info";
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (nayaxCandidate && !selectionAllowed) {
      return jsonResponse({
        error:
          "This Nayax transaction has a safety block and cannot be selected.",
      }, 400);
    }
    if (
    ...
    const officialManualRefundReference = isCashCompletion
      ...
      : null;
    const officialCashPayoutSentAt = isCashCompletion
      ? cashPayoutSentAt
      : null;
