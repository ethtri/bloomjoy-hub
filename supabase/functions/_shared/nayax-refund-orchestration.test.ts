...
Deno.test("production adapter is hard-disabled before manager action or attempt reservation", async () => {
  ...
  assert(result.errorCode === "provider_execution_not_yet_enabled", "hard-off code required");
  assert(!result.providerAttempted, "disabled adapter must make no provider attempt");
  assert(atomicReservations === 0, "disabled lane must not consume manager evidence");
});
...
Deno.test("fresh exact manager TOTP drives one success attempt, one finalization, and one original-thread customer operation", async () => {
  ...
  const first = await orchestrateNayaxRefund({ request, dependencies: harness.dependencies });
  const replay = await orchestrateNayaxRefund({ request, dependencies: harness.dependencies });
  const state = harness.state();

  assert(first.executed && first.status === "succeeded", "success must finalize");
  assert(first.reportingAdjustmentPresent, "success must write reporting");
  assert(first.customerCompletion?.status === "sent", "customer completion must send");
  assert(first.customerCompletion?.transport === "gmail_thread", "Gmail thread transport required");
  assert(first.customerCompletion?.originalThread === true, "original thread required");
  assert((first.customerCompletion?.managerCcCount ?? 0) > 0, "current managers must be copied");
  assert(first.customerCompletion?.managerCompletionNoticeSent === false, "no manager completion notice allowed");
  assert(replay.executed && replay.replayed && !replay.providerAttempted, "replay must reconcile without provider");
  assert(state.providerAttempts === 1, "provider must be attempted exactly once");
  assert(state.finalizationOperations === 1, "case/reporting finalization must apply exactly once");
  assert(state.customerOperations === 1, "customer completion must apply exactly once");
});
...
Deno.test("committed success stays successful when customer delivery throws", async () => {
  ...
  assert(result.executed && result.status === "succeeded", "committed payment must remain successful");
  assert(result.reportingAdjustmentPresent, "reporting completion must remain visible");
  assert(result.customerCompletion?.status === "delivery_unknown", "email-only uncertainty required");
  assert(replay.executed && replay.replayed && !replay.providerAttempted, "replay must not call provider");
  assert(state.providerAttempts === 1, "delivery failure must never cause a second provider attempt");
  assert(state.finalizationOperations === 1, "case/reporting finalization remains exactly once");
});
...
for (const scenario of [
  ...
  Deno.test(`${scenario.kind} keeps the case open, suppresses success mail/fallback, and never blindly retries`, async () => {
    ...
    assert(!first.executed && first.status === scenario.status, "case must stay incomplete");
    assert(first.customerCompletion === null && first.fallbackIssued === false, "no success mail or fallback");
    assert(first.reconciliationRequired === scenario.reconciliation, "hold classification must match outcome");
    assert(replay.replayed && !replay.providerAttempted, "replay must not call provider");
    assert(state.providerAttempts === 1, "provider attempt must remain exactly one");
    assert(state.caseOpen, "case must remain open");
    assert(state.finalizationOperations === 0 && state.customerOperations === 0, "no success side effects allowed");
  });
}
...
Deno.test("missing exact-factor evidence fails before provider execution", async () => {
  ...
  harness.dependencies.reserveAndConsumeAttempt = () => Promise.resolve({
    managerAction: {
      ...managerAction,
      verifiedTotpAt: "",
    },
  ...
  let rejected = false;
  try {
    await orchestrateNayaxRefund({ request, dependencies: harness.dependencies });
  } catch (error) {
    rejected = error instanceof Error && error.message.includes("exact TOTP evidence");
  }
  assert(rejected, "missing exact-factor evidence must fail closed");
  assert(harness.state().providerAttempts === 0, "provider must remain untouched");
});
...
Deno.test("missing attempt-scoped provider claim fails before provider execution", async () => {
  ...
  try {
    ...
    rejected = error instanceof Error && error.message.includes("provider claim");
  }
...
Deno.test("replaying an unsettled reservation becomes an unknown hold without a provider retry", async () => {
  ...
  const replay = await orchestrateNayaxRefund({ request, dependencies: harness.dependencies });
  const state = harness.state();
  assert(replay.replayed && !replay.providerAttempted, "crash replay must not call provider");
  assert(replay.status === "ambiguous" && replay.reconciliationRequired, "crash replay must hold unknown");
  assert(state.providerAttempts === 0, "provider must never be called after reservation replay");
  assert(state.finalizationOperations === 0 && state.customerOperations === 0, "no success effects allowed");
});
  ...
  Deno.test(`${phase} success without committed finalization/reporting is held without customer mail`, async () => {
    ...
    const result = await orchestrateNayaxRefund({
      ...
      dependencies: {
        ...
        settleProviderOutcome: () => Promise.resolve({
          attempt: {
            attemptId: "76300000-0000-4000-8000-000000000099",
            status: "succeeded",
    ...
    assert(!result.executed && result.reconciliationRequired, "partial success must be held");
    assert(result.errorCode === "success_finalization_incomplete", "explicit invariant error required");
    assert(result.customerCompletion === null && customerOperations === 0, "customer mail must stay suppressed");
  });
}
