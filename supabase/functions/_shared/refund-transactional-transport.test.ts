...
Deno.test("transactional refund mail uses the verified sender with the monitored Reply-To", async () => {
  ...
  try {
    ...
    restoreEnv("REFUND_CUSTOMER_FROM_EMAIL", originalFrom);
    restoreEnv("REFUND_REPLY_TO_EMAIL", originalReplyTo);
  }
});

Deno.test("transactional refund mail rejects a personal From address before provider access", async () => {
  const originalFetch = globalThis.fetch;
  const originalApiKey = Deno.env.get("RESEND_API_KEY");
  const originalFrom = Deno.env.get("REFUND_CUSTOMER_FROM_EMAIL");
  const originalReplyTo = Deno.env.get("REFUND_REPLY_TO_EMAIL");
  ...
  try {
    await assertRejects(
      () => sendRefundTransactionalEmail({
        to: ["customer@example.test"],
        cc: ["manager@example.test"],
        subject: "Synthetic refund update",
        text: "Synthetic body.",
      }),
      Error,
      "approved Bloomjoy support sender",
    );
...
Deno.test("transactional refund mail never falls back to the global internal sender", async () => {
  const originalFetch = globalThis.fetch;
  const originalApiKey = Deno.env.get("RESEND_API_KEY");
  const originalRefundFrom = Deno.env.get("REFUND_CUSTOMER_FROM_EMAIL");
  const originalInternalFrom = Deno.env.get("INTERNAL_NOTIFICATION_FROM_EMAIL");
  const originalReplyTo = Deno.env.get("REFUND_REPLY_TO_EMAIL");
  let fetchCount = 0;
  Deno.env.set("RESEND_API_KEY", "synthetic-resend-key");
  Deno.env.delete("REFUND_CUSTOMER_FROM_EMAIL");
  Deno.env.set(
  ...
  try {
    ...
    globalThis.fetch = originalFetch;
    restoreEnv("RESEND_API_KEY", originalApiKey);
    restoreEnv("REFUND_CUSTOMER_FROM_EMAIL", originalRefundFrom);
    restoreEnv("INTERNAL_NOTIFICATION_FROM_EMAIL", originalInternalFrom);
    restoreEnv("REFUND_REPLY_TO_EMAIL", originalReplyTo);
  }
});
