...
serve(async (req) => {
  ...
  if (!stripe) {
    return new Response(
      JSON.stringify({ error: "Checkout status is not configured." }),
      {
        status: 500,
  ...
  try {
    ...
    if (
      !isRecognizedSession ||
      !hasAllowedCheckoutPrices(session, checkoutPriceConfig)
    ) {
      return new Response(
        JSON.stringify({ error: "Unable to verify checkout status." }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
