...
const checks = [
  ...
  ['one projection drives digest and portal queue counts', sweep.includes('parseRefundManagerWorkProjection(claim.projection)') && operations.includes("get_refund_manager_work_projection") && portal.includes('overview.managerWork.bucketCounts')],
  ...
  ['customer replies outrank reminders in both arrival orders', tests.includes('Reply-first routine work keeps the canonical new-information action') && tests.includes('Reminder-first routine work still selects the canonical new-information action')],
