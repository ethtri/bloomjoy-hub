...
const checks = [
  ...
  ['public options require explicit published inventory', /inventory\.reconciliation_state = 'published'/i.test(migration)],
  ['cotton candy and Snapcase share the public path', /inventory\.refund_category in \('cotton_candy', 'snapcase'\)/i.test(migration)],
  ['public options have no Commercial Mini type filter', !/machine\.machine_type in \('commercial', 'mini'\)/i.test(migration.slice(migration.indexOf('create or replace function public.public_refund_machine_options()')))],
