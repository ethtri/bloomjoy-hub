...
assert.match(
  productionRunbook,
  /exactly 112 reviewed synthetic screenshots/,
  'Production runbook must use the current 112-screenshot evidence inventory'
);
assert.doesNotMatch(
  productionRunbook,
  /exactly (?:44|83|85|86|90|92|111) reviewed synthetic screenshots/,
...
for (const [documentName, document] of [
  ...
  assert.match(
    document,
    /exactly 112 reviewed synthetic screenshots/,
    `${documentName} must use the current 112-screenshot evidence inventory`
  );
  assert.doesNotMatch(
    document,
    /exactly (?:44|83|85|86|90|92|111) reviewed synthetic screenshots/,
