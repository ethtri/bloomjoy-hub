...
assert.match(
  productionRunbook,
  /exactly 114 reviewed synthetic screenshots/,
  'Production runbook must use the current 114-screenshot evidence inventory'
);
assert.doesNotMatch(
  productionRunbook,
  /exactly (?:44|83|85|86|90|92|111|112) reviewed synthetic screenshots/,
...
for (const [documentName, document] of [
  ...
  assert.match(
    document,
    /exactly 114 reviewed synthetic screenshots/,
    `${documentName} must use the current 114-screenshot evidence inventory`
  );
  assert.doesNotMatch(
    document,
    /exactly (?:44|83|85|86|90|92|111|112) reviewed synthetic screenshots/,
...
try {
  ...
  assert(
    repositoryMigrations.includes('20260902200508_refund_gmail_receipt_processing.sql') &&
      repositoryMigrations.indexOf('20260902195401_refund_historical_owner_notice.sql') <
      repositoryMigrations.indexOf('20260902200508_refund_gmail_receipt_processing.sql'),
    'Receipt-aware Gmail fact processing follows the canonical owner-notice contract'
  );
  assert(
