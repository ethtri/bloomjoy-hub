...
assert.match(
  productionRunbook,
  /exactly 86 reviewed synthetic screenshots/,
  'Production runbook must use the current 86-screenshot evidence inventory'
);
assert.doesNotMatch(
  productionRunbook,
  /exactly (?:44|83|85) reviewed synthetic screenshots/,
...
for (const [documentName, document] of [
  ...
  assert.match(
    document,
    /exactly 86 reviewed synthetic screenshots/,
    `${documentName} must use the current 86-screenshot evidence inventory`
  );
  assert.doesNotMatch(
    document,
    /exactly (?:44|83|85) reviewed synthetic screenshots/,
