...
assert.match(
  productionRunbook,
  /exactly 93 reviewed synthetic screenshots/,
  'Production runbook must use the current 93-screenshot evidence inventory'
);
assert.doesNotMatch(
  productionRunbook,
...
for (const [documentName, document] of [
  ...
  assert.match(
    document,
    /exactly 93 reviewed synthetic screenshots/,
    `${documentName} must use the current 93-screenshot evidence inventory`
  );
  assert.doesNotMatch(
    document,
    /exactly (?:44|83|85|86|90|92) reviewed synthetic screenshots/,
