...
const assertions = [
  ...
  ['query context is deferred as a transition until after hydration', contact.includes('const [queryReady, setQueryReady] = useState(false)') && contact.includes("queryReady ? searchParams.get('type') : null") && contact.includes('startTransition(() => setQueryReady(true))') && contact.includes('if (!queryReady) return;')],
