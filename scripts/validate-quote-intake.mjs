...
const assertions = [
  ...
  ['query context is deferred until after hydration', contact.includes('const [queryReady, setQueryReady] = useState(false)') && contact.includes("queryReady ? searchParams.get('type') : null") && contact.includes('if (!queryReady) return;')],
