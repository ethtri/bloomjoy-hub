...
import {
  ...
  disputeRefundNayaxPreselection,
  disposeRefundAcknowledgementException,
  approveReviewedNayaxCandidate,
  approveSelectedNayaxCandidate,
...
const nayaxResolutionResultOptions: Array<{
  ...
  {
    value: 'provider_confirmed_no_refund',
    label: 'No refund was issued',
    helper: 'Record exact Nayax or support proof. System will continue the same approved attempt without another manager decision.',
  },
];
...
> = {
  ...
  provider_confirmed_no_refund: [
    { value: 'nayax_dtm_not_refunded', label: 'The transaction record proves no refund was issued' },
    { value: 'nayax_support_confirmed_no_refund', label: 'Nayax support proves no refund was issued' },
  ],
  remain_on_hold: [
...
const nayaxResolutionReasonsForEvidence = (
  ...
  if (result === 'provider_confirmed_success' || result === 'provider_confirmed_no_refund') {
    return nayaxResolutionReasonOptions[result].filter(({ value }) =>
      evidenceType === 'nayax_support_ticket'
        ? value === (result === 'provider_confirmed_success'
            ? 'nayax_support_confirmed_success' : 'nayax_support_confirmed_no_refund')
        : value === (result === 'provider_confirmed_success'
            ? 'nayax_dtm_settled' : 'nayax_dtm_not_refunded')
    );
  }
...
type PrimaryActionConfig = {
  ...
  mode?: 'case_update' | 'retry_message' | 'nayax_refund_execution' | 'reviewed_nayax_final_decision' | 'selected_nayax_final_decision' | 'resolve_delivery_not_found' | 'review_transaction_evidence';
  disabled?: boolean;
};
...
const isReadyToPayCase = (refundCase: RefundCaseRecord) => {
  ...
  if (refundCase.paymentMethod === 'card' && refundCase.refundReadiness) {
    if (
      ...
      refundCase.confirmedDuplicate ||
      refundCase.lifecycle?.stage === 'duplicate_resolved' ||
      hasConfirmedRefundReceipt(refundCase) ||
      refundCase.status === 'card_refund_pending'
    ) return false;
    return hasCurrentRefundCardCapability(refundCase, refundCase.refundReadiness);
  }
...
const taskLabel = (refundCase: RefundCaseRecord) => taskManagerState(refundCase).label;
const taskBadgeClass = (refundCase: RefundCaseRecord) =>
  managerStateBadgeClass(taskManagerState(refundCase).tone);
  ...
  switch (summary.lookupStatus) {
    ...
    case 'lookup_failed':
      return summary.safeRetryEligible
        ...
        : 'The transaction search did not finish. Report the blocked card-refund search to Engineering. Do not ask the customer to repeat purchase details.';
    case 'no_match':
...
const formatCardSaleLine = (
  ...
  return `${formatCurrency(amountCents)} ${brand} ending ${last4} at ${formatRefundDateTime(
    authTime,
    refundCaseTimezone(refundCase)
  )}`;
};
  ...
  switch (summary.lookupStatus) {
    ...
    case 'lookup_failed':
      return summary.safeRetryEligible
        ...
        : 'Next: Report the blocked card-refund search to Engineering. No refund has been issued.';
    case 'not_applicable':
    default:
...
const transactionMatchSummary = (
  ...
  if (refundCase.paymentMethod === 'card') {
    ...
    if (hasSelectedCardEvidence(refundCase, editor)) {
      return `Selected transaction: ${formatCurrency(amountCents)} / ${brand} ending ${last4 || 'n/a'} / ${formatRefundDateTime(authTime, refundCaseTimezone(refundCase))}.`;
    }
    ...
    if (candidates.length > 0) {
      ...
      } ending ${recommended.cardLast4 || 'n/a'} / ${formatRefundDateTime(
        recommended.authorizedAt,
        refundCaseTimezone(refundCase)
      )}. Confirm the right transaction before completing the refund.`;
    }
  ...
  if (refundCase.hasMatchedSalesFact || refundCase.correlationStatus === 'matched') {
    return `Cash payment found for ${formatCurrency(refundCase.paymentAmountCents)} near ${formatRefundDateTime(
      refundCase.incidentAt,
      refundCaseTimezone(refundCase)
    )}.`;
  }
...
const primaryActionConfig = (
  ...
  if (
    ...
    refundCase.lifecycle?.stage === 'needs_transaction_selection' &&
    nextWork?.isOpen === true && nextWork.actor === 'manager' &&
    nextWork.actionCode === 'approve_or_deny_request' &&
    Boolean(nextWork.preparationProofId)
  ) {
    const candidate = selectedNayaxCandidate(editor, candidates);
    const chosenIsEligible = Boolean(candidate && candidate.selectionAllowed !== false &&
      nextWork.eligibleCandidateTokens?.includes(candidate.candidateToken));
    return {
      label: chosenIsEligible && candidate?.amountCents
        ? `Approve ${formatCurrency(candidate.amountCents)} refund`
        : 'Choose a reviewed purchase to approve',
      helper: 'Choose one reviewed purchase within this final decision. Bloomjoy will issue the approved refund and notify the customer after payment is confirmed.',
      targetStatus: 'card_refund_pending',
      targetDecision: 'approved',
      mode: 'reviewed_nayax_final_decision',
    };
  }
  if (
    refundCase.paymentMethod === 'card' && refundCase.decision == null &&
    ...
    nextWork.actionCode === 'approve_or_deny_request' &&
    refundCase.canPerformOfficialAction === true &&
    hasFreshPersistedNayaxSelection(refundCase, refundReadiness)
  ) {
    return {
      label: `Approve ${formatCurrency(refundCase.selectedNayaxTransaction?.saleAmountCents ?? refundCase.refundAmountCents ?? refundCase.paymentAmountCents)} refund`,
      helper: 'Approve the exact saved purchase once. Bloomjoy will queue the protected attempt; a paused machine waits for System activation before payment.',
      targetStatus: 'card_refund_pending',
      targetDecision: 'approved',
      mode: 'selected_nayax_final_decision',
    };
  }
  if (
    refundCase.paymentMethod === 'card' &&
  ...
  if (refundCase.paymentMethod === 'card' && refundCase.nayaxLookupSummary?.lookupStatus === 'lookup_failed') {
    return {
      ...
      helper: refundCase.nayaxLookupSummary.safeRetryEligible
        ...
        : 'Report the blocked card-refund search to Engineering. No card refund can be completed outside this flow.',
      disabled: true,
    };
  }
...
const getPrimaryActionIssues = (
  ...
  if (
    hasConfirmedRefundReceipt(refundCase) ||
    action?.mode === 'retry_message' ||
    action?.mode === 'nayax_refund_execution' ||
    action?.mode === 'reviewed_nayax_final_decision' ||
    action?.mode === 'selected_nayax_final_decision'
  ) return [];
...
export default function AdminRefundsPage() {
  ...
  const selectedRefundReadiness: RefundReadiness | null = useMemo(() => {
    ...
    if (
      ...
      Number(selectedCase.officialActionVersion ?? 0) <= 0 ||
      nayaxCardRefundAvailability.caseVersion !== selectedCase.officialActionVersion
    ) {
      return null;
    }
    return {
      transactionConfirmed: nayaxCardRefundAvailability.transactionConfirmed === true,
  ...
  const selectedCaseHasCurrentCardCapability = selectedCase?.paymentMethod === 'card' &&
    hasCurrentRefundCardCapability(selectedCase, selectedRefundReadiness);
  const selectedCaseNeedsLegacyPaymentReview =
  ...
  const selectedCaseHasCurrentCardDecisionAuthority =
    selectedCaseHasCurrentCardCapability &&
    selectedCase != null &&
    ...
    selectedCase?.status !== 'card_refund_pending';
  const selectedCaseOfficialActionBlockMessage = selectedCaseNeedsLegacyPaymentReview
  ...
  const historicalReviewBannerPresentation: RefundHistoricalReviewBannerPresentation | null =
    ...
    !selectedCase?.providerHold
      ? {
          testId: selectedCaseNeedsLegacyPaymentReview
            ? 'refund-legacy-state-review-banner'
            : 'refund-review-only-banner',
          title: selectedCaseNeedsLegacyPaymentReview
  ...
  const selectedTransactionView = selectedCase
    ? deriveRefundTransactionViewState({
        ...
        candidateCount: selectedCaseNeedsLegacyPaymentReview ? 0 : nayaxCandidates.length,
        selectableCandidateCount: selectedCaseNeedsLegacyPaymentReview
          ? 0
          : nayaxCandidates.filter((candidate) => candidate.selectionAllowed !== false &&
            (selectedCase.lifecycle?.nextWork?.preparationProofId == null ||
              selectedCase.lifecycle.nextWork.eligibleCandidateTokens?.includes(candidate.candidateToken))).length,
        hasSelectedMatch:
          !selectedCaseNeedsLegacyPaymentReview &&
          selectedCase.hasMatchedNayaxTransaction &&
  ...
  const handleSaveCase = async (
    ...
    const currentCardDenialAuthorized =
      selectedCaseHasCurrentCardDecisionAuthority &&
      (nextEditor.status === 'denied' || nextEditor.decision === 'denied');
    if (
      editorRequiresOfficialAction(nextEditor) &&
      selectedCaseIsReviewOnly &&
      !currentCardDenialAuthorized
    ) {
  ...
  const handleDisputeNayaxPreselection = async () => {
    if (
      !selectedCase ||
      !editor ||
      selectedCase.decision != null ||
      selectedCase.paymentMethod !== 'card' ||
      selectedCase.nayaxRecommendationState !== 'high_confidence' ||
      selectedCase.hasMatchedNayaxTransaction !== true ||
      editor.clearNayaxMatch ||
      officialActionVersion <= 0 ||
      isSaving
    ) {
    ...
    setIsSaving(true);
    try {
      const result = await disputeRefundNayaxPreselection(targetCaseId, officialActionVersion);
      if (
        result.disputed !== true ||
        result.status !== 'manual_exception' ||
      ...
      setRefundActionReceipt({
        tone: 'success',
        title: 'Transaction marked for review',
        message: 'No refund, approval, or customer message was sent. Review the current transactions and save the right one for manager approval.',
      });
      toast.success('Transaction marked for review. No refund was sent.');
    } catch (error) {
      if (selectedIdRef.current !== targetCaseId) return;
      const message = error instanceof Error ? error.message : 'Unable to mark this transaction for review.';
      setRefundActionReceipt({
        tone: 'warning',
        title: 'Transaction review could not be saved',
        message,
      });
      toast.error(message);
    } finally {
  ...
  const applyNayaxExecutionResult = async (
          ...
          ? {
              ...
              cases: currentOverview.cases.map((refundCase) =>
                refundCase.id === selectedCase.id
                  ? {
                      ...refundCase,
                      providerHold: ambiguous || rejected || result.providerAttempted === true || result.replayed === true,
    ...
    toast.error(
      ambiguous
        ...
        : rejected
          ? 'The refund was rejected. The case remains held for verification.'
  ...
  const handleApproveReviewedNayaxCandidate = async () => {
    ...
    try {
      ...
      const freshWork = freshCase?.lifecycle?.nextWork;
      if (selectedIdRef.current !== targetCaseId || !freshCase ||
          ...
          freshWork.preparationProofId !== proofId ||
          !freshWork.eligibleCandidateTokens?.includes(selectedCandidate.candidateToken)) {
        throw new Error('The reviewed purchase or case decision changed. Refresh before deciding again.');
      }
      const result = await approveReviewedNayaxCandidate({
        caseId: targetCaseId,
      ...
      setRefundActionReceipt({
        ...
        message: result.status === 'completed'
          ? 'The original approved refund is complete. No second payment was attempted.'
          : result.status === 'provider_hold'
            ? 'The existing refund attempt needs payment-result review. Do not retry or approve again.'
            : 'The exact reviewed purchase was approved once. Bloomjoy will continue the protected refund attempt; no payment was sent by this decision request.',
      });
    } catch (error) {
      await refresh();
      const message = error instanceof Error ? error.message :
        'The decision result could not be confirmed. Refresh the case history before taking another action.';
  ...
  const handleApproveSelectedNayaxCandidate = async () => {
    if (nayaxRefundInFlightRef.current || !selectedCase ||
  ...
  const handleRunNayaxRefund = async () => {
    ...
    if (editor.clearNayaxMatch || candidateBeingSelected) {
      setIsRefundConfirmationOpen(false);
      setNayaxExecutionNotice({
        tone: 'warning',
  ...
  const handlePrimaryAction = async () => {
    ...
    if (primaryAction.mode === 'reviewed_nayax_final_decision') {
      await handleApproveReviewedNayaxCandidate();
      return;
    }
    if (primaryAction.mode === 'selected_nayax_final_decision') {
      await handleApproveSelectedNayaxCandidate();
      return;
    }
  ...
  const renderCardSaleCandidates = () => {
    ...
    const effectiveCandidates = selectedCaseNeedsLegacyPaymentReview ? [] :
      reviewedFinalDecisionReady
        ? nayaxCandidates.map((candidate) => ({
            ...candidate,
            selectionAllowed: candidate.selectionAllowed !== false &&
    ...
    const systemSelectedClearMatch = hasPersistedSelectedMatch &&
      selectedCase.nayaxRecommendationState === 'high_confidence';
    const canReviewSystemSelection = systemSelectedClearMatch && selectedCase.decision == null;
      ...
      <div className="mt-3 space-y-3">
        ...
        {(!selectedCase.hasMatchedNayaxTransaction || editor.clearNayaxMatch) &&
          ...
          <RefundTransactionCandidateReview
            ...
            describeUnavailableCandidate={(candidate) =>
              reviewedFinalDecisionReady && !eligibleReviewedTokens.has(candidate.candidateToken)
        ...
        {systemSelectedClearMatch && (
          ...
          >
            ...
            {canReviewSystemSelection && (
              <Button
                data-testid="refund-review-other-transactions"
                type="button"
                variant="outline"
                size="sm"
                className="mt-3 h-auto min-h-10 whitespace-normal border-emerald-300 bg-white py-2 text-left text-emerald-950 hover:bg-emerald-100"
  ...
  const renderCardDecisionWorkbench = () => {
    ...
    const cardAmountCents = selectedCaseHasCurrentCardCapability
      ? selectedRefundReadiness?.refundAmountCents ??
        selectedTransactionEvidence?.saleAmountCents ??
        selectedCase.refundAmountCents ??
        selectedCase.paymentAmountCents
      : selectedCaseNeedsLegacyPaymentReview
        ? selectedCase.paymentAmountCents
        : matchedCardSaleAmountCents ?? selectedCase.paymentAmountCents;
    const cardLast4 =
      selectedTransactionEvidence?.cardLast4 ||
      comparisonCandidate?.cardLast4 ||
      (selectedCaseNeedsLegacyPaymentReview ? null : selectedCase.matchedNayaxCardLast4) ||
      (selectedCaseNeedsLegacyPaymentReview ? null : editor.matchedNayaxCardLast4) ||
    ...
    const transactionProviderTime =
      ...
      (selectedCaseNeedsLegacyPaymentReview ? null : selectedCase.matchedNayaxMachineAuthTime) ||
      (selectedCaseNeedsLegacyPaymentReview ? null : editor.matchedNayaxMachineAuthTime) ||
      null;
    ...
    const hasUnsavedTransactionChoice = !reviewedFinalDecisionReady &&
      !selectedCase.hasMatchedNayaxTransaction &&
    ...
    const showDisabledActionStatus =
      primaryAction?.disabled === true &&
      !selectedCaseIsTerminal &&
      cardManagerState.id !== 'match_attention' &&
      cardManagerState.id !== 'check_nayax_result';
    const hasUnsavedCardSelection = editor.clearNayaxMatch ||
      Boolean(editor.matchedNayaxCandidateToken.trim());
    const currentCardDenialAction = selectedCaseHasCurrentCardDecisionAuthority &&
    ...
    const cardActionDisabled = primaryAction?.mode === 'nayax_refund_execution'
      ? isRefundCardActionDisabled(selectedCase, selectedRefundReadiness, {
          ...
          demo: isUsingDemoData,
          hasUnsavedSelection: hasUnsavedCardSelection,
        })
      : primaryAction?.mode === 'reviewed_nayax_final_decision' ||
        primaryAction?.mode === 'selected_nayax_final_decision'
        ? isSaving || isSendingCustomerMessage || isRunningNayaxRefund ||
          isUsingDemoData || selectedCaseIsReviewOnly || officialActionVersion <= 0 ||
          (primaryAction.mode === 'reviewed_nayax_final_decision' &&
            (!reviewedChoice || reviewedChoice.selectionAllowed !== true)) ||
      ...
      : isSaving ||
        ...
        (primaryActionNeedsOfficialAccess && (
          officialActionVersion <= 0 ||
          (selectedCaseIsReviewOnly && !currentCardDenialAction)
        )) ||
        primaryActionIssues.length > 0;
    const cardManagerCapabilityAction: RefundCardManagerCapabilityAction =
      transactionDecisionPending && !reviewedFinalDecisionReady
      ? { kind: 'hidden' }
      : showDisabledActionStatus && primaryAction
        ...
        : primaryAction && primaryAction.disabled !== true
          ? {
              kind: 'button',
              testId: reviewedFinalDecisionReady
                ? 'refund-approve-reviewed-purchase'
                : primaryAction.mode === 'selected_nayax_final_decision'
    ...
    const nayaxOutcomeResolutionPresentation: RefundNayaxOutcomeResolutionPresentation = {
      freeze: {
        ...
        message: selectedCaseNeedsLegacyPaymentReview
      ...
      resolution: !selectedCaseNeedsLegacyPaymentReview && nayaxResolutionReadiness?.visible
        ? {
    ...
    const cardExecutionConfirmationPresentation: RefundCardExecutionConfirmationPresentation = {
      ...
      confirmDisabled: cardActionDisabled,
    };
    ...
    const customerCommunicationActions: RefundCustomerCommunicationActionsPresentation = {
      ...
      denial: primaryAction?.label !== 'Deny request'
        ? {
            ...
            disabled: isUsingDemoData || (
              selectedCaseIsReviewOnly && !selectedCaseHasCurrentCardDecisionAuthority
            ),
          }
        : null,
    };
  ...
  const renderCashDecisionWorkbench = () => {
    ...
    return (
      <RefundCashDecisionWorkbench
        ...
        customerUpdate={{
          label: getCustomerCommunicationLabel(selectedCase),
          hasPendingDenialAppeal: hasPendingDenialAppeal(selectedCase),
          nextDraft: nextCustomerDraft,
          onRequestCorrection:
        ...
        correctionSummary={(
          <CustomerCorrectionSummary
            refundCase={selectedCase}
            onReview={(trigger) => {
              correctionDialogTriggerRef.current = { caseId: selectedCase.id, element: trigger };
              setCorrectionSelection({
                caseId: selectedCase.id,
                version: officialActionVersion,
                fields: [...(selectedCase.customerCorrection?.requestedFields ?? [])],
                requestId: selectedCase.customerCorrection?.requestId,
                editing: false,
              });
            }}
          />
        )}
        revisionDeliveryReview={revisionDeliveryReview}
        denialReasons={customerSafeDenialReasons}
        onPrimaryAction={() => {
          if (isCashCompletion) {
            setRefundActionReceipt(null);
