...
- The reply stays in the original Gmail thread and carries `Auto-Submitted: auto-replied` plus `X-Auto-Response-Suppress: All`.
...
- Production active mode remains blocked until issue `#686` proves customer/staff/system participant classification and current mapped-manager CC behavior. An isolated synthetic label may be used before that dependency is complete.
...
| `active` | Code-blocked in `#688` | Enabled only after `#686` installs participant classification and mapped-manager CC |
...
5. Send one new synthetic customer thread. Confirm one linked case and one acknowledgement in the original thread. Replay the scheduler, replay ingestion, and add a later reply; confirm no duplicate acknowledgement.
