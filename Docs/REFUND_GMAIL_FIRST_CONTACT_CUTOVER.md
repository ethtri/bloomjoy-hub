...
- After the hosted form resolves the machine, every case-specific customer-facing refund reply originates through the designated support mailbox in the original Gmail thread and requires a complete route for one to four current active mapped Machine Managers. A customer-manager is represented once through To and every other manager is visibly copied. Verified direct-customer evidence, open-case state, case-wide delivery pauses, and the manager route are revalidated immediately before delivery; unresolved or invalid routing sends nothing.
