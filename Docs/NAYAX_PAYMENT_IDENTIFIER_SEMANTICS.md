...
| DTM `Authorization RRN` | **Documented.** Nayax calls it a Retrieval Reference Number and says it can locate a transaction by confirmation number. | It may help an operator find the exact DTM row. | Nayax documents uniqueness within the DTM account; cross-surface equivalence and suitability as an automated account-linking or refund-execution identifier remain unproved. It is absent from Last Sales and the current scheduled-email CSV, and it is not the refund execution key. |
