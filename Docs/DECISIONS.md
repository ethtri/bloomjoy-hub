...
- The Gmail message copy is scheduled for deletion after 180 days while the canonical refund case continues under Bloomjoy's governed business-record retention. The unmerged safety candidate reserves a database-derived, tokenized upload intent before Storage, binds it to the exact private quarantine bucket and canonical UUID path, and requires delete settlement before metadata purge. A corrupt or unknown stored-object path blocks that item for manual review without preventing unrelated storage-free expired content from purging. Production enablement requires recorded Operations and privacy/security approval of this period and the quarantine-until-scanned behavior.
