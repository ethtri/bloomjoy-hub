...
**Canonical behavior**
- An eligible first customer email receives one prompt, humble acknowledgement linking only the hosted Bloomjoy Hub refund form. The current candidate polls every ten minutes, so “instantaneous” is not a pilot promise. Replies, bots, bounces, bulk/list mail, and later messages do not receive it again.
...
## 2026-08-05 - Generic first contact is the sole pre-mapping no-CC exception (`#688`, `#707`)

The form link is needed before the machine—and therefore its manager—is known. The generic first-contact acknowledgement is the sole customer message that may send before manager resolution.

**Canonical behavior**
- First contact contains exactly one Bloomjoy hosted-form CTA, no Google Form CTA, no case-specific facts, and no manager CC.
- The acknowledgement carries a private, expiring, one-time context. A valid submission completes the originating Gmail draft as one case; the token cannot be replayed or used for a different customer.
- Every later case-specific message re-resolves the current portal machine-to-manager mapping and visibly CCs the complete safe manager set. Invalid, absent, changed, or over-cap routing blocks delivery.
- Website and Gmail attachments are disabled for the pilot. A future attachment feature requires a separate scanner, retention, and privacy decision.
- Conservative website/email duplicate detection creates manager review, never a silent merge. Pending or confirmed duplicate state blocks step-up, provider execution, case completion, and settlement until resolved.

**Why this choice**
- Requiring manager CC before machine identification deadlocked the very message that collects the machine.
- A narrowly typed non-case-specific exception preserves the sponsor’s manager-visibility rule for the communications where a responsible manager exists.
- Private linkage and duplicate holds keep the email-to-form bridge from creating two independently refundable records.
...
- Opening `/refunds?case=<case-id>`, selecting a queue row, or changing a filter is navigation-only. These actions perform no Nayax lookup, case mutation, customer message, or official action. The manager must explicitly choose the visible **Check Nayax transaction** control; refresh becomes available only after a result exists.
- The target production actor for an official decision is a currently active Machine Manager mapped to the case's machine. The unmerged `#689` implementation candidate excludes broader Super Admin/Scoped Admin, service, email, scheduler, GPT, and agent identities from official actions. Those roles may support setup and review but may not approve, decline, or execute during the production pilot; no break-glass policy is approved.
- A mapped-manager login alone is not enough for an official action. The unmerged `#692` candidate uses one two-minute, single-live-per-actor intent bound to the exact actor, case, action/target, case and mapping versions, owner-approved enrollment version, amount, payload hash, and transaction/evidence fingerprint. The manager personally verifies the exact approved TOTP factor after review; only an unambiguous newest TOTP authentication strictly newer than the intent may create one server-only, single-use proof and one receipt. Replay, concurrent consume, same-second/stale/refresh-only evidence, or enrollment/case/mapping/evidence drift fails closed. Agent-controlled or shared browser sessions are not valid payment-action sessions, and production controls remain statically hard-off pending the separate gate-on review and owner UAT.
...
- The first-contact acknowledgement is scoped to the provider Gmail thread and is sent exactly once for the first eligible inbound customer message. Under the later 2026-08-05 decision, this generic hosted-form link is the sole pre-mapping no-CC exception. It does not trigger on later replies, outbound mail, bounces, mailing lists, or bulk/automated messages. The legacy responder and the Hub responder may not be authoritative at the same time.
...
- Every manual or automatic case-specific customer-facing refund message requires a resolved machine and one to three currently active Machine Managers returned by the portal's authoritative manager-to-machine mapping at send time. Unresolved, zero-manager, invalid/over-cap, empty, or changed routing fails closed before provider delivery. The system creates a redacted internal routing exception and never guesses a manager; the internal operations fallback cannot substitute for the required customer-message CC.
...
- Before customer contact is enabled, Gmail ingestion must classify a sender as customer, assigned manager, Bloomjoy mailbox, automated system, or unknown. A manager's Reply All is manager correspondence and cannot be treated as new customer evidence.
...
- Card approval and card completion are not ordinary editable status messages. A card case completes and success copy becomes claimable only when one token-bound provider settlement atomically records confirmed success, completes the case, and writes reporting. Rejection, timeout, or unknown leaves the case open, sends no success or fallback message, and is never retried blindly.
- Synthetic provider outcomes are dependency-injected local/UAT fixtures only. No request body, browser value, or environment switch may select them in the production handler; live provider execution remains statically stopped until the separate `#430` contract and gate-on review.

This decision supersedes `#634`'s requirement that every Gmail-linked customer reply receive manager approval only for the explicit deterministic message classes above. It also supersedes `#674`'s separate or singular manager-confirmation wording: completion is one customer-facing operation in the original Gmail thread, whose visible CC contains the full send-time set of one to three active mapped managers, with no second manager-only completion email. It does not weaken the human-review boundary for GPT in `#635`, the one-manager approval decision in `#674`, the fresh human-presence boundary in `#692`, or the production Nayax gate in `#430`.
...
- **Approve refund** authorizes the server-side Nayax refund-request and refund-approval sequence after every separate gate passes. Only a token-bound confirmed provider success may atomically complete the case, write the reporting adjustment, and create one original-thread customer completion with every current active mapped manager visibly CC'd; there is no separate manager completion confirmation.
...
This decision supersedes the manual-only target for high-confidence wallet/QR-time transactions in the machine-QR decision below. The 2026-08-03 email-assistance decision supersedes its separate manager-completion wording. This decision does not authorize automatic refund approval, editable approval/completion success mail, or production payment execution.
...
- Model input is limited to recent inbound text, excludes customer identity, redacts prohibited payment and credential data, and treats all message text as untrusted. Raw model input and raw OpenAI response output are not stored; derived content expires after 30 days.
...
- Every allowed draft is editable and must be explicitly approved by an authorized manager. Sending still re-resolves the required current mapped-manager CC route. A database constraint prevents automatic sending, and GPT cannot obtain or settle an official-action/provider claim. Broader autonomy requires a separate reviewed migration and explicit sponsor decision.
- The GPT/OpenAI model-request lane remains disabled until a secure server-only credential destination, privacy controls, and sanitized live evaluation are approved in `#635`. Gmail and hosted-form workflows continue without GPT.
...
- Gmail message/thread IDs remain service-only. While location is unknown, only Super Admins and Scoped Admins may view an unassigned draft and prepare a human-reviewed response; location-only Machine Managers cannot see it. Delivery remains blocked until the machine is resolved and the full current mapped-manager CC route passes at send time. The manager UI receives sanitized plain text, redacts Luhn-valid full card numbers to last four, and never exposes raw attachment paths or Gmail payloads. Logs and workflow output remain aggregate-only.
- Superseded for the email pilot by the 2026-08-05 attachment-off decision above: the earlier future design limited attachments to PDF, JPEG, or PNG and held accepted bytes in private quarantine until scanning. No attachment bytes or metadata are accepted in the pilot.
- Customer replies are manager-approved only and use one prominent action in the existing refund workbench. Gmail-linked replies stay in the original Gmail thread. An uncertain Gmail delivery outcome is never retried automatically; the manager must reconcile Gmail first.
- The Gmail message copy is proposed for deletion after 180 days while the canonical refund case continues under Bloomjoy's governed business-record retention. The tokenized quarantine/delete design remains future-only and is not a pilot gate because attachments are disabled. Production enablement requires recorded Operations and privacy/security approval of copied-content retention and the attachment-off boundary.
...
The 2026-08-03 email-assistance decision supersedes the blanket manager-approval rule above only for explicitly approved, immutable deterministic template classes. GPT and materially free-form prose remain human-reviewed. Retention cleanup is independently gated and must remain able to purge eligible local copies after Gmail OAuth is revoked.
...
- Managers remain the business approver. Automation may send only the later-approved deterministic message classes, remind/escalate stale cases, and prepare payment execution; it cannot send editable approval/completion success copy or treat manager approval as provider success.
...
- `refund-case-admin-update` is the preferred manager update path for coherent non-provider state changes. It may not complete an eligible card case or send card-success copy; that boundary belongs only to the token-bound provider-success settlement.
