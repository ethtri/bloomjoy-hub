...
- Portal invitation and Timekeeping compensation setup are two explicit manager steps: the Technician first accepts the invitation and signs in once, then an account pay manager uses one atomic setup action to choose machines and define simple pay arrangements. The manager enters standard pay once and adds machine exceptions only when needed. A failed setup leaves no profiles, assignments, or rates partially created.
