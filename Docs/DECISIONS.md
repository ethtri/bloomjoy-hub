...
- Sunze imports must reconcile the visible Orders UI record count and revenue against the downloaded export before ingesting, and must fail closed if the selected date window, payment/status mappings, task selection, or export integrity cannot be verified.
...
- The Sunze UI exposes date presets and a repaired custom range flow. Daily scheduled imports stay on `Last 7 Days`; historical backfills may use explicit monthly custom date ranges only through the Export Task List flow, with all exported sale dates verified inside the requested window.
