...
- A mapped-manager login alone is not enough for an official action. The draft `#692` boundary uses one two-minute, single-live-per-actor intent bound to the exact actor, case, action/target, case and mapping versions, owner-approved enrollment version, amount, payload hash, and transaction/evidence fingerprint. The manager personally verifies the exact approved TOTP factor after review; only an unambiguous newest TOTP authentication strictly newer than the intent may create one server-only, single-use proof and one receipt. Replay, concurrent consume, same-second/stale/refresh-only evidence, or enrollment/case/mapping/evidence drift fails closed. Agent-controlled or shared browser sessions are not valid payment-action sessions, and production controls remain hard-off pending the separate gate-on review and owner UAT.
