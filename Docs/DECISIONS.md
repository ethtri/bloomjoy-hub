...
- A mapped-manager login alone is not enough for an official action. Receipt minting must fail closed unless the session carries a fresh human-entered TOTP verification, and `#692` must bind that per-action challenge to the exact reviewed case, action, version, and evidence before official controls can be enabled. Agent-controlled or shared browser sessions are not valid payment-action sessions.
