...
- After machine resolution, every Gmail-linked case-specific customer-facing refund reply must originate through the designated support mailbox in the original Gmail thread. Every manual or automatic Gmail or transactional case-specific customer message requires a complete route for one to four active, non-revoked Machine Managers returned by the authoritative portal mapping at send time. A manager who is also the customer is represented once through To; every other manager is visibly copied. The generic hosted-form acknowledgement is the sole pre-mapping exception. Unresolved, zero-manager, invalid/fifth-manager, mailbox-colliding, empty, or changed routes fail closed before every later Gmail or transactional provider delivery and create a redacted internal routing exception. The capped operations fallback is internal-only and can never replace the required customer-message route.
