...
- Use progressive disclosure. Show the simple formula and warning summary in the browser; put full audit detail in expandable sections and the PDF appendix.
...
- A partner viewer can open the Partner dashboard for permitted partnerships, view approved report snapshots, and download approved PDFs only when the snapshot status allows it.
- A partner viewer cannot change machine assignments, tax assumptions, financial rules, import runs, raw sales facts, schedules, recipients, or generated snapshot contents.
...
- Status rail:
  ...
  - Warning count with a link to warnings.
...
- Warnings: shadcn `Alert`, `Badge`, `Tooltip`, and `Accordion`/`Collapsible` for detail.
...
1. **Settlement answer**
   ...
   - Status: `Preview`, `Warnings`, `Ready for review`, `Generated`, `Reviewed`, or `Downloaded`.
...
3. **Trust panel**
   - Last import, latest sale date, rule version, tax method, fee basis, split basis, generated by, and warning count.
4. **Machine rollups**
   - Machine label, location, gross sales, tax, fee/cost deductions, net sales, split base, partner owed, and warning chips.
...
6. **PDF review workflow**
   - Preview, warnings, generate snapshot, review PDF metadata, download, and manual send handoff.
...
- Warning filter: all, blocking, non-blocking, resolved.
- `Generate PDF snapshot` is disabled when blocking warnings exist.

### Warning Model
Warnings should be visible before financial numbers are trusted.
...
- **Stale data**: show dashboard values with a prominent freshness warning and block PDF generation if stale data affects the selected period.

## PDF Export And Review Flow
### V1 Flow
1. Super-admin selects Partner dashboard, partnership, and reporting period.
2. System loads a browser preview with calculation strip, machine rollups, freshness state, and warnings.
3. Super-admin resolves blocking warnings or changes the period/partnership.
4. Super-admin clicks `Generate PDF snapshot`.
5. System creates an immutable snapshot/run with reporting period, rule version, assumptions, generated-by user, status, storage path, warning summary, and snapshot ID.
...
- Whether units sold means orders, sticks/items, or both for partner reporting.
...
- How warning severity is represented by the backend.
...
- Shows amount owed, gross sales, tax impact, net sales, fees/costs, split base, partner share, Bloomjoy retained share, warnings, and machine rollups.
...
- Generated PDF workflow exposes snapshot ID, generated timestamp, generated-by user, warning state, download action, and manual send handoff.
- Browser dashboard remains the interactive review workspace.
- PDF remains the formal settlement artifact with appendix-level calculation detail.
