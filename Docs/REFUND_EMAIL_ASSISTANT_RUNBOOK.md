...
- The Bloomjoy hosted form is the only refund-intake path and requires a valid email address. When a customer first texts the Bloomjoy number, the existing one-way link handoff supplies the form outside the Hub; it is not a staff/manual text step. The Hub has no SMS ingestion, ongoing SMS conversation, post-form SMS, or new SMS provider dependency. After submission, existing guarded automation sends routine supported customer email while managers or humans make the final business decision and handle named exceptions; customer contact alone never creates a Hub case.
...
9. When a case reaches an immediate action-ready, exception, or escalation transition, the existing internal email transport sends the current mapped Machine Managers one privacy-safe HTML and plain-text notice. Its actor, action owner, action code, and queue state come from the same server lifecycle contract as the portal; it includes only approved decision context and canonical case/queue links. Opening a link performs no action. Customer contact details, complaint text, card digits, provider identifiers/payloads, diagnostics, and attachments are omitted. Intake remains portal-only, and quieter coalesced delivery belongs to the separately tracked digest work.
...
Before this candidate can be considered release-ready, the same fresh workflow run must create exactly 115 reviewed synthetic screenshots plus these five strict, sanitized JSON artifacts:
...
GPT, TOTP, operator ceremonies, QR-code flows, Kexiazhan reporting, and cash fallback are not Refund Operations v1 pilot dependencies. The existing one-way text-to-link handoff remains outside the Hub; Hub SMS conversation, continuation, and provider integration are retired rather than deferred pilot work.
