...
- [ ] After the intended GitHub `Production` deployment reports its latest status as `success`, run `npm run refunds:release:verify-portal -- <deployment-id>` with the read-only `VERCEL_TOKEN`. Save the result in #1432: canonical `https://app.bloomjoyusa.com/refunds` URL, observation time, full SHA, Vercel deployment/project ID, successful main CI run ID, and checked index/Vite-asset SHA-256 values. The validator independently binds the canonical Vercel alias to a READY Production deployment in the fixed project, exact GitHub Production target URL/SHA, and successful main CI run; then it reads the canonical index and every manifest-referenced asset against the served metadata. Wrong alias/project/SHA, missing file, redirect, or changed bytes fail. `ciArtifactMatch` reports independent CI-byte equality separately; different Vercel build inputs can make it false without making served bytes internally inconsistent. `trackedSourceClean` reports literal byte cleanliness. Vercel has been observed to reserialize only `vercel.json` at checkout; `trackedSourceEquivalent` is true for that sole change only when canonical JSON matches the committed file exactly, and remains false for any other changed or untracked input. Null means unavailable. The command reports `sourceEvidenceVerified=false` and exits nonzero when neither equivalent complete build inputs nor identical independent CI bytes are proven, while retaining the separate served-byte result for diagnosis. Record these distinctions explicitly and do not infer byte equality or a clean build from a source SHA alone. This does not replace the Supabase migration/function checks above.
