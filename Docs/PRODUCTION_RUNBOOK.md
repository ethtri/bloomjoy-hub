...
- Prove with synthetic recipients that a resolved machine CCs only currently active mapped managers, a manager Reply All remains manager correspondence, the customer sees no internal portal link, and the separate manager notice opens the canonical `/refunds?case=<case-id>` route without changing case state.
...
- Force a synthetic hard bounce and confirm later automatic customer contact pauses while the mapped manager receives a safe exception with the exact case link.
