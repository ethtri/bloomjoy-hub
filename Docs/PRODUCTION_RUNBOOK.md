...
| `NAYAX_REFUND_ATTEMPT_QUEUE_ENABLED` | Server-only | `nayax-card-refund`, `refund-case-admin-update`, `refund-case-automation-sweep` | Keep `true` during healthy card-refund operation. Set `false` only for a recorded incident or rollback; approval fails before it is saved while this is off. | Release owner |
| `NAYAX_REFUND_ATTEMPT_QUEUE_ACCOUNT_KEY` | Server-only | `nayax-card-refund`, `refund-case-admin-update`, `refund-case-automation-sweep` | Normalized exact account processed by the bounded attempt queue. Approval fails before it is saved when the case account differs. | Technical owner |
