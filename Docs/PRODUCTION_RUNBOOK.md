...
- For an owner-approved synthetic shadow run, set `REFUND_MANAGER_AGING_NOTICES_ENABLED=true` without changing customer-contact switches. Prove waiting and verified-reply suppression, mapping repair/revocation between milestones, current-manager-only delivery, one capped redacted operations exception at zero managers, canonical `/refunds?case=` navigation without an official action, and durable review with no retry when delivery is uncertain. Evidence must be aggregate and omit customer identity, complaint text, card digits, transaction/provider identifiers, and raw payloads.
