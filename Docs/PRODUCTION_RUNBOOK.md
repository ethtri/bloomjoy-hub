...
- Prepare a private env file outside the repository. Set the exact production project twice (`REFUND_SYNTHETIC_GMAIL_PROOF_PROJECT_REF` and `REFUND_SYNTHETIC_GMAIL_PROOF_CONFIRM_PROJECT_REF`), the privately selected case UUID twice (`..._CASE_ID` and `..._CONFIRM_CASE_ID`), a database-owner connection, an owner-held Supabase management token able to read/write Edge secrets, read function metadata, and read backups (`backups_read`), the production publishable/anon key, and a current authenticated portal-user JWT whose user can manage that exact case. Never put this file in the repo, an issue, chat, shell history, screenshot, or artifact. There is no recipient, subject, body, alias, endpoint, attachment, or retry input.
...
- In one fresh workflow run, generate exactly 43 reviewed synthetic screenshots and exactly five sanitized JSON artifacts: `refund-portal-assertions.json`, `refund-database-counts.json`, `refund-gmail-mime-roles.json`, `refund-kill-switches.json`, and `refund-provider-outcomes.json`. The owner-authenticator readiness screenshot is pre-QR only; never add a screenshot of the open private setup dialog.
