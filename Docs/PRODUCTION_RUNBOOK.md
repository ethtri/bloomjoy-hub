...
3. Run the aggregate-only Nayax mapping smoke. It confirms every active refund-intake machine has one non-duplicate account/machine mapping, one to three active managers, and a location timezone, and that the live-enabled count exactly matches the reviewed pre-launch or activated count. The SELECT-only audit prints no identifiers, writes no records, makes no provider call, and does not replace the later transaction-lookup or controlled-execution evidence:
...
- First run `npm run refunds:synthetic-gmail-proof -- --mode dry-run --env-file <private-absolute-path>`. It performs aggregate-only checks: database-owner session; zero unclosed authorization; zero unresolved Gmail outbound; exact eligible Gmail/owner-plus-address case; one original thread; zero attachments; one-to-three complete current mapped managers; authorized current user; a latest completed production backup no more than 36 hours old; exact production function alignment; and every unrelated Edge/GitHub/database gate off. The backup proof is a read-only exact-project Management API check and does not assume PITR is enabled. It writes nothing and prints only booleans/counts.
...
- Provision only the dedicated account-scoped `NAYAX_REFUND_REQUEST_WRITE_TOKEN_<ACCOUNT_KEY>` and `NAYAX_REFUND_APPROVE_WRITE_TOKEN_<ACCOUNT_KEY>` secrets. Never provide the reporting or generic Nayax token as a write fallback. Set the exact reviewed `NAYAX_REFUND_MANAGER_CONTRACT_JSON` only after `NAYAX_REFUND_MANAGER_CONTRACT_CONFIRMED=true` and `NAYAX_REFUND_APPROVAL_SCOPE_CONFIRMED=true` have independent evidence.
