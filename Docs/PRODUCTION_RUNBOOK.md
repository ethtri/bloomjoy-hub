...
- [ ] The same fresh `Refund UAT Evidence` run contains exactly 42 reviewed synthetic screenshots and the five sanitized JSON artifacts named below; the final manifest hashes every artifact and binds to the reviewed PR head. The evidence covers form-only intake, Nayax inventory/Snapcase, branded messages and same-case appeals, duplicate decisions, and the source-aware manager queue without production data. Final migration/test-file counts and SHA are generated from that tree, not copied from an earlier branch or written by hand.
...
- In one fresh workflow run, generate exactly 42 reviewed synthetic screenshots and exactly five sanitized JSON artifacts: `refund-portal-assertions.json`, `refund-database-counts.json`, `refund-gmail-mime-roles.json`, `refund-kill-switches.json`, and `refund-provider-outcomes.json`. The set covers the current form-only, inventory/Snapcase, branded-message/appeal, duplicate, and source-aware queue states and contains no production data, provider identifier, QR, or TOTP.
