...
- In one fresh workflow run, generate exactly 90 reviewed synthetic screenshots and exactly five sanitized JSON artifacts: `refund-portal-assertions.json`, `refund-database-counts.json`, `refund-gmail-mime-roles.json`, `refund-kill-switches.json`, and `refund-provider-outcomes.json`. The set covers the current form-only and QR intake, card-network evidence, inventory/Snapcase, manual-portal-only machine state, branded-message/appeal, duplicate, source-aware queue, unsent manager-draft navigation protection, routine-manager isolation, Internal/test disposition/archive, inbound existing-case linking review, provider-free existing-refund states and contains no production data, provider identifier, or TOTP.
