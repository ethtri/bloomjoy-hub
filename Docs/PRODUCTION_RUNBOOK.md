...
supabase secrets set NAYAX_REFUND_PROVIDER_TIMEOUT_MS=10000
...
Do not set `NAYAX_REFUND_EXECUTION_SPONSOR_GO_NO_GO`, `NAYAX_REFUND_PROVIDER_CONTRACT_JSON`, or any `NAYAX_REFUND_API_TOKEN_<ACCOUNT_KEY>` write credential during shadow-mode setup. The sponsor value stays unset until a separate live card-refund execution pilot is explicitly approved. The provider contract and separate write credential stay unset until controlled account-specific evidence confirms write authority and exact public-Lynx `Result`/`Status` pairs for every configured outcome. Bloomjoy's 2026-07-27 Core audit confirmed major currency units but did not identify a QA account or endpoint-level token scopes.
...
- The manager's **Approve refund** request records the approved high-confidence recommendation before the execution gates are evaluated. A disabled or misconfigured environment may therefore show `Approval saved; refund not confirmed`; it must not complete reporting or send success confirmations.
...
- Prefer a Bloomjoy Nayax QA transaction when one is available. The 2026-07-27 Core audit found no Bloomjoy QA route, so the current fallback is a separately sponsor-approved, allowlisted, capped low-value production pilot. Record only HTTP status plus normalized `Result` and `Status`; do not record raw payloads, tokens, transaction IDs, site IDs, machine times, or customer data.
...
- Confirmed approval success must atomically mark the attempt succeeded, complete the case, write one reporting adjustment, and create exactly two service-only outbox rows: customer and approving manager. The function attempts them immediately; `refund-case-automation-sweep` retries pending/failed delivery at 15-minute intervals, no more than three attempts and only inside 23 hours. Each audience has one database row and one deterministic Resend idempotency key. Email delivery failure never permits a second payment attempt.
- If the refund succeeded but either confirmation remains `pending` or `failed`, leave the payment alone and restore/trigger the normal automation sweep. After the bounded window the row becomes `manual_review`; resolve communication only, never resend the refund.
