...
- [ ] The same fresh `Refund UAT Evidence` run contains exactly 45 reviewed synthetic screenshots and the five sanitized JSON artifacts named below; the final manifest hashes every artifact and binds to the reviewed PR head. The owner-authenticator image is captured before private QR setup opens and contains no QR or code. The support-resolution desktop/mobile images contain only synthetic structured evidence and are captured before synthetic verification. Final migration/test-file counts and SHA are generated from that tree, not copied from an earlier branch or written by hand.
...
6. In the mapped manager's portal, refresh the exact private case. Require **Payment history check**, **Historical payment review required**, **No provider refund is recorded**, and only the read-only fresh transaction-check path. Confirm even an adversarial stale browser response cannot display or select an old candidate or hide the fresh-check button; refund/retry, approve, deny, and customer-email paths remain frozen on desktop and mobile.
...
- Do not perform the remaining case-specific proof as a manual dashboard/portal ceremony. The portal intentionally has no proof-token control, and a normal **Reply in Gmail thread** click cannot carry the one-shot authorization. Use only the owner runner from `#810`; it keeps the browser free of recipient, case, copy, token, and gate controls.
...
Default-off Nayax outcome resolution (`#767`):
- Deploy the reviewed migration and updated manager-step-up function only with `refund_nayax_outcome_resolution_enabled()` still returning immutable `false` and `refund_nayax_resolution_operators` empty. Do not add an operator, change the gate, run a resolution, retry a provider call, or send a customer message as part of deployment.
- Run `npm run refunds:validate-nayax-resolution`, `npm run db:validate-migrations`, and the focused desktop/mobile portal UAT. Require one winner under a genuine two-session race, one immutable outcome row, original provider outcome preserved, zero new provider attempt, zero customer message, exact function/database-owner write provenance, current manager/operator/enrollment version binding, and fresh exact-factor proof.
...
- Each operator review must use exactly one structured result/evidence/reason tuple and a bounded reference only. **Keep hold** changes no outcome. **Safe for fresh review** makes no provider retry. **Provider success** or **documented manual completion** may commit case/reporting only after exact evidence and still sends no email. A later customer completion is independently claimed only from the fully committed state.
- Stop on expired/changed intent, actor/mapping/operator/enrollment drift, ambiguous evidence, missing exact attempt, stale case version, or inability to prove zero provider/message side effects. Never repeat an uncertain resolution; inspect the immutable record and current case/attempt state first.

Integrated Refund UAT evidence:
- In one fresh workflow run, generate exactly 45 reviewed synthetic screenshots and exactly five sanitized JSON artifacts: `refund-portal-assertions.json`, `refund-database-counts.json`, `refund-gmail-mime-roles.json`, `refund-kill-switches.json`, and `refund-provider-outcomes.json`. The owner-authenticator readiness screenshot is pre-QR only; never add a screenshot of the open private setup dialog. The two payment-support screenshots must remain structured pre-action desktop/mobile states and contain no real evidence reference, provider identifier, QR, or TOTP.
