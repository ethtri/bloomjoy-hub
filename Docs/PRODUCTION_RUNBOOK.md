...
Agents start with `Docs/REFUND_AGENT_OPERATIONS.md` and
`Docs/NAYAX_REFUND_WORKING_CONTRACT.md`. Historical sections below preserve
earlier rollout evidence and are not current activation gates.

### Normal refund

1. Triage saves one exact selected Nayax transaction from Last Sales. Customer amount is a clue; the saved provider amount is the refund amount.
2. The assigned Machine Manager or a Super-admin selects **Refund $X** and confirms once. The database atomically consumes that approval and creates exactly one System-owned attempt with frozen execution context.
3. The System worker claims that attempt, calls Nayax at most once, and settles the same row. A no-call claim expiry may reclaim only that row. Any provider-started expiry, malformed claim, timeout, unknown result, or post-transport failure places that same row on permanent hold.
4. Evidence may later confirm success or document why the attempt remains held. It never releases the transaction, creates a retry generation, or authorizes a manual card completion. Cash refunds remain manager-manual.
...
- [ ] The fresh `Refund UAT Evidence` run contains exactly 125 reviewed synthetic screenshots and covers the API-blocked machine state, exact selected transaction, one manager confirmation, one System attempt, permanent holds, and cash-manual flow without production data or provider identifiers. Final migration/test-file counts and SHA are generated from that tree.
...
The controlled-owner runner and its documentation are retired. Its tables remain historical only and none of its writer RPCs are deployment or operating tools. Current operation is governed by `Docs/REFUND_PRODUCTION_POLICY.md` and the first section of this runbook.
...
The manifest's `sourceGitCommit` is checked against every current function's transitive source. `preDeploymentProduction` records the historical eight-function live baseline, including missing functions. `approvedRestoreSource` validates that historical known-good source; disable-only functions such as `refund-gmail-sync` and `refund-gpt-triage` record `restoreAction=disable` and use their documented switch-off procedures instead of pretending an older deployed source existed. `additionalFunctionBaselines` separately validates the live outcome resolver's captured source and matching restore commit; omitting it fails current release validation.

A squash merge may retain the reviewed branch's source pointer. If that source is no longer an ancestor, the check requires a commit in the current first-parent history whose manifest names that exact source and whose entire Git tree differs from it only at `scripts/refunds/refund-production-release.json` (the separate source-pinning commit). File additions, removals, renames, mode changes and even documentation differences at that anchor fail equivalence. The proof records the canonical equivalent anchor without rewriting the manifest, production receipts or restore history. The source object must remain locally verifiable; missing history fails closed. The clean worktree, exact canonical-main deployment, project, function/migration digest, review, Auth and downloaded-source checks still apply. Later release-neutral documentation follows the rule below.

GitHub permits squash merges and deletes merged branches. CI, production drift and the guarded canonical deploy entrypoint explicitly use `--fetch-reviewed-source` to retrieve an absent pinned 40-character source commit from exactly `https://github.com/ethtri/bloomjoy-hub` (with or without Git's `.git` suffix). This reads the public repository without adding credentials, fetching another branch or changing the source pointer. Retrieval alone never proves provenance; unavailable objects and non-equivalent canonical source fail the normal checks. Local checks remain offline by default; on a fresh checkout, use `npm run refunds:release:check -- --fetch-reviewed-source` before the tooling suite when that exact object is absent.

A valid manifest anchor remains current across later documentation-only changes in root Markdown files, `Docs/`, `.agents/`, and `.codex/agents/`. Ordinary checks and any required independent policy review still run; prose alone does not repin deployed source or rerun refund screenshots. Every code, asset, configuration, dependency, workflow and unrecognized path stays protected, including new or renamed shared dependencies outside refund-named folders. Both sides of a rename/deletion are checked. Only a successful exact-base/head comparison proving neutral changes may skip Refund UAT; missing or failed scope evidence cannot skip it. The local digest comparison independently verifies every tracked function and migration. A changed integrated runtime/tooling release still requires a reviewed source anchor and full applicable validation before authorized deployment. This rule does not grant deployment or sending authority.
...
- Deploy database changes before dependent functions while execution is disabled, dry-run is enabled, and the kill switch is active. Open the runtime gates only after the exact reviewed source and schema are aligned.
...
- Read-only lookup and exact evidence selection remain usable. Manual Nayax portal candidates are historical and cannot authorize a card refund. For an approved attempt, the System uses the exact selected original transaction and full original amount.
...
- Require one exact-refund manager decision, exactly one System-owned attempt, at most one request and one approval, one terminal settlement or permanent hold, and exactly-once reporting/customer completion. Repeat clicks, reloads, workers, and replays cannot create another provider send.
- Rejection, timeout, unknown outcome, or any error after transport permanently holds the same attempt. Evidence can confirm success or document the continuing hold; it cannot create a new provider attempt. Unrelated transactions and customers continue normally.
...
- Each evidence review must use exactly one supported tuple and a prefixed non-sensitive reference. Confirmed success is either DTM evidence with `nayax_dtm_settled` or support evidence with `nayax_support_confirmed_success`. Remaining on hold uses `evidence_incomplete`, `provider_still_pending`, or `evidence_conflict`. The UI and database reject card-, account-, contact-, and customer-like values; only the documented `SUPPORT:NAYAX-########`, `SUPPORT:NAYAX-CS#######`, and 9-digit `DTM:NAYAX-#########` vendor shapes are exceptions. Durable evidence stores the type, reason, occurrence time, and reference digest. It may confirm the same attempt's success or leave it permanently held; it never releases, retries, or creates another card-refund attempt.
...
- In one fresh workflow run, generate the reviewed synthetic screenshots and five sanitized JSON artifacts. The set includes the API-blocked machine state, exact selected transaction, single confirmation, System-owned attempt, permanent hold, duplicate prevention, and cash-manual flow, and contains no production data or provider identifier.
