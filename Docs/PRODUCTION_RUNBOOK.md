...
A valid manifest anchor may remain current across later release-neutral repository changes. Release-neutral means the change is outside the protected boundary enforced by `scripts/refunds/refund-release.mjs`; examples include `AGENTS.md`, `README.md`, and unrelated local-development guidance. Any change to frontend source, Supabase functions/migrations/tests/configuration, refund tooling, dependency manifests, refund workflows, this production runbook, the refund cutover packet, or durable refund decisions remains protected and requires a new reviewed manifest-only anchor. The local digest comparison still independently verifies every tracked function and migration. Before production activation, build one final integrated refund release, refresh its anchor, and rerun the full release, evidence, migration, and production-drift checks.
...
Bounded Nayax outcome resolution (`#767`, `#427`):
