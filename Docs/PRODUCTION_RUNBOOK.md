...
- Approve and record the 180-day Gmail-copy retention, quarantine-until-malware-cleared behavior, visible-CC privacy model, participant classification, and first-contact cutover in `Docs/REFUND_GMAIL_DATA_HANDLING.md`. Do not enable the schedule while any approval is pending.
...
- Confirmed synthetic success must atomically commit provider outcome, case completion, and reporting before the DB-owned `refund_nayax_completion_v1` message can be claimed in the original Gmail thread. The full current manager set is visibly CC'd and no separate manager completion email is created.
...
- In one fresh workflow run, generate exactly 34 reviewed synthetic screenshots and exactly five sanitized JSON artifacts: `refund-portal-assertions.json`, `refund-database-counts.json`, `refund-gmail-mime-roles.json`, `refund-kill-switches.json`, and `refund-provider-outcomes.json`.
