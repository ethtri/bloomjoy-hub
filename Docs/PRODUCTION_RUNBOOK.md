...
1. Search Bloomjoy and Nayax records before asking the customer for more information.
   - If lookup reports missing machine mapping, account scope, or account access, follow Step 7 of `Docs/REFUND_AGENT_OPERATIONS.md`: finish any safe research, use the reviewed manual Nayax portal fallback, and create or link the privacy-safe Engineering issue needed to fix the portal. Confirm the exact reporting machine and location-scoped Nayax account. A separate account must never use the default account credential. Run at most one server-authorized read-only retry for the current fact version. Do not ask the customer to repeat the selected machine, location, or purchase facts Bloomjoy already holds.
...
- [ ] The same fresh `Refund UAT Evidence` run contains exactly 125 reviewed synthetic screenshots and the five sanitized JSON artifacts named below; the final manifest hashes every artifact and binds to the reviewed PR head. The evidence covers form-only and QR intake, card-network evidence, Nayax inventory/Snapcase, exact published Valley Mall product-unverified inventory on desktop/mobile, the manual-portal-only machine state, branded messages and same-case appeals, duplicate decisions, the source-aware manager queue, unsent manager-draft navigation protection, routine-manager isolation, the shared manager-work projection on desktop and at mobile 200% zoom, the Internal/test disposition/archive, inbound existing-case linking review, provider-free existing-refund reconciliation, exact-message delivery recovery, incomplete-history retry/fallback, persisted manager preparation, transactional delivery truth, and truthful customer-outreach lifecycle states without production data or provider identifiers. Final migration/test-file counts and SHA are generated from that tree, not copied from an earlier branch or written by hand.
...
- In one fresh workflow run, generate exactly 125 reviewed synthetic screenshots and exactly five sanitized JSON artifacts: `refund-portal-assertions.json`, `refund-database-counts.json`, `refund-gmail-mime-roles.json`, `refund-kill-switches.json`, and `refund-provider-outcomes.json`. The set covers the current form-only and QR intake, card-network evidence, inventory/Snapcase, exact published Valley Mall product-unverified inventory on desktop/mobile, manual-portal-only machine state, branded-message/appeal, duplicate, source-aware queue, unsent manager-draft navigation protection, routine-manager isolation, the shared manager-work projection on desktop and at mobile 200% zoom, Internal/test disposition/archive, inbound existing-case linking review, provider-free existing-refund states, exact-message delivery recovery, incomplete-history retry/fallback, persisted manager preparation, and truthful customer-outreach lifecycle states, and contains no production data, provider identifier, or TOTP.
