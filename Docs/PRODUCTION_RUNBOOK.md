...
- Soak both lanes for at least two hours. Require at least four primary sweep successes, no successful-run gap reaching 60 minutes, no duplicate run key, no concurrent case action, and no duplicate customer/manager/provider effect. Health must remain non-stale through 90 minutes since the last success, then open one incident after that threshold; the ledger must show no repeat inside 24 hours and one recovery claim only after 60 continuous healthy minutes.
