...
2. In a private database-owner session after confirming a production backup, run an aggregate-only precheck. Require exactly one card case with `card_refund_pending`, `decision=approved`, provider state `not_requested`, exactly zero provider-attempt rows, exactly one total/sent approved message, zero pending or completed messages, and no completion/reference/adjustment evidence. If the count is not exactly one, stop; do not broaden the predicate.
...
4. Save only the returned aggregate booleans/counts: normalized, already-normalized, status, decision, provider-attempt count, historical approved-message count, provider execution state, and payload-redacted. Never save the case UUID, customer identity, card details, provider identifiers, or message content as evidence.
5. Run aggregate-only postchecks. Require one append-only `legacy_card_state_normalized` event; current state `needs_review` with no current decision, refund amount, selected Nayax match, recommendation, or cached lookup candidate; provider `not_requested`; zero provider attempts; the original approved message unchanged; zero new customer messages; and zero provider, completion, reporting-adjustment, or official-action side effects. The redacted event must retain the prior decision and only presence-level match/recommendation facts plus a stale-candidate count, never a candidate token or provider identifier.
