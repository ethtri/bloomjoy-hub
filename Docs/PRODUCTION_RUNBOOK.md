...
6. Review each function's `ACTIVE` status, version, `verify_jwt`, bundle digest, and approved source digest.
7. Update `scripts/refunds/refund-production-release.json` through a reviewed PR. Do not treat the capture as automatic approval.
8. Run `npm run refunds:release:check-production -- --project-ref <project-ref>` and require all eight functions to pass.
9. Run the remaining refund production smoke rows in `Docs/QA_SMOKE_TEST_CHECKLIST.md` using sanitized evidence only.
...
- Manually run **Refund Automation Sweep** with `failure_test` and a new non-sensitive `run_key` (8-120 letters, numbers, underscores, or hyphens). Confirm the designated operations recipients receive the PII-free test alert and the workflow prints aggregate fields only. Dispatch `failure_test` again with the exact same `run_key`; require `duplicate_suppressed` and no second alert.
- With approved synthetic/shadow cases only, set Edge secret `REFUND_AUTOMATION_ENABLED=true`, manually run **Refund Automation Sweep** with `run` plus a new non-sensitive `run_key` during the configured policy window, and confirm each due action fires once. Dispatch `run` again with the exact same `run_key`; require `duplicate_suppressed` without another message, state change, or event. GitHub creates a new workflow run for the replay, so the reusable input—not `GITHUB_RUN_ID`—is the idempotency proof.
