...
| `REFUND_MANAGER_AGING_NOTICES_ENABLED` | Server-only | `refund-case-automation-sweep` | Independent default `false`; enable only for approved synthetic/shadow manager-notice proof | Release owner |
| `REFUND_MANAGER_REMINDER_BUSINESS_DAYS` | Server-only | `refund-case-automation-sweep` | Default `2`; Monday-Friday threshold in `REFUND_AUTOMATION_TIMEZONE` | Operations owner |
| `REFUND_MANAGER_ESCALATION_BUSINESS_DAYS` | Server-only | `refund-case-automation-sweep` | Default `5`; Monday-Friday threshold in `REFUND_AUTOMATION_TIMEZONE` | Operations owner |
...
| `REFUND_GMAIL_RETENTION_ENABLED` | Server-only | `refund-gmail-sync` retention worker | Default `false`; enable only after the owner-approved local-copy policy is recorded | Privacy owner |
| `REFUND_GMAIL_RETENTION_POLICY_VERSION` | Server-only | `refund-gmail-sync` retention worker | Must match the reviewed database policy; current proposal is `refund_gmail_retention_v1` | Privacy owner |
| `REFUND_GMAIL_ATTACHMENT_SCANNER_ENABLED` | Server-only | `refund-gmail-sync` copy gate | Default `false`; missing or disabled scanner configuration blocks all new Gmail copies | Privacy/security owner |
| `REFUND_GMAIL_ATTACHMENT_SCANNER_VERSION` | Server-only | `refund-gmail-sync` copy gate | Exact reviewed quarantine/scanner pipeline version; must match database policy | Privacy/security owner |
...
| `REFUND_GMAIL_RETENTION_ENABLED` | GitHub Actions variable | Refund Gmail Sync retention-only job | Default `false`; after approval, keep enabled until the last local copy has passed retention and cleanup is healthy | Privacy owner |
...
- Set `REFUND_AUTOMATION_SWEEP_URL` and `REFUND_AUTOMATION_SWEEP_TOKEN`. Keep GitHub variable `REFUND_AUTOMATION_SWEEP_ENABLED=false`, Edge secret `REFUND_AUTOMATION_ENABLED=false`, and Edge secret `REFUND_MANAGER_AGING_NOTICES_ENABLED=false` during setup.
...
- Keep manager aging disabled until `npm run refunds:validate-manager-aging`, `npm run db:validate-migrations`, and the exact-case browser UAT pass. The default contract is one manager-only reminder at two business days and one manager-only escalation at five, measured Monday-Friday at the attention anchor's local clock time in `America/Los_Angeles`. No holiday calendar is inferred. The 30-minute initial-triage target remains a staffing decision and is not a customer promise.
- For an owner-approved synthetic shadow run, set `REFUND_MANAGER_AGING_NOTICES_ENABLED=true` without changing customer-contact switches. Prove waiting and verified-reply suppression, trusted database-created/message-ID reply ordering, an A-to-B manager remap immediately before the final locked reservation (transport receives B only), one capped redacted operations exception at zero managers, and fail-closed behavior for an over-cap fallback. Open an eligible pending-card canonical `/refunds?case=` link and prove it invokes no Nayax lookup or official action while search/filter changes remain independent; queue selection must also remain navigation-only, and only a separate explicit advanced-tool request may invoke lookup. Also prove durable review with no uncertain retry, that more than 100 resolved rows do not hide the next due case, and that an in-flight/unknown attempt blocks all newer versions until its exact bound settlement. Persisted/logged/audit evidence must be aggregate, use only the non-address mapping-row-ID/policy fingerprint, and omit customer identity, recipient addresses, complaint text, card digits, transaction/provider identifiers, and raw payloads.
- Treat `notice_attempt_in_flight` and `delivery_unknown` as fail-closed recovery states. Do not clear the state or retry the notice manually. An authorized engineering recovery must reconcile the exact stored action key as known sent or known not sent; that settlement is redacted and auditable, clears the case-wide hold even after a version change, and never marks a newer version's milestone. If delivery cannot be proven either way, leave the hold in place and handle the case in the portal.
...
- Quick disable without taking down the refund workflow: set `REFUND_MANAGER_AGING_NOTICES_ENABLED=false` to stop only manager aging notices. For the whole scheduler, first set `REFUND_AUTOMATION_SWEEP_ENABLED=false`, then set Edge secret `REFUND_AUTOMATION_ENABLED=false`. Manually dispatch one `run` with a new synthetic UUID in `run_key` to record/confirm `disabled`; verify `/refunds` intake and manager actions still work. Re-enable only after the linked incident is resolved and synthetic proof passes again.

Refund Gmail intake validation:
- Apply the current Gmail migrations and deploy `refund-gmail-sync` with every Gmail, retention, and scanner switch `false`. Set only the encrypted workflow URL/token during this step. Do not configure OAuth or copy a test message yet.
- Production Gmail has never been enabled and remains unconfigured, so `select count(*) from public.refund_gmail_sync_runs;` must return `0` before applying the retention migration. Record only that aggregate count. The migration fully validates the trigger-bound numeric GitHub run-key constraint and stops if a surprise legacy row exists; do not print, preserve, or reinterpret an arbitrary legacy key. Resolve it through a separately reviewed redaction migration before retrying.
- Operations and privacy/security owners approve and record the proposed 180-day local-copy policy and the exact quarantine/scanner version in `Docs/REFUND_GMAIL_DATA_HANDLING.md`. Record approval through a reviewed forward change to the owner-policy row; do not use an ad hoc browser or service-worker write.
- After that approval, set the server-only retention policy version and retention enable switch, then set the GitHub `REFUND_GMAIL_RETENTION_ENABLED=true`. Manually dispatch `retention` with Gmail credentials absent. Require `succeeded`, a fresh healthy cleanup state, aggregate counts only, and zero Google token or Gmail requests. This retention-only bootstrap is required before OAuth/provider setup.
- Before every attachment upload, require a service-only reservation that records the database-derived `refund-gmail-quarantine` bucket/path and returns a one-intent token. A returned upload success, explicit failure, or unknown result must settle against that token. If the worker stops after Storage accepts the byte but before settlement, the durable reservation is the deletion coordinate; do not reconnect Gmail or reconstruct the path from provider data. A noncanonical legacy path or target mismatch is a manual-review hold and must never be sent to Storage deletion automatically.
- If a byte deletion returns a known failure, leave the metadata intact and let a later unique scheduler run retry. If the outcome is unknown, stop: do not rerun that object, clear the ledger, or guess whether it exists. Cleanup may continue for unrelated due messages whose attachments are proven storage-free, but global cleanup health and the new-copy gate remain blocked. Keep Gmail sync off. The technical owner verifies the exact byte state privately in Supabase Storage while the privacy owner approves the disposition. If the byte remains, a reviewed one-time service change must delete it and settle the existing hold from exact success evidence. If it is confirmed absent, a reviewed forward migration must reconcile that one action and redact its metadata. Record only the aggregate outcome, reviewer roles, approval reference, and validation result in the incident/PR; never include an address, object key, attachment name, provider ID, or payment detail. There is intentionally no browser or automatic unknown-outcome recovery control.
...
- Inventory every approved Gmail send-as identity in `GMAIL_SUPPORT_SEND_AS_ALIASES`. Verify each address is configured on the designated account; classify an alias as mailbox-origin only when Gmail also supplies the provider `SENT` label. Keep all Gmail and recipient settings server-only.
...
- Configure every first-contact value listed in `Docs/REFUND_GMAIL_FIRST_CONTACT_CUTOVER.md` as a server-only secret. Include every verified Gmail send-as identity in `GMAIL_SUPPORT_SEND_AS_ALIASES` so support/refunds replies are always classified as outbound. Start with `REFUND_GMAIL_FIRST_CONTACT_MODE=disabled`; never use a `VITE_` name. The integrated participant boundary revalidates verified direct-customer evidence, case-wide delivery pauses, and one to three current mapped Machine Managers immediately before any first-contact provider send. Active mode still requires the documented owner-approved label, legacy-responder shutdown, exact UTC cutoff, and rollback gates.
...
- Configure the approved quarantine/scanner pipeline and set its exact server-side version only after it matches the reviewed database policy. Missing, disabled, mismatched, overdue, retry-required, or manual-review cleanup health must block the first and every later Gmail copy before OAuth access.
...
- Complete the recipient-boundary/privacy review before any pilot because visible CC exposes mapped-manager work addresses to the customer and other mapped managers. Use only owner-approved synthetic customer and manager identities for the pilot, and do not paste addresses into release evidence.
- Run the participant matrix with a direct customer, two active managers, a revoked manager, forwarded mail, spoof-suspected mail, an unknown sender, a mailbox alias, and an automated notice. Only the verified direct customer may change customer state or enter GPT triage; the others remain manager correspondence/system/unverified context.
- Change one machine-manager mapping between two synthetic sends. Confirm the second customer message has the customer as sole To and only the new current active mapping set in visible CC. A resolved route with safe exclusions may proceed only when at least one valid current manager remains. Zero-manager, unresolved-machine, invalid/over-cap mapping, or an empty filtered result must block both Gmail and transactional customer delivery before any provider call. This deliberately delays even a missing-machine question until ownership/routing is repaired; the redacted internal action notice is the recovery path.
- Trigger a synthetic delivery-status report whose first receiver result uses authserv-id `mx.google.com`, authenticates the claimed Google mailer-daemon/postmaster domain through aligned SPF or DKIM/DMARC, and binds the exact case customer, `Action: failed`, and permanent `Status: 5.x.x` in one recipient block. Also test unrelated-domain passes with reporter failures, non-Google authserv-id passes, delayed `4.x`, delivered/relayed/expanded actions, split recipient blocks, wrong recipients, header/subject-only signals, and failed actions without `5.x`. Only the exact-customer permanent hard bounce may pause automatic contact. Link a newer synthetic Gmail thread to the same case and confirm the older pause still blocks every automatic outbound claim and remains visible in manager context. Recovery must use the authenticated `admin_recover_refund_gmail_customer_contact` manager RPC only after deliberately verifying the case customer address; it must clear all linked pause rows in one transaction and add one actor-attributed redacted event. Confirm unauthorized, service-worker, newest-only, and partial clear attempts fail. A deliberate manual response remains available after address/thread review only when a current active mapped manager can be copied. Customer-facing content must contain no authenticated case link; separate action-needed/aging/exception notices use the canonical `/refunds?case=` link and go to current mapped managers, or to a capped operations fallback only for a routing exception. The fallback is internal-only, excludes the case customer and every mailbox identity, and can never substitute for the required customer-message CC. Completion sends no duplicate manager-only notice.
- Manager action notices inherit the source case/message/scheduler replay guard, but their email-provider delivery is not an exactly-once claim. If delivery is uncertain, do not automatically resend; reconcile the provider and case event, then use the documented manual recovery path.
...
- Send allowed and rejected synthetic attachments. PDF/JPEG/PNG files at or below 5 MB and within the three-file limit must reserve a tokenized UUID-derived path before upload and remain private/quarantined; unsupported, oversized, or excess files must be rejected without exposing content or paths. In local database tests, simulate Storage acceptance followed by a worker stop before upload settlement while Gmail configuration is absent: the possible byte and its message must remain until retention claims that exact reserved path and confirmed deletion settles it. In the same test, an unrelated due storage-free attachment/message must purge while aggregate health and new copying remain blocked by any unresolved object.
...
- Enable `REFUND_GMAIL_SYNC_ENABLED=true` only after every check above passes. Quick-disable provider intake by setting the GitHub sync variable `false`, then the Edge Gmail switch `false`, and revoke OAuth if compromise is suspected. Keep both retention switches on until the last copied item has passed the approved boundary and a clean retention-only run is recorded. Cleanup uses only local service/database/storage paths: it must never delete, archive, mark read, relabel, or otherwise mutate source Gmail. Hosted-form intake and existing case handling remain available.
...
Gmail-only rollback: first set `REFUND_GMAIL_SYNC_ENABLED=false`, then `REFUND_GMAIL_FIRST_CONTACT_MODE=disabled`, then `REFUND_GMAIL_ENABLED=false`; revoke the Gmail refresh token if compromise is suspected. Keep the approved retention worker and its schedule enabled so cleanup continues without Gmail OAuth/provider access. Verify Hub first-contact claims have stopped before restoring any legacy responder. Do not clear an unknown deletion hold, disable retention while copied data remains, delete Gmail linkage/first-contact/retention ledgers, or mutate the source Gmail mailbox during an incident. Reconcile uncertain operations and verify hosted-form refund intake and non-Gmail case work remain available.
