...
1. **Identify the purchase (#1161/#1162/#1165, with #890):** inspect only the payment fields the integration actually uses. Record unknown semantics explicitly. Deliver a small evidence-based selection policy; do not wait for exhaustive network/token research or optional PAR access. Treat authorization, settlement, report/import and provider-arrival times as separate supporting facts. Exclude a transaction only when proved online purchase-occurrence semantics and bounded comparable clock intervals place the whole purchase interval after the immutable server receipt. Offline, deferred, batched, overlapping or unknown timing stays visible for manager review, never becomes automatic one-click evidence, and does not disable an otherwise sufficiently corroborated transaction. Repair exact affected mappings internally. An unknown field is not permission to pick an arbitrary same-price purchase.
...
4. **Make customer contact effortless (#1109/#1163):** reuse the current request and message history. Ask only facts that could change the result, together in one secure same-case correction. Preserve answers, including “Not sure” and “I can't provide this.” The existing one-way text-to-link handoff remains outside the Hub and supplies the hosted form; it is not another cutover or access gate. After required-email submission, existing guarded automation handles routine supported email, including reply intake and truthful delivery state. `#889` is complete; managers or humans make the final business decision and handle named exceptions.
