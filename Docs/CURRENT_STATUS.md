...
- While owner UAT is deferred, agents added repeatable refund portal and Machine Manager QA harnesses (`npm run refunds:validate-portal-uat`, `npm run refunds:validate-machine-manager-uat`) plus the manager-facing script (`Docs/MACHINE_MANAGER_SHADOW_UAT_SCRIPT.md`) so shadow pilot feedback can be collected consistently.
- Owner-feedback correction on `2026-05-12`: Admin > Machines now treats Machine Manager assignment as autosaved machine ownership setup, removes the redundant `Save Machine Managers` action, shows assigned managers in the machine row after the edit sheet closes, and provides read-only local demo refund cases when `/portal/refunds` has no live cases for sponsor UAT.
