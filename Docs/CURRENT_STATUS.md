...
- Payout percentages live in Payout Rules as stable participant-named allocation rows plus Bloomjoy, with whole-percent inputs, presets, a 100% allocation check, and weekly-preview labels that match the configured payout recipients while still mapping to the existing backend split fields for compatibility.
