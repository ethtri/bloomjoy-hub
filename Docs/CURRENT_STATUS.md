...
- Operator login hardening (`#120`, `2026-03-19`): added dedicated `/login/operator` entry with operator-first copy for training, onboarding, support, and orders, updated the public header login CTA to point there, and routed logged-out portal access through the operator login experience while preserving the existing auth methods.
