...
- Machine-QR refund intake (`#662`) and the QR-aware source matcher (`#663`) are merged. The QR asset-management source in `#664` adds scoped create/download/rotate/disable controls and per-version print, placement, label, phone-scan, and replacement-owner checks in Admin > Machines. A code contains only an opaque public identifier; retiring it immediately prevents new valid claims. Physical installation and real-phone signoff for the live six-machine cohort remain part of the shadow pilot in `#665`.
- The current source is green across 118 disposable-project migrations and 266 database/persona tests, including 33 QR asset-management assertions, plus deterministic QR generation/decoding, matcher/payment safety, full Refund portal browser UAT, responsive Admin > Machines QR UAT, and release/rollback validation. Production deployment and physical pilot evidence remain separate from source verification. The guarded synthetic email smoke was not executed because an owner-controlled test inbox and privately approved machine were not supplied.
...
- The read-only production matcher shadow has proved only the negative path: the eligible open card case was older than the provider history returned, so the matcher correctly produced no safe match and performed no write, refund, or email action. Positive QR-aware matcher evidence still requires a recent controlled request or an explicitly approved nonpaying fixture.
- The six-machine QR pilot now has QR-aware read-only shadow tooling, nine controlled matcher scenarios, a bounded aggregate-only report, and an execution/rollback packet. Source fixtures currently show zero controlled false-positive recommendations. This is readiness evidence only: `#665` remains blocked until `#664` is deployed, the sponsor approves the exact cohort, and Operations completes all six physical installs, real-phone scans, controlled cases, manager review, rollback proof, and the final go/no-go.
