...
- Current branch QA evidence: refund Edge Functions pass Deno checks, refund-specific static validators pass, mocked/demo browser UAT harnesses pass for Portal > Refunds and Admin > Machines, and the public refund request/thank-you demo pass confirms card/cash-only payment selection, cash Zelle collection, card last-four collection, mobile overflow safety, and 5-business-day reassurance copy. The latest Admin > Machines harness also covers customer-refund setup controls for pilot intake, customer-facing label, and read-only Nayax card lookup. Seeded functional UAT or post-deploy shadow smoke is still required before executive sponsor testing.
- Current release evidence: GitHub CI, Vercel, the GitHub Supabase migration workflow, and local `npm run db:validate-migrations` are green for PR `#432`. Production shadow-mode setup was approved and executed on 2026-05-13: refund secrets are present, the approved migration train was applied, and refund Edge Functions were deployed.
- Current release blocker: production shadow-mode platform setup is complete, including the Admin > Machines refund-readiness setup migration applied on 2026-05-13. Sanitized smoke shows 26 active Sunze-backed Commercial/Mini reporting machines, 0 refund-intake-enabled machines, 0 Nayax lookup mappings, 0 active Machine Manager assignments, and 0 refund cases. Snapcase is a separate portfolio and is not covered by Sunze cash-correlation facts yet. A read-only pilot readiness audit also fetched 43 Nayax inventory machines and generated local mapping-candidate files under `output/refund-pilot-readiness/`; the follow-on cohort config helper generated a 26-row local template and dry-ran with no selected rows/no writes. Sponsor-provided manager mapping has been reconciled locally against the 26 active reporting machines: 13 Annie-managed rows, 12 Adam-managed rows, and 1 Steve-managed row. Annie and Steve still need authenticated site accounts, and 12 Adam-managed rows still need Nayax machine IDs before card-capable public intake. Selected pilot machines still need intentional Machine Manager and Nayax setup before functional shadow UAT can start.
...
- The next gate is production rollout readiness plus AI-orchestrated manager-wide shadow UAT, not another planning meeting. Use `Docs/REFUND_OPERATIONS_SHADOW_PILOT.md` as the go/no-go runbook; all current authenticated Machine Managers may participate after production setup is confirmed, but the Google Form/AppSheet fallback stays live until cutover criteria pass.
