...
- GPT triage remains default-off and human-review-only. The server runner, strict schema, content-free job ledger, and local developer credential destination are implemented; no production OpenAI secret is configured. `#635` still requires approval of the production Supabase secret destination and privacy controls before any sanitized real-model evaluation; the database structurally prevents automatic sending.
