...
- The approved refund target is one mapped Machine Manager decision, not a manual Nayax workflow (`#674`, `#689`). Bloomjoy should automatically attempt matching, use the secure customer wallet-detail correction loop in `#673` when useful, and show the manager one **Approve refund** or **Decline** decision only after one safe transaction is identified. Approval should execute the server-side Nayax flow and send one customer-facing completion operation with the active mapped-manager set CC'd only after confirmed success.
- Official manager actions also require fresh human presence. `#689` must fail closed without a recent TOTP authentication event, and the new P0 `#692` must add a per-action TOTP enrollment/challenge flow bound to the exact reviewed action. A manager JWT, old AAL2 session, emailed link, or agent-controlled/shared browser is not sufficient; official controls remain disabled until both issues and manager enrollment/UAT pass.
...
- Gmail remains disabled and unconfigured in the production Hub. The sponsor designated `info@bloomjoysweets.com` for `#634`; a local agent mailbox connection or signed-in browser session does not configure or enable that server integration. Designated-mailbox OAuth/authorization, label/filter ownership, secure server-only credentials, retention/quarantine approval, and synthetic smoke evidence remain required. Hosted-form intake and manual case handling remain independent.
