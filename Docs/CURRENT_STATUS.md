...
- Admin Access person-first redesign for issue `#227` is merged: `/admin/access` now defaults to person search, shows an effective-access summary, source cards for Plus Customer, Corporate Partner, Technician, Scoped Admin, Super Admin, and manual reporting access, plus secondary activity review. Issue `#331` is the active P1 follow-up for access review reminders, expiring access, broad admin-access review, inactive partner review, and richer renewal/revoke guidance. Backend access semantics stay unchanged; customer-managed Technician mutation flows remain in `/portal/account`.
