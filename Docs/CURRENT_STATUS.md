...
- Partner PDF polish for issue `#250` is in progress on branch `agent/partner-pdf-polish`: the partner-facing artifact is being redesigned as a senior-manager settlement packet with a dashboard first page, calculation support, machine appendix, friendly report reference, and no machine IDs/raw snapshot IDs in the PDF. Blocking review warnings now stop export server-side instead of appearing as partner-facing caveats.
