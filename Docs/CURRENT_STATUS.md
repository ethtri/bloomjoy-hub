...
- The integrated Refund Operations release candidate is green across 115 disposable-project migrations and 209 database tests, including 54 GPT runner safety assertions, plus deterministic matcher, payment-safety, mocked OpenAI failure-path, full Refund portal browser UAT, Machine Manager UAT, and release/rollback validation. A dedicated PR check publishes a 30-day sanitized evidence bundle with 18 hashed synthetic desktop/mobile screenshots and the exact source commit. Production-smoke tooling now includes a fail-closed intake/email runner: read-only preflight verifies the privately selected public machine and assigned-manager coverage, while the mutating mode requires exact project confirmation, a retry-safe run UUID, an exact authorization phrase, and a twice-confirmed owner-controlled inbox before creating one retained synthetic case; retrying the same run reuses the case instead of sending again, and evidence is restricted to reference, event type, recipient count, and delivery state. Manual automation dispatches now require a reusable synthetic UUID, making the documented once-only sweep and PII-free failure-alert replay executable across separate GitHub runs rather than incorrectly relying on a new `GITHUB_RUN_ID` or allowing a human-readable value into aggregate logs. A new aggregate-only linked-production audit confirms there are 25 active Machine Manager assignments across three identities but no clean manager-only identity: the existing identities overlap with Super Admin, Scoped Admin, or Corporate Partner access, and one also has a reporting entitlement. Owner-controlled clean-account setup, authorized deployment, production smoke, sanitized real-model evaluation, and the named shadow pilot remain pending.
- Read-only production alignment on 2026-07-22 found production migrations current through `202607200001`; exactly eight approved migrations (`202607210001` through `202607220001`) remain pending. The no-auth, no-body route smoke confirms the five deployed refund routes return their source-expected `OPTIONS` status while `refund-case-message-send`, `refund-gmail-sync`, and `refund-gpt-triage` return `404` because they are absent. An aggregate public-options smoke finds six non-duplicate options with one Atlanta/DC/Seattle result each, but three still contain the internal `Unmapped`/`Unknown` label that pending migration `202607210001` removes. All live optional switches remain off.
