...
- Partnership contract-alignment fields are merged on `main` in migration `202604260014_partnership_contract_terms.sql`; production rollout still needs migration-history cleanup before `supabase db push` can safely apply it.
- Migration hygiene follow-up renamed the two local duplicate `202604260013` migrations to `202604260015_partner_report_csv_exports.sql` and `202604260016_technician_invite_resolution.sql` so future dry-runs have unique local versions.
- `supabase db push --dry-run` still blocks on the remote/local `202604260005` migration-history mismatch; do not run `supabase migration repair --status reverted 202604260005` until the production backup/snapshot is confirmed and the repair plan is approved.
...
- Follow-up review found the partner dashboard period-preview RPC counted only the second payout-recipient share as `Amount owed`; migration `202604260014_partner_dashboard_amount_owed_repair.sql` now includes both configured non-Bloomjoy payout-recipient share fields before reporting amount owed.
...
- Reporting gap found for the partner Weekly Preview week ending `2026-04-19`: the previous scheduled sync only loaded recent `Last 3 Days` Sunze data, while Sunze preset exports proved the selected machine IDs have sales during `2026-04-13` through `2026-04-19`. Backfills should use supported Sunze presets such as `Last Month` or `Last 3 Months`; `Custom Range` is not approved because it can produce corrupted workbooks.
- Historical Sunze live backfills use smaller Supabase ingest chunks by default because the full `Last 3 Months` preset validated successfully but a 10,000-row live chunk hit a Postgres statement timeout.
