...
- Current release blocker: the original production shadow-mode platform setup is complete, but machine readiness data is not. Sanitized smoke shows 26 active reporting machines, 0 refund-intake-enabled machines, 0 Nayax lookup mappings, 0 active Machine Manager assignments, and 0 refund cases. This branch now adds one pending Admin > Machines setup migration so selected pilot machines can be intake-enabled, assigned Machine Managers, and mapped to Nayax IDs without direct database edits after the migration is applied.
