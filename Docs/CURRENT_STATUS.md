...
- The approved refund target is one manager decision, not a manual Nayax workflow (`#674`). Bloomjoy should automatically attempt matching, use the secure customer wallet-detail correction loop in `#673` when useful, and show the manager one **Approve refund** or **Decline** decision only after one safe transaction is identified. Approval should execute the server-side Nayax flow and send one customer plus one manager confirmation only after confirmed success.
...
- The soft-cutover SMS lane is not pilot-ready (`#704`). A read-only 2026-08-04 audit found that the signed-in GoDaddy Conversations workspace has no Phone & SMS number connected and that the known historical Twilio account was suspended after its balance reached zero. The public legacy Google Form remains reachable, but the actual provider, owner/backup, trigger, loop suppression, delivery/dead-link alerts, response Sheet ownership, independent kill switch, synthetic test, and rollback proof remain externally gated. `Docs/REFUND_SMS_RESPONDER_AUDIT.md` is the sanitized owner-supervised verification runbook; no live text or provider change was made.
