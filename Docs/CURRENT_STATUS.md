...
- Machine-QR refund intake (`#662`) and secure wallet-detail correction (`#673`) are merged. Matching policy `2026-07-27.v3` keeps customer-reported time separate from verified QR-open time and can recommend either a strong card match or exactly one machine/amount/QR/time match. Wallet use alone no longer forces a manual Nayax workflow, but no recommendation is delivery-failure proof or refund approval.
- The approved one-manager-decision flow (`#674`) is implemented behind the disabled Nayax execution gates. For an eligible recommendation, **Approve refund** records the business decision and lets the server revalidate the recommendation, claim one provider attempt, run Nayax request-and-approve, complete the case and reporting exactly once, and send one customer plus one approving-manager confirmation after confirmed success. The browser no longer performs a second completion save.
- A wallet no-match receives one 48-hour single-use correction link plus at most one replacement reminder. The form accepts only wallet type, virtual/device last four, approximate local purchase time, and amount confirmation. Submission preserves machine and QR evidence, automatically re-runs matching, and notifies managers only for a high-confidence transaction. Raw link tokens and forbidden payment credentials are not stored.
- Local validation is green across 120 disposable-project migrations and 289 database assertions, including the one-approval wallet/QR path, ambiguous-candidate rejection, atomic provider claim, idempotent completion/reporting, and duplicate-safe two-recipient confirmation outbox. Live production execution remains disabled and still requires the account-contract and controlled-pilot decisions in `#430`.
...
- Live Nayax execution remains disabled and sponsor-gated. A read-only inspection of Bloomjoy's production Core account confirmed major currency units, production reconciliation fields including `Site ID`, transaction-level refund-support flags, and the pending/refunded/failed workflow without exposing a token or submitting a refund. Core displayed production `LYNX` tokens but no endpoint-level scopes, and no Bloomjoy QA route was identified. Before the one-approval target can execute live, `#430` still needs a transaction-safe automated `SiteID` source when Last Sales omits it, a separately approved allowlisted/capped low-value pilot to prove token write authority and exact public-Lynx response pairs, and same-day reconciliation/go-no-go evidence.
