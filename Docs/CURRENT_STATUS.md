...
- Sales import reliability controls require parser checks in CI, export-task downloads to reconcile against provider UI count/revenue before ingest, raw downloads to be cleaned on every path, order-level idempotency, top-level machine discovery, admin-managed imported-machine setup, backup/recovery syncs for missed scheduled runs, and stale/failure alerts through the ingest health check plus scheduled health workflow.
