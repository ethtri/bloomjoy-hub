...
- Refund Operations epic `#628` has one canonical integrated release candidate in draft PR `#644`; superseded drafts `#636` through `#643` are closed and must not be merged separately. The candidate includes seven refund functions, 22 required migrations, deterministic Nayax recommendation, the simplified manager card/cash workbench, automation health, Gmail thread linkage, and the credential-independent human-reviewed GPT triage foundation.
- The integrated Refund Operations release candidate is green across repository CI, 114 disposable-project migrations, 186 database tests, deterministic matcher and payment-safety validation, full Refund portal browser UAT, Machine Manager UAT, and release/rollback alignment. Clean manager-only production UAT, authorized deployment, production smoke, and the named shadow pilot remain pending.
- Read-only production alignment on 2026-07-22 found production migrations current through `202607200001`; exactly seven approved migrations (`202607210001` through `202607210007`) remain pending. `refund-case-message-send` and `refund-gmail-sync` are absent in production. All live optional switches remain off.
...
- GPT triage remains default-off and human-review-only, with no provider runner or credential configured. `#635` still requires approval of a secure server-only credential destination and privacy controls before any sanitized real-model evaluation; the database structurally prevents automatic sending.
