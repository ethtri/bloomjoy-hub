...
- Current branch QA evidence: refund Edge Functions pass Deno checks, refund-specific static validators pass, mocked/demo browser UAT harnesses pass for Portal > Refunds and Admin > Machines on `http://127.0.0.1:8092`, and the public refund request/thank-you demo pass confirms card/cash-only payment selection, cash Zelle collection, card last-four collection, mobile overflow safety, and 5-business-day reassurance copy. Seeded functional UAT or post-deploy shadow smoke is still required before executive sponsor testing.
- Current release evidence: GitHub CI, Vercel, and the GitHub Supabase migration workflow are green for PR `#432`, including migration apply validation on the PR head SHA.
- Current release blockers: refreshed production commerce preflight on 2026-05-13 shows missing remote refund secrets by name, and production migration/function/secret rollout still requires explicit sponsor go/no-go. The exact missing secret list and rollout order live in `Docs/REFUND_FULL_AUTOMATION_GO_NO_GO.md`. Local migration apply validation remains blocked on this workstation by Docker Desktop returning a 500 before SQL is applied, but the GitHub Supabase migration workflow provides clean CI migration evidence for the branch.
