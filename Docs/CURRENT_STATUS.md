...
- Production has six non-duplicate public options, no internal `Unmapped`/`Unknown` labels, and at least one Atlanta, DC, and Seattle option. The aggregate access audit finds 25 active Machine Manager assignments across three broader-access identities, plus three clean unassigned identities; two clean candidates have signed in. The access owner must privately select one candidate and assign only the approved pilot machines before manager-only production UAT.
- Live Nayax execution remains disabled and sponsor-gated. Nayax's public documentation confirms a two-step refund request/approval flow. A real read-only production matcher shadow returned 98 parseable Last Sales rows with `SiteID` and machine authorization time present on every row, but no explicit payment-status or refund-state field and no candidate because the only open card case is about 68 days old while the provider history covers about 28 days. This proves fail-closed behavior, not a positive match. Before one-click execution can leave manual review, `#430` must still confirm production endpoint/token write authority, amount and exact response semantics, idempotency/reconciliation behavior, customer-email behavior, and an account-approved refundable-sale rule, then record caps, allowlist, and controlled low-value smoke approval. Positive matcher evidence needs a new eligible request or an explicitly approved isolated nonpaying fixture.
