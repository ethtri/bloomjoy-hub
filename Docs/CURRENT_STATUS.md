...
- Refund Operations issue `#634` now has a default-off Gmail intake slice in implementation: one designated mailbox and explicit label, minimal read/send permissions, idempotent draft-case and thread linkage, central-admin-only access while location is unknown, chronological replies in the original Gmail thread, incoming card-number redaction, private attachment quarantine, provider-independent retention cleanup, aggregate health, and a single reply action. PR review, Google authorization/security review, retention-owner approval, manager UAT, and production shadow proof remain pending; revoking or disabling Gmail leaves hosted-form refund intake and case handling available.
- Refund Operations issue `#635` now has a credential-independent GPT triage foundation in implementation: strict classification/extraction schema, deterministic missing-field and safety checks, minimized/redacted model input, a 30-day derived-content ledger, manager edit/approve/reject review, and a sanitized evaluation suite. No model-provider runner or credential is configured, processing defaults off, the database prevents automatic sending, and live evaluation still requires approval of a secure server-only credential destination and privacy controls.
