...
- Dual-intake privacy approval remains a P0 pilot gate (`#705`). The recommended packet in `Docs/REFUND_DUAL_INTAKE_DATA_POLICY.md` keeps production Gmail/Sheet copying and attachment processing off while owners decide 30/90-day Google transport, 180-day Gmail copy, 400-day non-content ledger, proposed seven-year canonical-case retention, attachment disable/scanner behavior, least-privilege access, offboarding, deletion/export, and incident ownership. The audit added `#710` for Form-plus-Sheet source deletion and `#711` for Gmail/hosted-form attachment hardening; neither production cleanup nor attachment behavior was changed.
