...
- Scoped Admin Corporate Partner permissions follow-up now lets scoped admins manage Corporate Partner membership and portal-access flags from `/admin/access` only when the partner/party active machine scope is fully inside the scoped admin grant. Out-of-scope partner records remain hidden and blocked by RPC checks.
