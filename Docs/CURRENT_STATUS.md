...
- Machine-QR refund intake (`#662`) is merged, and the source matcher now uses policy `2026-07-26.v2` (`#663`) to keep customer-reported time separate from verified QR-open time. It can recommend either a strong card match or exactly one machine/amount/QR/time match; wallet and QR/time recommendations stay manual-only, and no recommendation is delivery-failure proof or refund approval. Production use still requires deployment plus the shadow pilot in `#665`.
