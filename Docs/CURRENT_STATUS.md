...
- Admin Access person-first redesign for issue `#227` is merged: `/admin/access` now defaults to person search, shows an effective-access summary, source cards for Plus Customer, Corporate Partner, Technician, Scoped Admin, Super Admin, and manual reporting access, plus secondary activity review.
- P0 access recovery issue `#368`/#369 adds Super Admin Technician controls to `/admin/access`: admin-specific RPC wrappers enforce training-only or one-machine scope, support grant/update/renew/revoke with required reasons, and preserve source-aware reporting cleanup so manual reporting grants are not removed by Technician changes.
- Issue `#367` remains a parked review-reminder follow-up layered on the merged access-management mainline, not the source of truth for recovery acceptance. Issue `#331` is the active P1 follow-up for access review reminders, expiring access, broad admin-access review, inactive partner review, and richer renewal/revoke guidance.
