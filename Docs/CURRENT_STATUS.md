...
- Reporting gap found for the partner Weekly Preview week ending `2026-04-19`: the current scheduled sync only loaded recent `Last 3 Days` Sunze data, while a manual `Last 7 Days` workbook proved the selected machine IDs have Sunze sales on `2026-04-19`. Backfills should use supported Sunze presets such as `Last Month` or `Last 3 Months`; `Custom Range` is not approved because it can produce corrupted workbooks.
