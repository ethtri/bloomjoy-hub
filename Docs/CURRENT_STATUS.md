...
- Access/UX direction for PR `#410`: refund-only managers use Portal > Refunds and do not see a separate top-level Refunds workspace, the Admin workspace/nav, or setup controls. Super admins manage machine refund manager assignments from Admin > Machines > Edit Machine so case processing and entitlement setup stay separate.
