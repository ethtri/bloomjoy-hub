...
- Current release blocker: production platform setup is complete, but machine readiness data is not. Sanitized smoke shows 26 active reporting machines, 0 refund-intake-enabled machines, 0 Nayax lookup mappings, 0 active Machine Manager assignments, and 0 refund cases. Manager-wide shadow UAT is blocked until selected machines are enabled, Machine Managers are assigned, and Nayax IDs are mapped for card lookup.
