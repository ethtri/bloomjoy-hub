...
- The MVP implementation branch starts the hosted refund system in parallel with the Google Form/AppSheet process: noindex `/refunds/request`, primary authenticated `/portal/refunds`, compatibility `/admin/refunds`, manager assignments, cash correlation against sales facts, customer confirmation/more-info emails, and reporting write-through for approved/completed correlated cases.
- Access/UX direction for PR `#410`: refund-only managers use the operator-facing `/portal/refunds` route and do not see the Admin workspace/nav or setup controls. Super/scoped admins can still manage machine refund manager setup separately from case processing.
- While owner UAT is deferred, agents added a repeatable refund portal QA harness (`npm run refunds:validate-portal-uat`) and manager-facing script (`Docs/REFUND_MANAGER_SHADOW_UAT_SCRIPT.md`) so shadow pilot feedback can be collected consistently.
...
- The next gate is AI-orchestrated manager-wide shadow UAT, not another planning meeting. Use `Docs/REFUND_OPERATIONS_SHADOW_PILOT.md` as the go/no-go runbook; all current authenticated refund managers may participate, but the Google Form/AppSheet fallback stays live until cutover criteria pass.
