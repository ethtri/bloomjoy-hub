...
- Sunze hardening PR `#161` now uses an admin mapping queue: mapped machine rows keep importing, unmapped Sunze rows are quarantined without raw order numbers, and `/admin/reporting` lets admins map, ignore, or reopen discovered Sunze machine IDs. The next launch proof is a fresh dry-run and non-dry run after this queue behavior is deployed from the PR branch.
