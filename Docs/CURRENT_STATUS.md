...
- Admin Access UX is now the next access-management priority: `/admin/access` is functionally capable, but issue `#227` is promoted to the redesign from overlapping tabs into a person-first workspace with access source cards, impact previews, and clearer grant/revoke flows.
