...
- Payment-first storefront policy is tracked in P0 `#715`: the Commercial Machine is the only quote/request product; available sugar, Bloomjoy branded sticks, Micro Machine, and Plus checkout before Ethan/Ian and WeCom alerts. Mini and custom sticks remain visibly unavailable until their complete payment-first pricing and fulfillment rules are ready. The implementation is review-ready but production rollout remains gated on Stripe Micro price/shipping configuration, Stripe Tax registration/tax-code review, Edge Function deployment, migration application, and test-mode checkout/webhook evidence.
