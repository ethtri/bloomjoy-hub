...
- Payout percentages live in Payout Rules as stable participant-named allocation rows plus Bloomjoy, with whole-percent inputs, a 100% allocation check, and weekly-preview labels that match the configured payout recipients while still mapping to the existing backend split fields for compatibility.
- Weekly Preview now surfaces actionable readiness issues for the selected week, including missing active machine assignment coverage and missing active payout rules, and keeps RPC errors visible in-page instead of relying only on toast feedback.
