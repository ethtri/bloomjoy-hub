...
- Benchmark-driven UX/CX pass now makes the manager queue more operational: top metrics prioritize needs-action, ready-to-pay, waiting-customer, and blocked/failed work; queue rows show amount, age, customer-contact state, match status, and blocker badges; the case header is a read-only status strip; and decision alternatives stay secondary to one guided primary action.
- Guarded card execution remains fail-closed until provider execution gates are approved. Blocked execution leaves the case open and does not send the customer completion email; mocked successful execution completes only after `nayax-card-refund` returns executed. Pilot card auto-refund amount must equal the matched Nayax sale, with partial card refunds staying manual review until policy/provider capability is approved.
- Current evidence: GitHub PR `#441` latest pushed head has CI `verify`, Vercel, and Supabase migration `validate` green. Local evidence includes `npm run build`, `npm test --if-present`, `npm run lint --if-present` (existing Fast Refresh warnings only), `npm run seo:check`, `npm run db:validate-rpc-surface`, `npm run security:validate-public-intake`, `npm run reporting:validate-refund-adjustments`, `npm run refunds:validate-nayax-execution`, `npm run refunds:validate-customer-comms`, and `npm run refunds:validate-portal-uat -- --app-url http://127.0.0.1:8083` passing. Local Docker-backed `npm run db:validate-migrations` is blocked until Docker Desktop is running, but GitHub Supabase migration validation is green.
...
## Refund manager rescue sprint (2026-05-15)
- Active branch/worktree: `agent/refund-manager-rescue` in `C:\Repos\wt-refund-manager-rescue`, using `bloomjoy-sprint-orchestrator` with Codex as PO/PM and independent UX/data/test QA challenge lanes.
- This rescue focuses on the manager pilot blockers from UAT: primary case actions now update the case and send/log the matching customer email through `refund-case-admin-update`; failed customer emails become visible retry work; normal processing no longer depends on a separate send-email step.
- `/portal/refunds` is being reshaped into a guided workbench: one recommended action, transaction result first, card-sale candidates inside the transaction step, clear `Candidate found` versus `Match selected` language, contextual Nayax/Zelle completion references, customer-email preview, and collapsed advanced/history sections.
- Nayax lookup clarity now includes persisted manual no-candidate/setup/failure events, overview summaries for both manual and automated lookup events, sanitized provider display fields, and guarded JSON metadata casts. Live Nayax refund execution remains disabled.
- Current local evidence on this branch: `npm ci`, `npm run build`, `npm test --if-present`, `npm run lint --if-present` (existing Fast Refresh warnings only), `npm run seo:check`, `npm run db:validate-rpc-surface`, `npm run security:validate-public-intake`, `npm run reporting:validate-refund-adjustments`, `npm run refunds:validate-nayax-execution`, `npm run refunds:validate-customer-comms`, and `npm run refunds:validate-portal-uat -- --app-url http://127.0.0.1:8083` pass. Local Docker-backed `npm run db:validate-migrations` is blocked because Docker Desktop is not running.
...
- Current branch QA evidence: independent product/UX, data/privacy, pilot-ops, browser UAT, and challenge agents were run on 2026-05-13. Browser/demo UAT passed for Portal > Refunds, Admin > Machines, public refund intake, thank-you, mobile overflow, and demo/empty states. The QA challenge produced no-go findings around readiness gates, Snapcase/Commercial-Mini scope, and caller-supplied Nayax lookup fields; the branch now includes a forward hardening migration, Edge Function guards, UI readiness blocks, and validators for those findings. Seeded functional UAT or post-deploy shadow smoke is still required before executive sponsor testing.
- Current release evidence: PR `#432` is code-merge ready after the 2026-05-13 QA hardening patch: GitHub CI, GitHub Supabase migration validation, Vercel, `npm ci`, `npm run build`, `npm test --if-present`, `npm run lint --if-present`, `npm run seo:check`, `npm run db:validate-rpc-surface`, `npm run security:validate-public-intake`, `npm run reporting:validate-refund-adjustments`, `npm run refunds:validate-nayax-execution`, browser UAT harnesses, and read-only pilot readiness dry-runs pass. Local Docker-backed `npm run db:validate-migrations` is blocked until Docker Desktop is running, but GitHub migration validation is green and `supabase db push --dry-run --linked` sees only the new hardening migration pending.
- Current rollout blocker: latest sanitized production readiness smoke on 2026-05-13 shows 26 active Sunze-backed Commercial/Mini reporting machines, 15 active Machine Manager assignments, 0 refund-intake-enabled machines, 0 public refund selector options, 0 Nayax lookup mappings, and 0 refund cases. Snapcase is a separate portfolio and is not covered by Sunze cash-correlation facts yet. The read-only Nayax inventory audit fetched 43 machines and generated local mapping-candidate files under `output/refund-pilot-readiness/`; the dashboard export confirms the same operational mapping source is available through Nayax machine names, numeric Nayax machine IDs, and operator machine numbers. Clear name/location matches are acceptable for pilot mapping; only ambiguous or missing rows need sponsor resolution. Annie still needs an authenticated site account before her assignment wave, and selected pilot machines still need reviewed Nayax IDs before card-capable functional shadow UAT can start.
...
- The merged MVP includes noindex `/refunds/request`, `/refunds/thank-you`, authenticated Portal > Refunds workflow at `/portal/refunds`, compatibility redirect from `/admin/refunds`, machine-level Machine Manager assignments in Admin > Machines, cash correlation against sales facts, customer confirmation/more-info emails, read-only Nayax card lookup, and reporting write-through for approved/completed correlated cases.
- Access/UX direction is now the accepted product baseline: Machine Managers use Portal > Refunds and do not see a separate top-level Refunds workspace, Admin workspace/nav, or setup controls. Super admins manage Machine Managers from Admin > Machines > Edit Machine so case processing and machine ownership setup stay separate.
- Agent-first QA is the gate before executive proof review. The latest merge evidence passed local refund portal and Machine Manager harnesses (`npm run refunds:validate-portal-uat`, `npm run refunds:validate-machine-manager-uat`) plus UAT-branch proofs for Nayax Edge lookup, assigned-machine access boundaries, max-3 Machine Manager enforcement, and sanitized reporting write-through.
- Nayax card lookup is activated for the official read-only Last Sales path behind server-only Edge Function configuration (`NAYAX_LYNX_BASE_URL=https://lynx.nayax.com/operational/v1`, account-specific `NAYAX_LYNX_API_TOKEN_TGPACI_USA_DB`, or fallback `NAYAX_LYNX_API_TOKEN`) and per-machine Nayax IDs. Card refund execution remains out of MVP; managers still complete card refunds manually in Nayax.
- The next gate is production rollout readiness plus AI-orchestrated selected Commercial/Mini shadow UAT, not another planning meeting. Use `Docs/REFUND_OPERATIONS_SHADOW_PILOT.md` as the go/no-go runbook; broader manager-wide rollout follows only after selected pilot evidence is clean, and the Google Form/AppSheet fallback stays live until cutover criteria pass.
- Refund/reporting PR `#399` has merged into `main`; reporting validation should continue to be re-run after any further reporting-foundation changes.
...
- Scoped Admin Corporate Partner permissions follow-up now lets scoped admins manage Corporate Partner membership, portal-access flags, and machine reporting tax rates from `/admin/access` only when the partner/party/machine scope is inside the scoped admin grant. Out-of-scope partner records, parties, and machine tax changes remain hidden or blocked by RPC checks.
...
- Corporate Partner production UAT on `2026-04-29` passed with isolated temporary users/data and cleanup verified: non-Plus baseline stayed baseline, Plus/Corporate Partner support and supply resolvers worked server-side, partner reporting respected portal-enabled active partnerships, Technician grants/expiry/reporting scope worked, revoked Corporate Partner access removed capabilities, and effective-access preview now shows all reporting machines plus Corporate Partner/Technician/Scoped Admin source breakdowns.
- Corporate Partner access implementation issues `#128`, `#328`, `#329`, and `#330` are closed as completed by `#332`.
- Admin Access UX is now being redesigned for issue `#227`: `/admin/access` moves from overlapping grant-type tabs into a person-first workspace with default person search, effective access summary, source cards for Plus Customer, Corporate Partner, Technician, Scoped Admin, Super Admin, and manual reporting access, plus secondary activity review. Backend access semantics stay unchanged; customer-managed Technician mutation flows remain in `/portal/account`.
- P0 access recovery issue `#368` adds Super Admin Technician controls to `/admin/access`: admin-specific RPC wrappers enforce training-only or one-machine scope, support grant/update/renew/revoke with required reasons, and preserve source-aware reporting cleanup so manual reporting grants are not removed by Technician changes.
- P0 access recovery issue `#368` is also the current acceptance source for the access-management rescue. `Docs/ACCESS_MANAGEMENT_RECOVERY_UAT.md` defines the Super Admin, Plus Customer, Corporate Partner, Technician, Scoped Admin, Reporting User, and non-admin UAT matrix, key URLs, screenshot expectations, and PR verification requirements; judge implementation branches by end-user/admin workflows, not backend completion alone.
- Access Launcher follow-up adds `/admin/access?action=add-access` as the broad "Add or invite access" entry point. Corporate Partner and Technician are the only email-first invite presets in this pass; Scoped Admin, Super Admin, and Plus Customer remain existing-user flows that open the person workspace with clearer guidance.
- Issue `#367` is secondary until the `#368` core rescue lands. Treat it as a review-reminder follow-up, not the source of truth for recovery acceptance.
...
- Technician resolver production check for issue `#214` on `2026-04-27` confirmed the live REST endpoint for `resolve_my_technician_entitlements` returns `400/P0001 Authentication required` with the client anon key instead of `404` or `PGRST202`, so PostgREST currently sees the RPC. Migration `202604270006_restore_technician_entitlement_resolution_rpc.sql` is the forward-only repair path for environments that missed the resolver during the temporary `202604260013` migration-version collision/renumbering; rollout should use validated Supabase migration push and schema-cache reload, not one-off SQL.
- RPC execute-grant hardening for issue `#344` adds a forward-only migration that removes anon execution from existing `SECURITY DEFINER` functions, keeps browser-facing self-service/admin wrappers on `authenticated`, preserves `service_role` helper access for Edge Functions, and repairs the currently flagged helper `search_path` settings. PR `#332` still owns the pending Corporate Partner migration slice, so migration-order validation must be rerun if that PR lands first.
- Partner PDF polish for issue `#250` was shipped on `2026-04-27` via PR `#260`: `partner-report-export` now produces a senior-manager revenue-share report with a Bloomjoy-branded dashboard, net-sales trend context, calculation support, machine detail totals, friendly report reference, and no machine IDs/raw snapshot IDs in the PDF. Blocking review warnings stop export server-side instead of appearing as partner-facing caveats.
...
- The Bubble Planet workbook baseline uses sales-source order amount as gross sales, subtracts machine tax plus a configured `$0.40` per-stick/item fee before the 60/40 split, counts no-pay orders as `$0`, and reports completed Monday-Sunday weeks.
...
- Follow-up review found the partner dashboard period-preview RPC counted only the second payout-recipient share as `Partner Revenue Share`; migration `202604260017_partner_dashboard_amount_owed_repair.sql` now includes both configured non-Bloomjoy payout-recipient share fields before reporting Partner Revenue Share.
...
- Partner dashboard timeframe selection for issue `#251` now exposes explicit completed-week and completed-month controls in `/portal/reports`; the selected period drives the KPI summary, trend comparison, machine rollups, warning notes, and PDF/CSV export request period.
- Adam UAT follow-up adds a Partner Dashboard `Month to date` mode alongside Weekly and Completed month, a shared machine selector that scopes Partner performance summary and Net sales trend while preserving all-machine rollups, and partner-facing `Partner Revenue Share` terminology across dashboard, print, PDF, CSV, and XLSX output. Downloadable artifacts now use revenue-share/reporting language instead of settlement/payout-rule wording for partner-facing explanations. Official partner export snapshots now include a machine scope key so all-machine and single-machine exports for the same period remain separate. Production follow-up on `2026-04-29` deployed `partner-report-export` version `23` to Supabase project `ygbzkgxktzqsiygjlqyg` after Adam's PDF review showed downloads were still using the pre-merge Edge Function template; a later export failure confirmed production was still missing migration `202604290002_partner_report_snapshot_machine_scope.sql`, which was then applied and `supabase db push --dry-run` reported the remote database up to date. Post-production validation passed all checks on `2026-04-29` across Weekly, Month to date, Completed month, all-machine and single-machine scopes, and PDF/XLSX/CSV artifacts.
- Adam reporting feedback remediation on `2026-05-01` keeps all-machine partner rollups unchanged but changes single-machine Partner Dashboard views to show machine history across the existing trend periods. Partner PDF graph labels now use full whole-dollar values instead of compact `$1.5k`-style labels, XLSX worksheets include management-level taglines, and machine-scoped PDF/CSV/XLSX exports include historical context when trend periods are available.
...
- Sales-provider browser automation is implemented as a scheduled GitHub Actions Playwright worker. Sunze now confirms exports asynchronously, so the worker requests the Orders export, downloads the newest completed file from `#/taskExportList`, parses `.xlsx` or `.zip` exports, deletes raw files after parsing, and sends normalized rows to the locked sales-ingest Edge Function.
- Sunze monthly sync reconciliation hardening addressed the `2026-05-01` monthly sweep failure where a generic UI scrape read an untrusted record count while export revenue matched the workbook. The worker treats only trusted Orders pagination counts and explicitly trusted revenue sources as UI hard blockers; weak UI totals are logged while selected date window, pinned export-task selection, workbook parsing, workbook row-date validation, and Supabase ingest validation remain hard blockers. PR-branch `Last Month` dry-run and live replay passed on `2026-05-01`: 6,733 rows parsed, 6,360 mapped rows imported, 373 rows quarantined for 5 pending unmapped machines, and the health check reported `latestCompletedAt=2026-05-01T16:41:48.197+00:00` with `stale=false`. PR `#370` is merged, so the scheduled monthly sweep now uses the hardened path.
- Sunze stale-data prevention on `2026-05-05` traces the alert to a scheduled `Sales Import Sync` cancellation before runner allocation, not a parser/Sunze/ingest failure. Manual live replay restored freshness to `latestCompletedAt=2026-05-05T22:50:49.068+00:00`; this prevention slice adds a backup daily `Last 7 Days` scheduled replay and a `Sales Import Recovery` workflow that automatically replays failed, cancelled, or timed-out scheduled syncs before the standalone health sentinel pages.
- Sunze reliability hardening on `2026-05-12` addresses a backup run where the Orders export was pinned and downloaded, but weak UI revenue text appeared stale/partial and Machine Center exposed no top-level machine IDs. The worker now treats Machine Center discovery as advisory, retries transient export/UI reconciliation failures, allows weak revenue text to be bypassed when row/date evidence matches, uploads sanitized failure diagnostics, and suppresses scheduled-sync failure email until recovery or freshness monitoring confirms an actual unresolved incident.
- Sunze post-merge follow-up on `2026-05-13` addresses a primary scheduled sync where the provider Export Task List never exposed the newly requested task before the 5-minute wait expired. Automatic recovery succeeded minutes later (`latestCompletedAt=2026-05-13T16:13:04.872+00:00`), proving no unresolved data outage. Export-task wait and download-start timeouts are now treated as same-worker retryable provider-export failures, with sanitized diagnostics recording poll count, request time, and whether a task was pinned before the retry.
- Adam's data-coverage feedback is being handled as a full all-machine source-data baseline from `2025-08-01`, not a Bubble Planet-only display issue. The local all-machine workbook `Order Record-1776995822749.xlsx` covers `2026-01-01` through `2026-04-19`; the new parse-file `--filter-date-window` mode lets that multi-month file be dry-run/imported in monthly chunks while keeping default out-of-window rejection intact. August-December 2025 still requires production coverage verification or the missing all-machine source workbook before full baseline completion can be claimed.
- Reporting gap found for the partner Weekly Preview week ending `2026-04-19`: the previous scheduled sync only loaded recent `Last 3 Days` provider data, while provider exports proved the selected machine IDs have sales during `2026-04-13` through `2026-04-19`. Daily imports stay on the rolling `Last 7 Days` preset; six-month backfills should run as explicit monthly custom ranges after dry-run validation, or from manually supplied monthly Export Task files with `--parse-file` if the provider selector is unavailable.
- Historical provider live backfills use smaller Supabase ingest chunks by default because large live chunks can hit Postgres statement timeouts; monthly custom-range chunks are the preferred backfill unit.
- Sales import reliability controls require parser checks in CI, export-task downloads to reconcile against provider UI count/revenue before ingest, raw downloads to be cleaned on every path, order-level idempotency, advisory top-level machine discovery, admin-managed imported-machine setup, backup/recovery syncs for missed scheduled runs, and stale/failure alerts through the ingest health check plus scheduled health workflow.
- Sales import hardening PR `#161` now uses an admin setup queue: configured machine rows keep importing, unconfigured provider rows are quarantined without raw order numbers, and `/admin/reporting` lets admins set up, ignore, or reopen discovered external machine IDs. Fresh dry-run and live runs passed on `2026-04-25`; the order-hash idempotency repair protects environments affected by the brief `202604260002` migration timestamp collision. The next launch proof is the first scheduled daily run after merge.
- Imported machine setup cleanup on `2026-04-27` replaces the account-first setup path with a report/partnership-first flow from `/admin/reporting`: the external machine ID stays read-only, admins choose the report, confirm label/location/type/tax, and one save creates the reporting machine, assigns the partnership, applies tax, and promotes queued rows. The same migration normalizes the Merlin machines out of `Sunze Import Holding`, creates Merlin Minneapolis from external ID `17525706476037914545569`, assigns Dallas/Chicago/Minneapolis to `Merlin Revenue Share`, and applies the existing provisional `0%` Merlin tax setup.
- The internal sales-source discovery note records the validated provider routes, export headers, payment/status mappings, and remaining open questions without storing credentials or raw order data.
- Provider-name cleanup for issue `#224` now uses neutral wording in admin/portal reporting copy, partner PDF/CSV text sanitization, smoke/UAT checklist language, and sample fixture headers. Remaining provider-name references are internal implementation identifiers for existing integration scripts, database columns/functions, environment variables, migration history, and internal runbooks.
- Refund adjustment imports now use a safe review layer: rows are staged with source-row audit fields, deterministic machine-alias matching, match confidence/status, and applied adjustment links before they can affect partner revenue-share reporting. Approved uniquely matched refunds reduce partner net sales, split base, and Partner Revenue Share while gross sales remains the imported sales basis.
- Refund source ingestion now has a service-account API path for production: `refund-adjustment-sync` can read the private sheet with a read-only Google service account, stage rows as review records, and auto-apply only uniquely matched `Status=Closed` plus approve-style `Decision` rows. The current customer service sheet contract is `Location of Purchase`, `Decision Date`, `Date and Time of Incident`, `Refund Amount`, `Request Amount`, `Status`, `Decision`, and `Request ID`; when `Refund Amount` is blank, `Request Amount` is used only for closed approved rows. The source refund list is broader than Bloomjoy Hub's cotton-candy reporting scope and can include out-of-scope phone-case machines, so not every unmatched row should become a machine alias. Raw customer/payment/free-text source rows are not stored in reporting payloads.
- Refund ingestion QA tightened the production guardrails: manual workflow dispatch defaults to dry-run, scheduled sync remains disabled until `REFUND_ADJUSTMENT_SYNC_ENABLED=true`, transport failures now fail the workflow, duplicate detection keys off source row references without using free-text incident content, auto-matched review rows stay unresolved until the adjustment fact is written, and live source sync is paged so full-sheet imports stay below Edge Function timeout limits. Non-blocking refund review alerts stay in Admin Reporting Operations instead of the portal reporting tab; the reporting tab only surfaces blocking setup issues that lock export. Backend refund review warnings are now partner-scoped, so broad unmatched or out-of-scope source rows remain visible to admins without triggering unrelated partner report warnings. Removed-source-row reconciliation remains tracked in issue `#256`.
- Refund Operations MVP privacy policy for issue `#436` is now decided: authorized Machine Managers, scoped admins, and super admins may view customer-provided card last four and sanitized Nayax candidate evidence for cases they can manage, while partner reporting, exports, logs, screenshots, issue notes, and PR notes must exclude real card digits, payment IDs, raw Nayax payloads, customer PII, and raw complaint text. Live Nayax refund execution remains disabled unless separately approved through the refund execution safety gates.
...
- Contract-alignment slice `#217` adds legal partner names, weekly/monthly cadence terms, invoice/payment due terms, ownership/pricing authority fields, contract references, labeled deduction notes, and snapshot-backed export traceability.
...
- Training hub findability follow-up delivered on branch `codex/training-hub-ux`:
  ...
  - Final issue `#125` polish on `2026-04-28`: the training hub primary action now distinguishes true resume state from a fresh start path, Start Here is a compact sequence instead of another large card wall, job aids are grouped by flow moment near library search, and certificate UI remains secondary below the main library.
  - Issue `#125` QA follow-up on `2026-04-28`: Supabase-shaped browser smoke with UUID training row ids confirmed the Safe Power Off job-aid moment resolves through the canonical shutdown task route instead of relying on local slug ids.
- Verification run on this branch:
  - `npm ci`
  - `npm run build`
  - `npm test --if-present` (no test script present)
