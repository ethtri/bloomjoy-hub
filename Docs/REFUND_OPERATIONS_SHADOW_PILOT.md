...
> **Historical record only.** Later decisions remove this shadow pilot, dollar cap, special personas, staffing, cohort, and sponsor ceremony from the current refund path. Do not use unchecked items below to block `#628`, `#990`, or `#427`. Current operations use `Docs/PRODUCTION_RUNBOOK.md` and `Docs/NAYAX_REFUND_WORKING_CONTRACT.md`; the fixed `#427` window is not restarted.
...
- Use an owner-approved current Machine Manager mapped to every selected pilot machine for official-action UAT. Separate scoped-admin or super-admin access neither grants nor revokes that exact mapping. Also use the clean Machine Manager-only persona from `#435` for the distinct assigned-scope visibility boundary; broader access is not valid evidence for that narrower visibility test.
- At this historical shadow-pilot checkpoint, live Nayax execution was fail-closed and the candidate's production adapter was statically disabled. That is not current availability or permission guidance; current authority is in `Docs/PRODUCTION_RUNBOOK.md` and the newest entries in `Docs/DECISIONS.md`.
...
| Access | Current mapped operator for action authority, plus clean manager-only and super-admin comparison personas | Exact mapping—not Admin access—authorizes the action; clean manager sees assigned scope only; super-admin sees authorized global scope; Admin controls remain hidden from the clean manager |
