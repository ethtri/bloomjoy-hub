...
- [ ] A normal matched card case shows the customer request and selected Nayax transaction side by side at desktop width, exposes exactly one visible primary action (`Refund $X and notify customer`), and does not render manual status/decision selectors.
...
- [ ] Choosing `Clear selected card sale` immediately removes the prior refund action and makes `Save and recheck card sale` the only next action; an unsaved candidate cannot expose final refund execution before the server records manager confirmation and returns execution eligibility.
- [ ] Choosing a possible Nayax transaction exposes only `Save possible transaction`; its confirmation says no refund, approval, or customer email will occur. Saving keeps the case in review with no decision, no approval/completion message, and no Nayax refund call. The page continues to say `No refund has been issued.` until verified provider success.
...
- [ ] Guarded card success shows a confirmation receipt plus customer- and manager-email states; blocked execution leaves the case open with `Refund not sent`; an unconfirmed provider outcome warns the manager not to retry until Nayax is reconciled. Rejected, pending, timed-out, and unknown outcomes show neither a `Ready to refund` badge nor an active refund action. Pending, timed-out, and unknown outcomes move immediately to `Processing / check needed` and leave `Needs action` until reconciliation is complete.
...
- [ ] Card lookup runs only after the manager explicitly chooses **Check Nayax transaction** or through the separately gated automation sweep, using a +/- 6 hour window; opening a link or selecting a case causes no lookup. Only a uniquely high-confidence transaction is labeled recommended, while alternates stay behind progressive disclosure and are tokenized long enough for manager review.
...
- [ ] Run the owner operation with its exact confirmation phrase. Confirm current state becomes **Payment history check**, stale current match/recommendation fields and every cached lookup candidate are removed, only presence/aggregate-count evidence enters immutable history, the page says **No provider refund is recorded**, and both historical sent messages remain byte-for-byte and metadata-equivalent with no inserted message.
- [ ] Confirm refresh preserves **Historical payment review required** and shows only the read-only **Check Nayax transaction** next step. Inject an adversarial stale candidate response and confirm it cannot display, be selected, or hide the fresh-check button; refund/retry, approval, denial, and customer-email controls remain unavailable.
