...
- [ ] Run `npm run refunds:validate-route-smoke`, then post-deploy run `npm run refunds:smoke-routes -- --project-ref <project-ref> --confirm-project-ref <project-ref>`; all ten no-auth, no-body `OPTIONS` probes return their exact safe status, create no case/email/provider action, and manual/retry messaging does not return `404`.
...
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains the 26 synthetic, hashed desktop/mobile portal, machine-manager, QR-intake, reconciliation, source-aware queue, and unique QR/time recommendation screenshots plus `refund-uat-evidence.json`; the manifest source commit matches the reviewed PR head and says `containsProductionData: false`.
...
- [ ] Refund soft-cutover entry points match `Docs/REFUND_GOOGLE_FORM_BRIDGE.md`: email directs customers to the hosted Hub form, SMS continues to the Google Form, and both create cases in the one Hub Refund Operations queue rather than separate manager inboxes or sheets.
- [ ] Run `npm run refunds:validate-google-form-bridge` and `npm run db:validate-migrations`; the locked 11-header contract, read-only Sheets scope, server-only secrets, default-off switches, explicit pilot boundary, PII-free ledgers, and no-send/no-official-action rules pass.
- [ ] Run `npm run refunds:validate-cross-channel-reconciliation` and `npm run db:validate-migrations`; website, Gmail, and SMS Google Form same-source replays remain idempotent, exact/possible cross-source candidates expose only fixed comparison reasons, and pending/confirmed duplicates cannot approve, execute, complete, or write settlement.
- [ ] Run `npm run refunds:validate-source-aware-queue`, `npm run db:validate-migrations`, and `npm run refunds:validate-portal-uat`; website, support-email, and SMS Google Form cases share one scoped queue with fixed source badges, exact no-action case links, saved missing/unmapped/failure/duplicate/aging/provider-hold filters, and no mobile overflow.
- [ ] Manually run **Refund Source Reconciliation** with `failure_test=true`; it emits one synthetic aggregate warning and no customer/Sheet/Gmail/provider content. After both default-off switches are approved, run a live 24-hour check and require `accepted source submissions = Hub cases + authorized quarantine items`, no unexplained delta, and an owner for every stale/failing/unmapped/quarantined signal.
- [ ] With synthetic data, open both linked references from the `/refunds` duplicate panel, keep one canonical case, confirm the duplicate cannot take an official action, reverse the decision to **different purchases**, and confirm both redacted audit timelines update without a customer email or provider call.
- [ ] With synthetic data only, a mapped Google Form row appears as a Hub `draft` within 15 minutes and is visible only to the machine's authorized managers; incomplete optional facts stay incomplete and do not invent a transaction match or decision.
- [ ] Replaying the same Sheet row, moving it to another row, and editing it before manager progress preserves one case; an edit after manager progress fails closed and never overwrites the manager's work.
- [ ] Synthetic unmapped, ambiguous, invalid-email, and invalid-source-timestamp responses create no case. Invalid optional amount/last-four/local-time facts may create only an incomplete `draft` with official actions disabled. Every outcome appears only as opaque reason/count evidence in the authorized quarantine/health view.
- [ ] Disable repository variable and Edge switch `REFUND_GOOGLE_FORM_SYNC_ENABLED`; the next run reports disabled while hosted-form/email intake, existing Hub cases, and manual manager work remain available.
