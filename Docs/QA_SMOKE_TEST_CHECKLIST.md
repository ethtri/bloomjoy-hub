...
- [ ] Run `npm run refunds:validate-dual-intake-data-policy`; the Form/Sheet/Gmail/Hub data inventory, exact 11-field contract, owner/backup/privacy approvals, 30/90/180/400-day and canonical-record decisions, Viewer-only Sheet bridge, attachment blocker, PII-free evidence, deletion, and independent kill switches remain explicit and fail closed.
- [ ] Follow `Docs/REFUND_DUAL_INTAKE_DATA_POLICY.md` with synthetic records: revoke Sheet and Gmail access independently, prove unaffected intake/Hub work continues, expire transport/derived copies without deleting the canonical case/audit, remove a synthetic owner/manager, and execute source-specific quick-disable/restore.
- [ ] Do not copy Gmail/hosted-form attachment bytes for the pilot until `#711` records a source-by-source owner decision. If enabled later, prove private quarantine, signature/type/count/size checks, malware-clean-before-access, delete-before-metadata-redaction, and unauthorized-user denial; if disabled, preserve text-only intake with humble recovery copy.
