...
- [ ] Choosing `Clear selected transaction` immediately removes the prior refund action and makes `Save and recheck card sale` the only next action; an unsaved candidate cannot expose final refund execution before the server records manager confirmation and returns execution eligibility.
- [ ] Choosing a possible Nayax transaction exposes only `Save possible transaction`; its confirmation says no refund, approval, or customer email will occur. Saving keeps the case in review with no decision, no approval/completion message, and no Nayax refund call. The payment badge remains `Payment: Not issued` until verified provider success.
...
- [ ] Approved card path uses the matched transaction amount, requires one explicit manager confirmation, calls guarded in-app `nayax-card-refund`, leaves the case open and both customer and manager without a success receipt when execution is blocked, and completes only after a successful execution response without showing raw provider IDs in the UI.
- [ ] Card refund availability is read-only and fail-closed: loading, errors, disabled configuration, kill switch, missing configuration, and unconfirmed contract states show `Card refunds aren\u2019t available right now`, never show `Ready to refund` or an active refund button, and remain closed after Refresh without a provider call.
