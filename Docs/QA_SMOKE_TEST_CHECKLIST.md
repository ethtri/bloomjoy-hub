...
- [ ] Repository Auth preflight proves the steady state is TOTP enrollment-off/verification-on. The owner-supervised read-only control-plane probe passes `--expect open` immediately before private setup and `--expect closed` immediately after success/cancel/expiry/failure; no broad config push or application/agent setter exists.
