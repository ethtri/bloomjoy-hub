...
- [ ] Super admins can configure selected pilot machines from Admin > Machines > Edit Machine: enable/disable public refund intake, set the customer-facing form label, save a Nayax machine ID/account key for read-only lookup, and confirm the UI says live card refund execution remains disabled/manual.
...
- [ ] Scoped Admin opens `/admin` and is redirected to `/admin/access?tab=reporting-access`; admin navigation shows only Access; `/admin/partnerships`, `/admin/reporting`, and `/admin/access?tab=global-roles` do not expose global controls.
...
- [ ] Scoped Admin can open `/admin/access`, see the Corporate Partner permissions panel, and grant/revoke Corporate Partner membership only for partner records whose active partnership machines are fully inside the scoped admin grant.
