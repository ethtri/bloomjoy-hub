...
- [ ] Run `npm run refunds:validate-gmail`; the designated-mailbox check, label-only intake, minimal Gmail endpoints, idempotency, redaction, attachment-free pilot boundary, retention, aggregate health, default-off schedule, and original-thread reply guards pass.
- [ ] Preserve the isolated first-contact evidence: exact `info@` OAuth with Gmail read-only/send scopes, verified send-as aliases, an isolated label/sender population excluded from the legacy responder, one original-thread acknowledgement, replay/later-reply suppression, private-form continuation without a duplicate case, and disabled teardown. Separately prove one case-specific original-thread reply with the complete current mapped-manager CC route before active cutover.
...
- [ ] In a disposable local Supabase project, seed only the exact legacy structure: card, `card_refund_pending`, `decision=approved`, `not_requested`, zero provider attempts, exactly one total/sent approved message, zero pending or completed messages, and no completion/reference/adjustment evidence.
- [ ] Run the owner operation with its exact confirmation phrase. Confirm current state becomes **Payment history check**, stale current match/recommendation fields and every cached lookup candidate are removed, only presence/count evidence enters immutable history, the page says **No provider refund is recorded**, and the historical approval message/event remains unchanged.
