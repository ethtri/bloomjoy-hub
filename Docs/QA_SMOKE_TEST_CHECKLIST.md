...
- [ ] Preserve the isolated first-contact evidence: exact `info@` OAuth with Gmail read-only/send scopes, verified send-as mailbox identities, an isolated label/sender population excluded from the legacy responder, one original-thread acknowledgement, replay/later-reply suppression, private-form continuation without a duplicate case, and disabled teardown. Confirm `etrifari@bloomjoysweets.com` or a plus-address is used only as an owner-controlled synthetic customer/test sender or recipient, never as the production refund-assistant mailbox. Before the remaining case-specific proof, use only the database-owner preparation path from `#800`: one exact existing Gmail-linked case whose customer is `etrifari+refundpilot...@bloomjoysweets.com`, exactly one original thread, no attachments, one-to-three current mapped managers, one opaque token digest, and a five-minute expiry. Wrong/missing case, recipient, token, template, route, expiry, replay, concurrent use, or an admin/intake/follow-up/completion message creator without the exact internal binding must create zero message and make zero OAuth/provider call. The approved run must add exactly one case message and one Gmail outbound from exact `info@` in the original thread with the complete current manager CC digest; read the redacted aggregate summary, then close the window and require `activeAuthorizationCount=0`. All Gmail/customer/GPT/aging/official/Nayax/schedule gates remain off before and after.
