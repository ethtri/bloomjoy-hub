...
- [ ] Portal Reports > Partner Dashboard can switch to Weekly, choose a completed reporting week, and show the selected week in the controls, KPI summary, machine rollups, calculation detail, warning/status notes, and PDF/XLSX export metadata
- [ ] Portal Reports > Partner Dashboard can switch to Monthly, choose a completed calendar month, and show the selected month in the controls, KPI summary, machine rollups, calculation detail, warning/status notes, and PDF/XLSX export metadata
...
- [ ] Portal Reports > Partner Dashboard export menu offers `Polished PDF report` and `Detailed Excel workbook (.xlsx)`, and both downloads use the same selected partnership, period, warning state, and machine scope
- [ ] Portal Reports > Partner Dashboard generated XLSX workbook includes Summary, Machine Rollups, Assumptions, Warning State, and Reconciliation sheets; workbook totals reconcile to the dashboard/PDF net sales and amount owed for the same selected period
- [ ] On mobile widths (`360x800`, `390x844`, `414x896`), Portal Reports > Partner Dashboard keeps partnership/period controls, amount owed, warning notes, chart values, machine rollups, calculation detail, and PDF/XLSX export menu actions readable without exposing the view to non-super-admin users
