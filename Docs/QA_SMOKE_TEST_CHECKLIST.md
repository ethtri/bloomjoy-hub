...
- [ ] `/solutions/food-trucks` direct-loads with the approved audience, Mini-first evaluation path, space/total-load/service-flow constraint, and `Request a machine-fit quote` action visible in the first viewport
...
- [ ] Mobile quote CTAs contain only allowlisted `type=quote`, approved `interest` when present, canonical `source`, and `use=mobile-food`; no free-form setup, financial, or contact value appears in the URL
...
- [ ] Both mobile routes are prerendered, canonical, in `sitemap.xml` with a defensible `2026-08-09` lastmod, and served by the static Vercel rewrites
