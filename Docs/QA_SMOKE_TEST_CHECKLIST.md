...
- [ ] Submit a hosted refund request with a structured reader interaction, customer time confidence, issue category, amount, and permitted last 4; confirm there is no product/selection question or photo control, the form says wallet/device digits may differ from the physical card, and the customer confirmation is logged in `refund_case_messages`.
