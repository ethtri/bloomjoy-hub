...
- [ ] Post-deploy route smoke proves `refund-case-intake`, `nayax-transaction-lookup`, `refund-case-admin-update`, `refund-case-message-send`, `refund-case-automation-sweep`, `refund-gmail-sync`, `refund-gpt-triage`, and `nayax-card-refund` respond with their expected safe success/auth/validation status; manual/retry messaging does not return `404`.
...
- [ ] Run `npm run refunds:preflight-gpt-triage -- --project-ref <project-ref>` and confirm required Supabase secret names exist without printing values; no `VITE_OPENAI_*` or `VITE_REFUND_GPT_*` setting exists. Before any provider call, `#635` records the exact OpenAI project retention mode and privacy/security approval, then the server-only `OPENAI_REFUND_TRIAGE_DATA_CONTROLS_APPROVED` acknowledgement is set only for the approved window; `store=false` is not treated as proof of zero abuse-monitoring retention.
