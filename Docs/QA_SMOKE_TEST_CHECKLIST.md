...
- [ ] Re-run one scheduled window and one due case/state action key; `refund_automation_runs` and `refund_automation_actions` keep one row each, with no duplicate customer message, state change, escalation event, or audit event.
- [ ] Force one synthetic failure-test alert; the designated operations recipients receive a useful health alert with no customer name/email, payment data, complaint text, or provider payload. Repeating the same failure-test run key suppresses a duplicate alert.
...
- [ ] Quick disable: set GitHub variable `REFUND_AUTOMATION_SWEEP_ENABLED=false` and Edge secret `REFUND_AUTOMATION_ENABLED=false`; a manual run returns `disabled`, while refund intake, queue review, manual manager actions, and customer-email retry remain available.
