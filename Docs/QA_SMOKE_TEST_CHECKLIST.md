...
- [ ] Run `npm run refunds:validate-release-tooling`, `npm run refunds:release:check`, and `npm run refunds:release:check-production -- --project-ref <project-ref>`; all ten reviewed Refund Operations functions are `ACTIVE`, match the reviewed production metadata, and use the approved `verify_jwt` setting.
...
- [ ] Run `npm run refunds:validate-public-options-smoke`, then post-deploy run `npm run refunds:smoke-public-options -- --project-ref <project-ref> --confirm-project-ref <project-ref>`; the aggregate result contains no internal `Unmapped`/`Unknown` label or duplicate machine/display row, includes Atlanta, DC, and Seattle, and prints no identifiers.
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains the 34 reviewed, synthetic, hashed desktop/mobile portal, provider-outcome, machine-manager, QR-intake, and unique QR/time recommendation screenshots; five strict, sanitized, hashed JSON artifacts covering portal assertions, disposable-database counts, Gmail MIME roles, kill switches, and provider outcomes; plus `refund-uat-evidence.json`. The manifest source commit matches the reviewed PR head and says `containsProductionData: false`.
...
- [ ] Run `npm run refunds:validate-gmail`; the designated-mailbox and send-as-alias checks, label-only intake, explicit participant roles, mapped-manager CC MIME, minimal Gmail endpoints, idempotency, redaction, private quarantine, retention, aggregate health, default-off schedule, exactly-once first contact, and original-thread reply guards pass.
- [ ] Follow `Docs/REFUND_GMAIL_FIRST_CONTACT_CUTOVER.md` with synthetic data only. In `shadow`, the first eligible inbound message records one no-send operation; replay, duplicate ingestion, and later replies record no second operation.
- [ ] In an isolated label excluded from every legacy sender, one new synthetic customer thread receives exactly one `refund_first_contact_v1` acknowledgement in the original Gmail thread. A separate new thread receives its own acknowledgement; automated mail, a bounce, and outbound messages from the primary mailbox or any configured send-as alias receive none.
- [ ] Confirm the acknowledgement includes the public reference and approved public refund/support links, contains no internal `/refunds?case=...` URL or sensitive-payment request, carries standard automatic-response suppression headers, and has the customer as the sole To recipient with one to three current active mapped Machine Managers in visible CC. Unresolved or invalid manager routing must create no provider send.
- [ ] Simulate a known Gmail rejection and an uncertain delivery outcome. A known failure offers a human-triggered supported follow-up in the original thread; an uncertain outcome disables guided, triage, and advanced reply paths and is also rejected by the server-side thread guard. Reconciliation rotates fairly across first-contact and manager-reply operations, keeps Gmail health degraded while any remain, uses a five-minute lease plus attempt-version fencing, and uses deterministic Message-ID even after first-contact mode is disabled. Prove an active attempt cannot be reclaimed before lease expiry and that an older delayed result cannot change state after a newer claim. For a verified synthetic no-send, the latest versioned search must complete with exactly zero results; provider errors, ambiguity, stale results, and in-flight searches keep resolution blocked. Then an authorized user confirms `I checked; no message was sent`, the actor/redacted audit is recorded, and exactly one controlled follow-up becomes available.
- [ ] Before active cutover, record a content-free legacy-sender inventory, complete `#634` and `#686`, disable and verify the legacy sender first, set an exact UTC cutoff, and prove the rollback order disables Hub before restoring legacy.
...
- [ ] Synthetic participant UAT proves a verified direct customer reply can clear waiting state, while mapped-manager Reply All, revoked/former manager, forwarded customer/manager-address mail, spoof-suspected mail, alias-address mail without Gmail `SENT` evidence, automated mail, and unknown senders are labeled correctly and cannot change customer facts/state, start customer GPT triage, or trigger follow-up. A configured alias becomes mailbox-origin only with Gmail `SENT` evidence.
- [ ] For a resolved synthetic machine with one to three managers, the customer is the sole To recipient and the current active non-revoked mapped managers appear once in visible CC. Revoke/add a mapping between sends and confirm the next send uses the changed set; malformed, duplicate, customer, mailbox, and unrelated fallback addresses never appear in CC. Simulated direct-write drift to four distinct active managers returns an invalid-mapping exception and creates no outbound Gmail or transactional customer send.
- [ ] Before machine resolution, and for zero-manager or invalid-mapping cases, every manual and automatic customer-facing send fails closed before provider delivery while the manager workbench/action notice shows a redacted routing exception. After a current active manager mapping is restored, the next deliberate send includes at least one valid manager CC. This deliberately suppresses even a request for the missing machine until ownership/routing is resolved; raw To/CC addresses never appear in browser data, logs, health output, issue/PR evidence, or screenshots.
- [ ] Only a synthetic DSN whose first receiver result is `mx.google.com`, whose SPF or DKIM/DMARC pass aligns to the claimed Google reporter domain without contradictory reporter failures, and whose same recipient block binds the exact case customer to `Action: failed` plus `Status: 5.x.x` pauses scheduled/automatic contact. Unrelated or non-Google passes, delayed `4.x`, delivered/relayed/expanded actions, split recipient blocks, wrong recipients, subject/`X-Failed-Recipients`/report-type-only evidence, and failed actions without `5.x` remain non-pausing. After linking a newer unpaused Gmail thread to the same case, the older hard-bounce pause still blocks every automatic claim and remains visible in manager context. One authenticated manager recovery, after exact customer-address verification, clears every linked pause atomically and writes one actor-attributed redacted event; unauthorized, service-worker, newest-only, partial, and automatic clear attempts fail. Deliberate manual handling remains available only with a valid current manager CC route; Gmail authorization revocation still affects only Gmail health/transport.
- [ ] No customer-visible subject, text, or HTML contains `/refunds?case=` or internal/provider analysis. Separate action-needed, aging, and exception notices give current mapped managers the canonical authenticated case link; a capped operations fallback receives it only when manager routing is unresolved/invalid and excludes the case customer and all mailbox identities. A success/completion path sends no duplicate manager-only notice.
- [ ] Replaying the Gmail message, public-intake dedupe key, or scheduler action does not duplicate a manager action notice. A simulated uncertain provider delivery is left for manual reconciliation and is not represented as exactly-once delivery or retried automatically.
- [ ] With synthetically configured Gmail credentials but `REFUND_GMAIL_ENABLED=false`, run `npm run refunds:validate-gmail` and confirm a Gmail-linked customer delivery returns the stable redacted `gmail_integration_disabled` state with zero outbound claim, OAuth, or Gmail calls. Confirm an unlinked hosted-form delivery still receives its current mapped-manager authorization and remains available, and the authenticated refund workbench remains reviewable. With the switch enabled only in the fake transport, confirm one customer To recipient, exactly two current manager CC recipients, the original provider thread, `In-Reply-To`/`References`, automatic-response headers, and no internal case link. Generate the composable Gmail-only evidence into a fresh target with `npm run refunds:evidence-gmail -- --evidence-dir output/refund-uat-fragments`; the executable harness must finish every assertion before it creates `refund-gmail-mime-roles.json` and `refund-gmail-kill-fragment.json`, and existing target files make the command fail rather than silently reusing stale evidence. Both files are deterministic, synthetic, enumerated-schema/count/boolean-only, smaller than 16 KiB, and contain no addresses, URLs, IDs, message content, provider references, or payment-like digit runs. These two files prove only Gmail outbound, automatic customer-contact, and first-contact MIME behavior; the fragment deliberately does not claim manager-aging shutdown, hosted-form intake availability, or authenticated portal availability. The final integration aggregator must add those outcomes only after its own executable assertions pass and reserve `refund-kill-switches.json` for that combined result; these two Gmail-slice files alone are not integrated-UAT completion evidence.
...
- [ ] Run `deno test --no-lock supabase/functions/_shared/refund-deterministic-follow-up.test.ts supabase/functions/_shared/refund-email.test.ts`, `npm run refunds:validate-customer-comms`, `npm run refunds:validate-automation`, and `npm run db:validate-migrations`; exact missing-field, no-safe-match, wallet-link-only, two-cycle, one-reminder, one-receipt, verified-reply, GPT participant, provider-exception, and default-off assertions all pass.
- [ ] With `REFUND_AUTOMATIC_CUSTOMER_CONTACT_ENABLED=false` or the singleton database contact switch false, intake, Gmail sync, and manager portal work continue but no deterministic follow-up claim or customer send occurs. Enabling only one switch still sends nothing.
- [ ] An incomplete synthetic Gmail case receives one versioned message asking for every absent required field and no present field; it never asks for a full card number, security code, expiration date, PIN, credential, wallet screenshot/digits, or payment-screen photo. A wallet-last-four mismatch uses only the existing secure single-use correction link.
- [ ] A complete card case receives no customer correction message for provider setup, outage, rejection, timeout, or unknown result; the current mapped Machine Managers instead receive one redacted exact-case-link notice. Only a confirmed versioned `no_safe_match` result may send the humble correction-focused template.
- [ ] Repeated and concurrent scheduler runs produce at most one request and one fixed-cycle reminder. A failed or delivery-unknown operation is not retried; a verified customer reply suppresses the reminder, receives one receipt-only acknowledgement, and claims one deterministic recheck tied to that exact source message.
- [ ] Link two Gmail conversations to one synthetic case. A request replies to its verified customer source, its reminder remains on that same conversation, and the receipt replies to the conversation containing the exact verified customer reply; making another linked conversation newer never changes the target.
- [ ] Change the database contact switch, case decision, or active Machine Manager mapping after a message is prepared but before transport claim. The new send fails closed. Replaying an operation already confirmed `sent` by Gmail reconciles only that exact local message milestone and makes no second provider call.
- [ ] Age a claimed request, reminder, receipt, and reply recheck beyond the bounded stale-claim threshold. Unknown delivery becomes manager-review work with no automatic retry; known Gmail delivery advances only its matching action. A known receipt plus an independently abandoned recheck must fail the recheck and leave the cycle in manager review, never close it.
- [ ] Mapped-manager, forwarded, spoof-suspected, automated, unknown, legacy-unverified, and content-deleted Gmail messages cannot claim follow-up receipt/recheck or GPT work. A verified customer reply that changes no structured fact is acknowledged once and routed to a person without repeating the original request.
- [ ] `/refunds` customer-message history shows only safe automatic evidence (reason, requested field names, template version, delivery kind, and cycle reference) and never raw model/provider payloads. Pre-decision emails label the amount `Reported amount`; only approved/completed templates label it `Refund amount`.
...
- [ ] Scan an enabled machine QR; the public URL contains only an opaque identifier, the form locks to the correct refund-enabled Commercial/Mini machine, and the stored claim context uses database server time rather than a browser timestamp.
...
- [ ] Submit a hosted refund request with card payment, amount, virtual last 4, Apple Pay/wallet checked, issue description, and optional photo; the form says the wallet/device digits may differ from the physical card and the customer confirmation email is logged in `refund_case_messages`.
...
- [ ] Before owner activation is separately approved and deployed, `/refunds` clearly says official actions are not enabled and keeps approve, decline, cash completion, and Nayax execution disabled even for an otherwise eligible mapped manager; review, lookup, and customer-information follow-up remain available. Confirm `refund_official_actions_enabled()` is still hard-coded false and both Supabase Auth TOTP enrollment and the database enrollment window are closed.
- [ ] Run `npm run refunds:validate-official-actions`, `npm run refunds:validate-manager-totp`, and `npm run db:validate-migrations`; the suites prove AAL1, no-TOTP, stale/pre-intent, ambiguous, malformed, same-second, and refresh-only proofs fail, while intents and receipts are short-lived, SHA-256-bound, single-use, and invalidated by actor/case/action/target/amount/decision/version/mapping/candidate-evidence drift.
- [ ] In a disposable local Supabase stack only, explicitly open TOTP enrollment, run `npm run refunds:validate-manager-mfa-freshness`, and immediately close/restart enrollment afterward. Record only the sanitized PASS result proving repeated verification advances the TOTP AMR timestamp, refresh-only preserves but does not advance it, password-only stays AAL1, and an old AAL2 proof predates a new action intent; never record local keys, JWTs, codes, factor IDs, QR material, or secrets.
- [ ] `#692` activation UAT uses an owner-approved, manager-only pilot account in a human-controlled, non-shared browser session. Enroll TOTP only during the controlled window; never screenshot, log, store, copy, email, or share the QR/secret. Verify cancellation removes an unfinished factor, successful verification hides the setup secret permanently, lost-device recovery stops official actions for owner-supervised factor review, and enrollment is closed again before proceeding.
- [ ] For approve, decline, cash completion, and Nayax execution, first show the exact case, action, amount/decision, and transaction evidence; then require a fresh six-digit authenticator code from the exact refund-specific factor in the manager's active owner-approved enrollment. Directly calling either target with a generic AAL2 token and no or an incorrect internal proof fails. A generic pre-existing factor, enrollment revocation/replacement, cancellation, bad/expired code, stale or same-second AMR, refresh-only token, replay, a second intent using the same proof, and case/mapping/evidence change take no official action and force a fresh review/challenge.
- [ ] For Nayax execution, change the selected-match facts or machine ID/account key/refund enablement/amount cap between review and verification, and again between verification and service consumption. Both valid-to-valid drift cases fail closed; the latter rolls back receipt consumption. Run the local two-session race regression and confirm concurrent prepare leaves one live intent while concurrent consume creates exactly one receipt/audit row.
- [ ] During owner-targeted enrollment, confirm durable approval and sanitized audit evidence commit together and the one-use database window closes. Force the record call to fail after Auth verification; verify best-effort durable compensation and Auth-factor removal run, and confirm no official intent can be prepared or consumed without active durable approval. Race compensation against intent consumption and confirm the shared actor lock prevents deadlock and leaves either a valid consumed action or a revoked enrollment with no pending intent, never a partial state.
- [ ] Portal UAT screenshots include the normal step-up prompt, cancellation/bad-code/expired states, and the enrollment-closed recovery state. Never capture a screenshot while an enrollment QR/secret is visible. Audit/log review contains only sanitized actor/case/intent/action/timestamps and no code, factor ID, secret, QR material, raw JWT, customer/payment payload, or recipient address.
- [ ] Persona UAT proves an active mapped Machine Manager can pass the future action-bound challenge, while a mapped Super Admin, mapped Scoped Admin, unrelated user, revoked manager, or identity granted admin authority after receipt mint remains review-only and cannot commit the action.
...
- [ ] The manager workbench displays customer-reported incident time separately from server-recorded machine QR-open time, explains the confidence class and sanitized signals, and states that a payment match does not prove a delivery failure or approve a refund.
...
- [ ] Guarded card success shows a confirmation receipt plus one customer-facing email state with the send-time current mapped-manager CC count/status; blocked execution leaves the case open with `Refund not sent`; an unconfirmed provider outcome warns the manager not to retry until Nayax is reconciled.
...
- [ ] New refund submissions send a redacted canonical-link action notice to current assigned Machine Managers with reference, machine, amount, incident time, payment method, and status only. Bloomjoy operations receives the notice instead only when no eligible current manager can be resolved.
...
- [ ] Mobile refund queue/detail at `390x844` has no clipped columns, hidden controls, or horizontal page overflow; queue rows stack as cards, healthy/waiting system status stays compact, and long card/cash primary-action labels wrap without clipping.
...
- [ ] Run `npm run refunds:validate-manager-aging` and `npm run db:validate-migrations`; the 2- and 5-business-day boundaries, weekend/DST handling, trusted reply ordering, version invalidation, final-reservation routing/fingerprint consistency, duplicate suppression, pre-provider attempt hold, cross-version known-sent/known-not-sent recovery, and unknown-delivery review assertions pass.
- [ ] Keep `REFUND_MANAGER_AGING_NOTICES_ENABLED=false` while deploying. With synthetic cases only, enable it during the configured local policy window and prove exactly one manager reminder at two Monday-Friday business days and one escalation at five. No holiday is inferred and the 30-minute triage target appears in no customer copy.
- [ ] Put a synthetic case into `waiting_on_customer`, then ingest a newer verified direct customer reply. Both suppress the old manager milestone; replaying the same message creates no new version, and only a later manager-relevant re-evaluation begins a fresh attention clock.
- [ ] Resolve synthetic manager A, remap the machine to manager B, then reserve the notice. Confirm the final database reservation serializes the remap and returns only B to transport; A cannot reach transport. A zero-manager milestone returns one redacted, capped operations routing exception that excludes the customer and mailbox identities; an over-cap fallback fails closed. Persisted/logged/audit evidence contains only a mapping-row-ID/policy fingerprint, never a raw recipient or an unsalted address hash.
- [ ] Open an eligible pending-card notice's encoded `/refunds?case=` URL and confirm the exact authenticated case opens without calling `nayax-transaction-lookup`, `refund-case-admin-update`, or `nayax-card-refund`. Search and status filters remain independent, queue selection remains navigation-only, and only a separate explicit advanced-tool request may invoke lookup. The notice and audit evidence contain no customer identity, recipient address, complaint text, card digits, transaction/provider identifiers, or raw payloads.
- [ ] Force manager-notice delivery uncertainty, then change the case/reply attention version. The old bound attempt still blocks both reminder and escalation. Reconcile the exact attempt as known sent and known not sent in separate synthetic cases; both clear the global hold, neither marks the newer version's milestone, the scheduler never retries blindly, and health/reason counters expose only aggregate redacted evidence. Disable only `REFUND_MANAGER_AGING_NOTICES_ENABLED` and confirm inbox intake, customer-contact controls, and manual portal actions still work.
- [ ] Seed 100 resolved manager-aging states ahead of a 101st due synthetic case. Confirm the bounded due-candidate RPC returns and reserves the 101st case, proving resolved, stale, held, and not-yet-due rows are removed before the worker cap.
...
- [ ] Authorized managers see compact automation and Gmail status on `/refunds`; healthy/waiting states share one low-emphasis summary, warning states remain prominent with recovery copy, and raw run/action ledgers remain inaccessible to browser roles.
...
- [ ] Confirmed Nayax success creates exactly one completion transition, one reporting adjustment, and one customer-facing confirmation with the send-time current mapped managers visibly CC'd; it creates no separate manager completion email. Repeated manager clicks, function retries, browser refreshes, and replayed provider responses create no duplicate refund, record, or email.
