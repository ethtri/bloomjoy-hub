...
- [ ] `npm run refunds:validate-nayax-provider` and `npm run refunds:validate-nayax-execution` pass: mocked request/approval, exact response classification, timeout/non-success/unfamiliar-response stop, redaction, no-redirect token protection, and no caller amount override are covered. `npm run db:validate-migrations` also proves the atomic single-use claim, parallel-safe daily caps, and ambiguous-attempt cap accounting. Post-deploy smoke proves `nayax-card-refund` remains fail-closed by default without calling live Nayax refund endpoints.
