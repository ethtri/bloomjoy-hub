...
- [ ] Run `npm run refunds:validate-manager-aging` and `npm run db:validate-migrations`; the 2- and 5-business-day boundaries, weekend/DST handling, version invalidation, recipient/outcome consistency, mapping repair/revocation, duplicate suppression, and unknown-delivery review assertions pass.
...
- [ ] Force manager-notice delivery uncertainty. The attention state requires manual review, the scheduler never retries blindly, and the health/reason counters expose only aggregate redacted evidence. Disable only `REFUND_MANAGER_AGING_NOTICES_ENABLED` and confirm inbox intake, customer-contact controls, and manual portal actions still work.
