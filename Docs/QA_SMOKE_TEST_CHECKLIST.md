...
- [ ] A normal matched card case shows the customer request and selected Nayax transaction side by side at desktop width, exposes exactly one visible `Review $X refund` action leading to one **Approve refund** decision, and does not render manual status/decision selectors.
- [ ] Choosing `Clear selected card sale` immediately removes the prior approval action and makes `Save and recheck card sale` the only next action. A current high-confidence recommended candidate may be selected automatically, but the server must revalidate it while recording the one approval before any provider call.
- [ ] The card refund action opens a keyboard-accessible approval confirmation that restates location, machine, transaction time, amount, card last four, and the completion-email preview; canceling does not call Nayax, and the single **Approve refund** button disables while processing so a double-submit cannot create a second provider attempt.
...
- [ ] Guarded card success shows a confirmation receipt plus customer- and manager-email states; a setup block after approval leaves the case open with `Approval saved; refund not confirmed`; an unconfirmed provider outcome warns the manager not to retry until Nayax is reconciled.
...
- [ ] One **Approve refund** call can record the current recommended high-confidence transaction and execution eligibility before the provider claim; ambiguous, no-safe-match, manual-exception, hard-blocked, expired, and alternate selections cannot set it. Alternate selection requires a structured disagreement reason in the audit event.
...
- [ ] `npm run refunds:validate-nayax-provider` and `npm run refunds:validate-nayax-execution` pass: mocked request/approval, exact response classification, timeout/non-success/unfamiliar-response stop, redaction, no-redirect token protection, no caller amount override, immediate unauthorized-call rejection, exact account-scoped refund-write credential isolation, and fail-closed dedicated idempotency-secret handling are covered. `npm run db:validate-migrations` also proves the atomic single-use claim, changed-evidence rejection, exact frozen execution snapshot, parallel-safe daily caps, ambiguous-attempt cap accounting, one-approval wallet/QR eligibility, exactly-once reporting, and the two-recipient notification outbox. Post-deploy smoke proves `nayax-card-refund` remains fail-closed by default without calling live Nayax refund endpoints.
