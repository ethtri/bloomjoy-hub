...
- [ ] Submit a hosted refund request with card payment, amount, virtual last 4, Apple Pay/wallet checked, issue description, and optional photo; the form says the wallet/device digits may differ from the physical card. The customer confirmation is logged in `refund_case_messages`, remains on the original Gmail thread, addresses only the customer, and visibly CCs the full current active mapped-manager route; unresolved routing blocks delivery.
...
- [ ] One new eligible synthetic Gmail customer thread receives exactly one approved first-contact acknowledgement; replayed delivery and later replies receive none, while outbound mail, bounces, mailing-list, bulk, and automated messages are excluded. The legacy and Hub responders are never authoritative at the same time, and rollback restores only one responder.
...
- [ ] Before machine resolution, and for zero-manager, invalid/over-cap, or empty-safe-route cases, every Gmail and transactional customer-facing send fails closed before provider delivery while the workbench/action notice shows a redacted routing exception. After a current mapping is restored, the next deliberate send includes at least one valid manager CC. The internal operations fallback never substitutes for customer-message CC.
...
- [ ] Gmail quarantine reserves a tokenized database upload intent before Storage and accepts only the exact private bucket/canonical UUID path. Crash/unknown upload or delete outcomes and corrupt/noncanonical coordinates hold only that item for manual review; they never permit guessed deletion or metadata purge, while unrelated expired storage-free content still purges. Retention runs with Gmail sync/OAuth off after its separate GitHub, Edge, database, and owner-approval gates are enabled.
...
- [ ] Independent switch matrix stays fail-closed: Gmail schedule/transport, deterministic customer contact (Edge plus DB), manager aging, Gmail retention (GitHub plus Edge plus DB owner gate), attachment scanning, GPT (GitHub plus Edge plus DB/data-control acknowledgement), immutable official-action gate, and live Nayax adapter can each be disabled without enabling another lane. With Gmail/customer-contact/manager-aging off, executable tests prove zero provider/fetch/claim/reservation/send calls; retention can remain independently available after OAuth revocation.
