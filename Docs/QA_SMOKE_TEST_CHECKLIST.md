...
- [ ] Run `npm run refunds:validate-release-tooling`, `npm run refunds:release:check`, and `npm run refunds:release:check-production -- --project-ref <project-ref>`; the candidate manifest tracks exactly ten Refund Operations functions, including the two manager TOTP functions, and each deployed function is `ACTIVE`, matches the reviewed production metadata/source, and uses the approved `verify_jwt` setting. Before deployment, final source commit, migration/test totals, and manifest SHA must no longer be **Pending**.
...
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains exactly 38 fresh, distinct synthetic screenshots, five strict sanitized JSON artifacts, and the hashed manifest; the manifest source commit matches the reviewed PR head and says `containsProductionData: false`. No artifact contains production data, addresses, UUIDs/provider IDs, URLs, or customer/provider free text.
- [ ] Run `npm run refunds:smoke-intake-email` in read-only preflight mode for the privately approved machine; then, only in the approved production-smoke window, run its fail-closed synthetic submission with a new retry-safe run UUID, the exact authorization phrase, and owner-controlled identities. Reusing the run UUID must reuse the case without sending again. Customer delivery requires the exact current mapped-manager visible CC set; any operations fallback is a separate internal routing notice and never substitutes for CC. PASS evidence contains only aggregate safe fields.
- [ ] Run `npm run refunds:validate-gmail` and `npm run refunds:validate-email-pilot`; the designated-mailbox check, label-only intake, minimal Gmail endpoints, idempotency, redaction, retention, aggregate health, default-off schedule, one-link pre-mapping acknowledgement, private email-to-form linkage, attachment-off intake, duplicate guards, and manager queue signals pass.
- [ ] Run `npm run refunds:validate-gpt-triage`; all sanitized classification, routing, missing-field, schema-rejection, redaction, deterministic replay, strict Responses API request, `store=false`, no-tools, key exclusion, default-off, no-auto-send, and mocked refusal/HTTP/timeout/schema/configuration checks pass with a 0% unsafe-action rate and no paid OpenAI API call.
- [ ] Run the GPT pgTAP safety suite through `npm run db:validate-migrations`; GPT/OpenAI processing defaults off, browser roles cannot read either ledger, jobs contain no raw input/response, duplicate and failed jobs do not auto-retry, latest-message completion supersedes stale unreviewed output, approval cannot be recorded before delivery, and expired derived content is purged.
- [ ] For an approved GPT evaluation, run `npm run refunds:preflight-gpt-triage -- --project-ref <project-ref>` and confirm required Supabase secret names exist without printing values; no `VITE_OPENAI_*` or `VITE_REFUND_GPT_*` setting exists. If GPT is deferred during the core deployment, OpenAI credentials and scheduler secrets may remain absent while all three GPT switches stay off. Before any real OpenAI model request, `#635` records the exact OpenAI project retention mode and privacy/security approval, then the server-only `OPENAI_REFUND_TRIAGE_DATA_CONTROLS_APPROVED` acknowledgement is set only for the approved window; `store=false` is not treated as proof of zero abuse-monitoring retention.
...
- [ ] A Super Admin or Scoped Admin sees one concise Gmail health signal and may review an incomplete unassigned draft. The once-per-thread generic hosted-form acknowledgement may send before machine mapping with the customer as sole To and no CC; all case-specific delivery and the `Reply in Gmail thread` action remain blocked until the machine is resolved and the full current mapped-manager CC route passes. A location-only Machine Manager cannot see the unassigned draft, and no Nayax, card-refund, or cash-completion action is available until required transaction details are complete.
- [ ] After the synthetic draft is assigned to a resolved machine and the send-time current mapped-manager route passes, the deliberately approved response appears once in the original Gmail thread with the customer as sole To and the full current active manager set in visible CC. A simulated uncertain send tells the manager to reconcile Gmail and does not automatically retry.
...
- [ ] Gmail retention cleanup claims local work before Google access, purges metadata only after token-bound quarantine-object deletion settles, holds corrupt/unknown objects for review without blocking unrelated storage-free cleanup, and still completes when Google authorization is revoked but the independently gated maintenance job runs.
...
- [ ] The public hosted form renders no file/photo control, the intake endpoint rejects any non-empty attachment list, and Gmail ingestion copies no attachment metadata or bytes. Any future attachment pilot requires a separate scanner, retention, privacy, and manager-download acceptance decision.
- [ ] Operations and privacy/security owners approve the proposed 180-day Gmail-copy retention, visible-CC handling, and attachment-off pilot boundary in `Docs/REFUND_GMAIL_DATA_HANDLING.md` before either Gmail enable switch is turned on.
...
- [ ] GPT-assisted UAT shows one editable missing-information draft followed by one `Approve and reply in Gmail` action; editing changes the one sent reply, rejection sends nothing, and policy-sensitive or uncertain input shows `Needs a person before any reply` with no draft or send action. Every approved reply uses the original Gmail thread, addresses only the customer, and visibly CCs the full current active mapped-manager route; unresolved routing blocks delivery.
- [ ] Run `npm run refunds:validate-customer-comms` and confirm missing-information or other deterministic non-success outcomes send/log at most one policy-allowed customer message, while refund confirmation is sent only after token-bound provider settlement succeeds. Known delivery failures create deliberate recovery work; uncertain delivery is reconciled before retry; the normal path has no separate send-email step or duplicate manager notice.
...
- [ ] Submit a hosted refund request with card payment, amount, virtual last 4, Apple Pay/wallet checked, and issue description; no photo control is present, and the form says the wallet/device digits may differ from the physical card. The customer confirmation is logged in `refund_case_messages`, remains on the original Gmail thread, addresses only the customer, and visibly CCs the full current active mapped-manager route; unresolved routing blocks delivery.
...
- [ ] Submit a cash request with no conservative match; case lands in `waiting_on_customer` and logs one humble, friendly more-info email on the original Gmail thread, addressed only to the customer with the full current active mapped-manager route visibly CC'd. Unresolved routing blocks delivery.
...
- [ ] Opening `/refunds?case=<case-id>`, selecting a queue row, changing filters, or returning from sign-in is navigation-only and records exactly zero Nayax lookup, `nayax-card-refund`, `refund-case-admin-update`, customer-message, manager-step-up, and mutating-RPC calls. Before a result exists, **Check Nayax transaction** is the only visible primary lookup control; clicking it records exactly one lookup, and **Refresh** appears only after a result.
...
- [ ] Guarded card success shows a confirmation receipt plus the one original-thread customer-completion state with managers CC'd and no manager-only completion state; blocked execution leaves the case open with `Refund not sent`; rejection, timeout, and unknown warn the manager without claiming success or inviting a blind retry.
- [ ] The integrated UAT workflow produces exactly 38 fresh, distinct reviewed synthetic screenshots, including the hosted-form and duplicate-review desktop/mobile states plus `refund-provider-success.png`, `refund-provider-rejected.png`, `refund-provider-timeout.png`, and `refund-provider-unknown.png`, and exactly five strict sanitized JSON artifacts: `refund-portal-assertions.json`, `refund-database-counts.json`, `refund-gmail-mime-roles.json`, `refund-kill-switches.json`, and `refund-provider-outcomes.json`. The finalizer rejects stale, missing, extra, malformed, duplicate-image, PII/identifier/URL/free-text-bearing evidence.
- [ ] The authenticated source fragments for `refund-portal-assertions.json` (`evidenceMode=synthetic_browser_mocks`) and `refund-provider-outcomes.json` (`evidenceMode=local_injected_provider_adapter`) are HMAC-bound to their exact filename, producer, timestamp, and evidence with the environment-only run token. The finalizer verifies that provenance before emitting sanitized final JSON; neither source nor final artifact contains the token, secrets, production data, raw provider payloads, recipient addresses, or payment identifiers. Browser mocks and injected adapters do not prove the real HTTP handler or Nayax accepted a refund.
...
- [ ] A manager may review/edit and deliberately send safe more-info or non-success status copy; a denial message is available only after a valid mapped-manager official denial is durably recorded. Card approval/completion success is never an ordinary editable or automatic status message. Only the DB-owned/versioned provider-success completion may send after token-bound provider success plus atomic case/reporting completion.
- [ ] One new eligible synthetic Gmail customer thread with no machine receives exactly one approved first-contact acknowledgement. It contains exactly one Bloomjoy hosted-form CTA, no Google Form CTA, no internal case link, no manager CC, and one private context. Replayed delivery and later replies receive none, while outbound mail, bounces, mailing-list, bulk, and automated messages are excluded. The legacy and Hub responders are never authoritative at the same time, and rollback restores only one responder.
- [ ] Submitting that private hosted form completes the originating Gmail draft as exactly one actionable case; replaying the context fails, a different customer cannot use it, and a fallback second case is held by conservative duplicate review before any official action.
- [ ] Website form and Support email badges, Missing information / Possible duplicates / Aging cases / Provider holds filters, and Open/Copy exact-case controls render on desktop and mobile. Opening any exact link remains navigation-only.
- [ ] Every Gmail-linked automatic reply stays in the original Gmail thread and creates no parallel Resend conversation. Uncertain delivery is reconciled without blind retry.
- [ ] For a resolved synthetic machine with one to three managers, every manual and automatic customer message has the customer as sole To and each current active, non-revoked mapped Machine Manager exactly once in visible CC. Revoke/add a mapping between preview and send and confirm the next send uses the changed set. Duplicate normalized valid rows appear once only when every distinct active identity remains covered; mixed malformed, customer, or mailbox-colliding mappings make the complete route fail closed, and unrelated fallback addresses never appear in customer CC.
- [ ] Before machine resolution, every case-specific Gmail and transactional customer-facing send fails closed; the sole allowed message is the generic first-contact form link. For zero-manager, invalid/over-cap, or empty-safe-route mapped cases, all customer sends fail closed before provider delivery while the workbench/action notice shows a redacted routing exception. After a current mapping is restored, the next deliberate case-specific send includes at least one valid manager CC. The internal operations fallback never substitutes for customer-message CC.
- [ ] Same-customer website/email cases within the conservative machine/time/amount window create one PII-free manager review. Different machine, different customer, legitimate purchase outside six hours, and wallet/last-four mismatch fixtures do not silently merge. Pending or confirmed duplicate state blocks step-up, Nayax reservation/execution, official case updates, and settlement.
- [ ] For one, two, and three active mapped managers, provider success creates exactly one customer-facing completion-message operation with the full authorized CC set and no second manager completion email. Replays send nothing; a mapping change between preview and send is re-evaluated at send time.
- [ ] Customer-visible refund email contains the public reference and no internal portal link, provider identifier, internal note, or complaint analysis. The separate manager notice contains one next action and opens canonical `/refunds?case=<case-id>` without mutating the case.
- [ ] Approved deterministic incomplete/no-safe-match templates ask only for exact useful missing or correctable fields, use versioned humble customer-centric copy, prohibit full card/CVV/expiration/PIN/login/wallet screenshot requests, and never promise a refund. A received-information confirmation is limited to one per explicit correction cycle and is suppressed when an acknowledgement already covers the event. GPT-authored or materially free-form copy still requires explicit human review.
- [ ] The reviewed template registry is exact: `refund_first_contact_v1`, `refund_follow_up_v1`, `refund_manager_aging_v1`, and provider-success-only `refund_nayax_completion_v1`. Unknown versions fail closed; arbitrary approval/completion subject or body text cannot enter the automatic path.
- [ ] A synthetic hard bounce pauses later automatic customer contact and creates a manager-visible exception with the exact case link; resolved, closed, denied, newly customer-updated, or delivery-held cases receive no stale reminder.
- [ ] After a hard bounce, automatic contact resumes only after an authorized recipient correction/approval creates a new bounded operation; ordinary replay cannot resume it.
- [ ] The customer waiting reminder uses the bounded `#687` correction-request cycle. Separately, `#685` sends current mapped managers at most one internal reminder at two business days and one escalation at five business days from each versioned manager-attention anchor, using Monday-Friday `America/Los_Angeles` calculations that preserve local clock time. Draft, waiting-on-customer, denied, completed, closed, delivery-held, disabled/outside-window, stale/version-changed, and delivery-uncertain cases send no manager-aging notice.
- [ ] Gmail OAuth, scheduler, GPT runner, and agent identities cannot approve/decline a case or invoke Nayax. An emailed case link provides authenticated read-only navigation until a Machine Manager takes an explicit portal action.
- [ ] The official-action target permits only a currently active manager mapped to the case's machine. Super Admin and Scoped Admin identities cannot approve/decline or invoke Nayax during the production pilot. No break-glass policy is approved; any future exception requires a separate owner decision, reviewed implementation, and dedicated UAT.
- [ ] Every official approve, decline, cash-completion, and Nayax action is blocked until the current mapped Machine Manager personally completes a fresh challenge for the exact owner-approved TOTP factor after reviewing a frozen two-minute intent. The intent binds actor, case, action/target, case and mapping versions, enrollment version, amount, exact payload hash, and transaction/evidence fingerprint; Nayax rechecks its locked machine/account/refund-control fingerprint at receipt consumption.
- [ ] Only one live intent and one receipt exist per actor. New preparation supersedes the old intent, and per-actor/row locks plus a random 256-bit one-use server proof whose domain-separated digest is stored prevent concurrent or replayed consumption. The newest TOTP AMR must be uniquely parseable, strictly newer than the intent second, and no more than 30 seconds in the future. Missing, stale, same-second, refresh-only, future-skewed, ambiguous, wrong-method, cancelled, expired, replayed, enrollment-changed, mapping-changed, case-changed, evidence-changed, shared-session, and agent-session fixtures all fail closed; one TOTP authentication cannot authorize two actions.
- [ ] First-factor TOTP enrollment is available only through an exact-manager, owner-targeted, short-lived, one-use pilot window in a human-only session. An ordinary AAL1/shared/agent session cannot bootstrap authority; pre-existing or ambiguous factors block setup, recording failure revokes the approval and attempts Auth-factor removal, recovery is owner-supervised, and enrollment closes after the approved managers are verified. Codes, factor IDs, secrets, QR material, raw JWTs, raw one-use proofs, and customer/payment payloads are absent from records, logs, screenshots, and evidence.
- [ ] Participant fixtures cover verified customer, active mapped manager, revoked/former manager, Bloomjoy mailbox, mailbox alias, forwarded participant, automated system, unknown, and spoof-suspected sender. Only the verified customer can update customer facts, clear waiting state, start customer GPT triage, or trigger automatic follow-up.
- [ ] To/CC addresses and delivery-recipient claims remain service-only, never enter telemetry/GitHub evidence/durable audit payloads, and are purged/redacted with the approved 180-day Gmail copy. Retention tests cover one, two, and three mapped managers.
- [ ] Pilot intake has no website file control, rejects non-empty attachment lists, copies no Gmail attachment metadata or bytes, and shows no attachment evidence in the manager walkthrough. The tokenized quarantine/delete contract is future-only and requires a separate approval before any attachment feature is enabled. Retention runs with Gmail sync/OAuth off after its separate GitHub, Edge, database, and owner-approval gates are enabled.
- [ ] Automatic messages include approved auto-response suppression headers. Provider setup, outage, rejection, timeout, and unknown-result fixtures send no misleading customer correction or success message.
...
- [ ] The portal starts its first card lookup only after the manager explicitly chooses **Check Nayax transaction**; links, row selection, filters, and initial render do not call it. Any separately scheduled lookup remains behind the independent default-off automation controls. The server uses the configured +/- 6 hour window; only a uniquely high-confidence transaction is labeled recommended, while alternates stay behind progressive disclosure and are tokenized long enough for manager review.
...
- [ ] Manager denial requires a friendly reason and does not write to `sales_adjustment_facts`; customer denial copy may send only after the valid official denial is stored. More-info/non-success status messages remain human-reviewed. Card approval does not send success copy.
- [ ] `refund-case-admin-update` rejects any attempt to complete an eligible card case or send approved/completed card-success copy. Card completion and its DB-owned/versioned customer message are reachable only through token-bound confirmed provider-success settlement; valid cash/Zelle completion retains its separate evidence-gated path.
...
- [ ] `refund-case-automation-sweep` sends reminder/escalation messages from synthetic stale cases only inside the configured local policy window and returns only redacted aggregate output.
...
- [ ] Quick disable: set GitHub variable `REFUND_AUTOMATION_SWEEP_ENABLED=false` and Edge secret `REFUND_AUTOMATION_ENABLED=false`; a manual run with a new UUID in `run_key` returns `disabled`, while refund intake, queue review, independently enabled portal actions, and deliberate email recovery after a known failure remain available. Uncertain delivery is reconciled rather than blindly retried.
- [ ] Independent switch matrix stays fail-closed: Gmail schedule/transport, deterministic customer contact (Edge plus DB), manager aging, Gmail retention (GitHub plus Edge plus DB owner gate), GPT (GitHub plus Edge plus DB/data-control acknowledgement), immutable official-action gate, and live Nayax adapter can each be disabled without enabling another lane. Attachments are absent rather than scanner-gated in the pilot. With Gmail/customer-contact/manager-aging off, executable tests prove zero provider/fetch/claim/reservation/send calls; retention can remain independently available after OAuth revocation.
- [ ] `npm run refunds:validate-nayax-execution` passes as a static guardrail check, and post-deploy smoke proves the candidate `nayax-card-refund` production adapter is statically disabled before any live Nayax call; historical environment flags cannot activate it.
- [ ] The reviewed release manifest tracks exactly ten Refund Operations functions, including `refund-manager-action-step-up` and `refund-manager-totp-enrollment`. The separate route smoke deliberately sends exactly eight no-auth, no-body `OPTIONS` probes to the established application routes; eight probes is not the manifest function count.
...
- [ ] Approved card path uses the matched transaction amount, requires one explicit current mapped Machine Manager confirmation plus fresh TOTP, atomically consumes that authorization with one token-bound provider attempt, and leaves the case open without customer success when execution is blocked. The raw attempt claim is server-only and never appears in browser, logs, screenshots, or evidence.
- [ ] Confirmed provider success atomically records the terminal provider outcome, one case completion, and one reporting adjustment before exactly one DB-owned/versioned original-thread customer completion becomes claimable with the full send-time mapped-manager CC set. Repeated clicks, function retries, browser refreshes, and replayed provider responses create no duplicate provider call, transition, adjustment, record, or email.
- [ ] Dependency-injected local provider fixtures produce exactly one success, rejection, timeout, and unknown attempt. Replay produces zero additional provider attempts; rejection/timeout/unknown leave the case open, send no success/fallback/manager-completion message, and timeout/unknown require reconciliation. The production handler exposes no request, environment, or browser selector for synthetic outcomes and remains statically disabled before a live call until `#430` passes.
