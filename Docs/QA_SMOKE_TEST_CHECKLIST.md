...
- [ ] Run `npm run refunds:validate-release-tooling`, `npm run refunds:release:check`, and `npm run refunds:release:check-production -- --project-ref <project-ref>`; all eight approved Refund Operations functions are `ACTIVE`, match the reviewed production metadata, and use the approved `verify_jwt` setting.
...
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains the 31 synthetic, hashed desktop/mobile portal, machine-manager, QR-intake, and unique QR/time recommendation screenshots; four strict, sanitized, hashed JSON artifacts covering portal assertions, disposable-database counts, Gmail MIME roles, and kill switches; plus `refund-uat-evidence.json`. The manifest source commit matches the reviewed PR head and says `containsProductionData: false`.
