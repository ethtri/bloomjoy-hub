...
- [ ] New refund submissions send a redacted manager notification to the current assigned Machine Managers with reference, machine, amount, incident time, payment method, case link, and status only. Bloomjoy operations receives the internal routing-repair fallback only when no eligible current manager route can be resolved; that fallback never substitutes for customer-message CC.
