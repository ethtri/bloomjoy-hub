...
- [ ] Before `#692` is approved and deployed, `/refunds` clearly says official actions are not enabled and keeps approve, decline, cash completion, and Nayax execution disabled even for an otherwise eligible mapped manager; review, lookup, and customer-information follow-up remain available.
- [ ] Run `npm run refunds:validate-official-actions` and `npm run db:validate-migrations`; the official-action suite proves the production protocol stub is false, AAL1/non-TOTP/stale-TOTP sessions fail, receipts are SHA-256-bound/single-use, customer and transaction evidence drift invalidates them, raw/service/legacy-wallet paths cannot create or reopen official state, and no provider-success finalizer exists before the attempt-bound integration.
- [ ] `#692` activation UAT uses an owner-approved, manager-only pilot account in a human-controlled browser session. Enroll TOTP only during the controlled window, never screenshot/log/store the QR or secret, then close or tightly govern enrollment. Prove a fresh action-bound code is consumed once; ordinary AAL2, shared sessions, email assistants, agents, service credentials, and stale codes remain insufficient.
