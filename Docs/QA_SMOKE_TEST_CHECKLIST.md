...
- [ ] With no or invalid `VITE_GA_MEASUREMENT_ID`, public routes, forms, planners, and checkout navigation remain functional and local console output shows only sanitized `[Analytics disabled]` payloads
- [ ] Before consent, the public analytics choice is keyboard/touch accessible, `No thanks` loads no Google tag, `Allow analytics` loads at most one tag, and `/privacy` accurately shows and changes the saved choice
- [ ] With a valid GA4 measurement ID, one Google tag initializes on public routes; `/`, `/machines`, one machine page, `/supplies`, both planner routes, `/solutions/food-trucks`, the four mobile-operator Playbook routes, and `/contact` each emit one query-free `page_view` per route change
- [ ] Representative quote/supplies/Plus links, Contact form start/success/failure, planner/checker start/completion, and checkout start/server-verified success appear in GA4 Realtime or DebugView without duplicate events; `?checkout=success` alone emits no success event
- [ ] GA4 web-stream Enhanced Measurement has `Page changes based on browser history events` disabled so manual SPA `page_view` events are not duplicated
- [ ] Analytics requests and local debug logs contain no name, email, phone, message, user/account ID, uploaded-file detail, arbitrary query string, or exact planner/price/quantity input; private portal/admin/refund routes do not initialize public analytics
...
- [ ] Submit a hosted refund request with a structured reader interaction, customer time confidence, issue category, amount, permitted last 4, and optional machine/product photo; confirm there is no product/selection question because the selected machine supplies that category, the form says wallet/device digits may differ from the physical card, warns against card/wallet screenshots, and logs the customer confirmation in `refund_case_messages`.
