...
- [ ] Run `npm run refunds:validate-manager-aging` and `npm run db:validate-migrations`; the 2- and 5-business-day boundaries, weekend/DST handling, trusted reply ordering, version invalidation, recipient/outcome consistency, duplicate suppression, pre-provider attempt hold, cross-version known-sent/known-not-sent recovery, and unknown-delivery review assertions pass.
...
- [ ] Change and revoke the active machine-manager mapping between synthetic milestones. Each send uses only the current one-to-three mapped managers. A zero-manager milestone sends one redacted, capped operations routing exception that excludes the customer and mailbox identities; repairing the mapping routes the next milestone to the repaired current set.
- [ ] Open the notice's encoded `/refunds?case=` URL and confirm the exact authenticated case opens without calling `refund-case-admin-update` or `nayax-card-refund`. The notice and audit evidence contain no customer identity, complaint text, card digits, transaction/provider identifiers, or raw payloads.
