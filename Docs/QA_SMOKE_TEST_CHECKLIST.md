...
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains the 23 synthetic, hashed desktop/mobile portal, machine-manager, and QR-intake screenshots plus `refund-uat-evidence.json`; the manifest source commit matches the reviewed PR head and says `containsProductionData: false`.
