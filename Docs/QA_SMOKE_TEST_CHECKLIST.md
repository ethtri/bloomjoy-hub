...
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains exactly 38 reviewed synthetic screenshots plus the five sanitized JSON artifacts defined in the production runbook; the manifest source commit matches the reviewed PR head and says `containsProductionData: false`.
