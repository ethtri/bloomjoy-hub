...
Current production-readiness checkpoint (2026-08-14): the deployed ten-function/51-migration default-off foundation, exact dual-role mapping predicate, isolated exactly-once first contact and case-specific owner-controlled Gmail proof, private email-linked form continuation/assignment, PII-free automation alert/replay/disabled-lane proof, and retention-only no-op have passed. The `#767` audited outcome-resolution foundation, `#829` default-off production deployment, and Gmail intake-shadow foundation are complete. This is **not** live-launch evidence. An approved outcome-resolution operator cohort and explicit activation remain absent; owner-private TOTP enrollment, the account-specific `#430` Nayax write contract, approved machine/cap configuration, and one capped supervised provider test remain open.
...
- [ ] Run `npm run refunds:validate-public-options-smoke`, then post-deploy run `npm run refunds:smoke-public-options -- --project-ref <project-ref> --confirm-project-ref <project-ref>`; public option count equals the complete active Commercial/Mini portfolio count, missing portfolio options and active machines hidden by unsafe/missing public labels are both zero, the aggregate contains no internal `Unmapped`/`Unknown` label or duplicate machine/display row, includes Atlanta, DC, and Seattle, and prints no identifiers.
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains exactly 47 reviewed synthetic screenshots plus the five sanitized JSON artifacts defined in the production runbook; the manifest source commit matches the reviewed PR head and says `containsProductionData: false`. The owner-authenticator image is the pre-QR readiness card only; no QR or code is captured. The payment-support images contain synthetic structured fields only and remain before the synthetic code submission.
...
- [ ] Preserve the completed isolated first-contact and case-specific evidence: exact `info@` OAuth with Gmail read-only/send scopes, verified send-as mailbox identities, an isolated label/sender population excluded from the legacy responder, one original-thread acknowledgement, replay/later-reply suppression, private-form continuation without a duplicate case, one owner-controlled original-thread reply with the complete mapped-manager CC route, and disabled teardown. Confirm `etrifari@bloomjoysweets.com` or a plus-address is used only as an owner-controlled synthetic customer/test sender or recipient, never as the production refund-assistant mailbox. Any explicitly approved future case-specific rerun must use only the reviewed database-owner preparation path from `#800` and owner runner from `#810`: one exact existing Gmail-linked case whose customer is `etrifari+refundpilot...@bloomjoysweets.com`, exactly one original thread, no attachments, one-to-three current mapped managers, one opaque token digest, and a five-minute expiry. Wrong/missing case, recipient, token, template, route, expiry, replay, concurrent use, or an admin/intake/follow-up/completion message creator without the exact internal binding must create zero message and make zero OAuth/provider call. The approved run must add exactly one case message and one Gmail outbound from exact `info@` in the original thread with the complete current manager CC digest; read the redacted aggregate summary, then close the window and require `activeAuthorizationCount=0`. All Gmail/customer/GPT/aging/official/Nayax/schedule gates remain off before and after.
...
- [ ] Reply in the original synthetic thread with exact labeled values for the remaining machine/location, purchase date (`YYYY-MM-DD`), time with AM/PM, payment method, amount, and physical-card last four. Confirm only missing facts change, an exact unique machine label resolves, the same case becomes lookup-ready, and the existing Nayax lookup path is eligible without a manager lookup click. An ambiguous location, unlabeled prose, quoted old answers, or wallet digits must not select or overwrite evidence.
...
- [ ] Run `npm run refunds:validate-nayax-matching`; strong-card and unique QR/time fixtures classify correctly, while two close-together sales, unresolved wallet mismatch, wrong/uncertain amount, missing/late/replayed QR, wrong machine, duplicate/already-refunded, failed provider status, no-match, and DST fixtures keep unsafe one-click execution closed.
- [ ] Run `npm run refunds:validate-nayax-mapping-smoke`, then post-deploy run `npm run refunds:smoke-nayax-mapping -- --project-ref <project-ref> --confirm-project-ref <project-ref>`; every active refund-intake machine has one non-duplicate Nayax account/machine mapping, one to three active managers, and a location timezone, while live refund execution remains off. The smoke is SELECT-only, prints counts only, and does not call Nayax or prove transaction matching/provider execution.
- [ ] Run `npm run refunds:validate-nayax-controlled-owner-pilot`. The migration-52 source must remain default-off: zero pilot authorization/attempt/journal rows, zero enabled Nayax machines/caps and active pilot executor, broad official/Nayax gates and schedules off, no portal/UI selector, no reporting-token fallback, and no Hub customer/Gmail completion. Prove Auth-email-matched self-case/card/amount and machine/account binding, fixed-class/digest-only journal rows, the provider-email suppression-or-self-owner-consent contract, and lease-aware no-ID recovery. Initialization, dry-run, live TOTP, recovery, and retirement are separate owner-private ceremonies in `Docs/REFUND_NAYAX_CONTROLLED_OWNER_PILOT.md`; no phase is authorized by this checklist. Live must remain hard-blocked while `refund_operations_owner` has no separately reviewed financial-audit duration and verified purge/discharge procedure. A future approved live result is acceptable only with exactly one request and at most one approval, immutable redacted stage order, no retry, exact terminal reconciliation, zero Hub customer/Gmail delta, terminal worker acknowledgement, and conclusively closed machine/executor/authorization state.
- [ ] Run `npm run refunds:validate-nayax-resolution` and `npm run db:validate-migrations`; require the hard-off gate, zero seeded payment-support operators, private/RLS-protected intent and immutable outcome evidence, exact current mapped-manager/operator/active-enrollment/factor/version binding, two-minute single-use intent, four fixed evidence/result shapes, and a genuine two-session race with exactly one winner. Raw references must pass evidence-type-specific prefix/privacy checks and be stored only as SHA-256 digests. Provider-confirmed or documented-manual success must use the authoritative UTC evidence action time for case/reporting and create exactly one deterministic pending original-thread completion; the Edge path attempts only that email, with one exact retry after a proven safe failure and no retry after uncertain delivery. Confirmed-safe review must advance one generation and then permit a new separately approved attempt with a distinct idempotency key; remain-on-hold changes no outcome. Every resolution path creates zero new provider attempt.
...
- [ ] With the broad official-action/TOTP lane still disabled, the scoped overview exposes refund authority only for the exact active mapped manager after a server-recorded Nayax selection and machine allowlist/cap are present. Unmapped users, unrelated cases, missing selections, disabled machines, duplicate links, and reconciliation holds remain `official_actions_disabled`; the overview itself creates no authorization or provider attempt.
...
- [ ] Guarded card success shows a confirmation receipt plus customer- and manager-email states; blocked execution leaves the case open with `Refund not sent`; an unclear refund result warns the manager not to retry until payment support confirms what happened. Rejected, pending, timed-out, and unknown outcomes show neither a `Ready to refund` badge nor an active refund action. Pending, timed-out, and unknown outcomes move immediately to `Check refund result` and leave `Needs action` until the result is confirmed.
- [ ] `npm run refunds:validate-portal-uat -- --app-url <local-or-preview-url>` produces screenshot evidence for exact match, confirmation, processing, success, setup-needed, no-match, ambiguous, lookup-failed, wallet/manual, desktop, and mobile states under `output/playwright`.
...
- [ ] For a resolved synthetic machine with one to three managers, every manual and automatic customer message has the customer as sole To and each current active, non-revoked mapped Machine Manager exactly once in visible CC. Duplicate normalized valid rows appear once only when every distinct active identity remains covered; mixed malformed, customer, or mailbox-colliding mappings make the complete route fail closed, and unrelated fallback addresses never appear in customer CC.
...
- [ ] Admin > Machines explains that active Commercial/Mini machines appear on the customer form automatically. Super admins can separately enable/disable refund automation readiness, set the customer-facing label, and save a Nayax machine ID/account key through the single `Save machine changes` button; switching automation off leaves public intake available and live card execution remains separately gated.
...
- [ ] Run `npm run refunds:validate-automation`; the sweep and independent health schedules are versioned, serialized, disabled by default, limited to aggregate logs, and wired to safe manual failure-test alerting.
...
- [ ] Quick disable: set GitHub variable `REFUND_AUTOMATION_SWEEP_ENABLED=false` and Edge secret `REFUND_AUTOMATION_ENABLED=false`; a manual run with a new UUID in `run_key` returns `disabled`, while refund intake, queue review, manual manager actions, and customer-email retry remain available.
...
- [ ] Card refund availability is read-only and fail-closed: loading, errors, disabled configuration, kill switch, missing credentials/configuration, or machine/cap setup show `Card refunds aren\u2019t available right now`, never show an active refund button, and remain closed after Refresh without a provider call.
- [ ] The manager-session reservation rechecks the exact mapping, case version, recommendation evidence, stored USD amount, machine allowlist/cap, duplicate link, and daily caps. Repeated or concurrent confirmation of unchanged evidence produces exactly one provider attempt; the audit record says `manager_session` and does not claim that TOTP was used.
- [ ] Confirmed Nayax success creates exactly one completion transition, one reporting adjustment, and one original-thread customer confirmation with the full current mapped-manager set visibly CC'd. It creates no separate manager completion email. Repeated manager clicks, function retries, browser refreshes, and replayed provider responses create no duplicate refund, record, or email.
...
- [ ] Cash completion requires a positive amount no greater than the recorded customer payment, payment-sent timestamp, short non-sensitive confirmation/reference, and an explicit `payment was sent` checkbox; it rejects card/bank/contact/credential-like input (including bare 8+ digit account/routing values) and never stores the manual reference in the audit-event payload.
...
- [ ] Cash completion history identifies the acting manager and event time, while desktop and `390x844` screenshots show the review, confirmation, and durable success/recovery states without horizontal overflow; the primary action wraps fully and the `Use current time` shortcut remains at least 44px tall.
