...
- [ ] Run `npm run refunds:validate-nayax-matching`; strong-card and unique QR/time fixtures classify correctly, while two close-together sales, wallet mismatch, wrong/uncertain amount, missing/late/replayed QR, wrong machine, duplicate/already-refunded, failed provider status, no-match, and DST fixtures keep unsafe one-click execution closed.
...
- [ ] Run `npm run refunds:validate-nayax-shadow`, then for one privately selected open card case run `npm run refunds:shadow-nayax -- --project-ref <project-ref> --env-file <server-only-env-file> [--case-id <private-case-uuid>] --output-file output/refund-nayax-shadow/evidence.json`. Require one Supabase read-only audit plus one Nayax Last Sales GET, aggregate-only output, zero raw identifiers/customer/card details, zero production writes, and zero refund endpoint calls. A `no_safe_match` result proves the negative fail-closed path only; positive-match acceptance still requires a recent eligible case with a uniquely high-confidence recommendation.
- [ ] Run `npm run refunds:validate-qr-shadow-pilot`; all nine required QR/card/wallet/ambiguity/manual/provider-failure scenarios pass with zero controlled false-positive high-confidence recommendations and unique QR/time remains manual-only.
- [ ] After sponsor approval and six physical installs, follow `Docs/REFUND_QR_SHADOW_PILOT_PACKET.md` and run `npm run refunds:report-qr-shadow-pilot` with private cohort/observation files and a bounded pilot window. Require aggregate-only output, all four rollback proofs, no stop condition, and `evidenceReadyForSponsorReview=true`; the sponsor decision is still required.
...
- [ ] Run `npm run refunds:validate-qr-assets`; a high-error-correction synthetic QR decodes to the production Bloomjoy refund route, and the static contract proves scoped actions, redacted audits, rotation/disable, and sequential rollout checks.
...
- [ ] Run `npm run refunds:validate-machine-manager-uat -- --app-url http://127.0.0.1:8081` to exercise mocked Admin > Machines sample data, searchable Machine Manager lookup, autosave payload, customer-refund setup, read-only Nayax lookup setup, QR download/clipboard/rotation/disable behavior, per-version rollout checks, and desktop/mobile evidence.
...
- [ ] Demo QR visual review is explicit: `/admin/machines?demo=on` opens an enabled synthetic machine with an active QR version, generates a print-ready SVG, copies a production-origin opaque URL, records rollout checks in-browser only, resets checks on rotation, and shows the safe disabled state without calling live QR RPCs.
...
- [ ] In Admin > Machines, the Refund QR pilot card counts every currently enabled public machine. Before pilot signoff it must show the expected six-machine cohort and every row must show one current active QR version.
- [ ] For each of the six physical machines, download the current SVG, print at full size, install it on the matching human-readable location/machine label, record the replacement-owner role, scan the installed label on a real phone, and mark the four version-specific checks. Synthetic browser/mobile evidence does not substitute for the real-phone scan.
- [ ] Rotate one synthetic or approved test code and confirm the prior identifier immediately shows the safe unavailable-code message, the new identifier differs, and all physical rollout checks reset. Disable the replacement and confirm its path also fails safely.
- [ ] Authorization check: Super Admin and in-scope Scoped Admin can manage QR assets for an eligible Commercial/Mini machine; ordinary users, Machine Managers without admin authority, and out-of-scope Scoped Admins cannot read or change QR asset data.
...
- [ ] Ambiguous, no-match, lookup-failed, setup-needed, wallet/manual, unique QR/time, and hard-blocked card states never expose an enabled refund action; each shows one plain-language recovery action and keeps alternate transactions behind progressive disclosure.
...
- [ ] Guarded card success shows a confirmation receipt and customer-email state; blocked execution leaves the case open with `Refund not sent`; an unconfirmed provider outcome warns the manager not to retry until Nayax is reconciled.
