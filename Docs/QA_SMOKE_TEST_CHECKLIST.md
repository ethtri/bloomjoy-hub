...
- [ ] For approve, decline, cash completion, and Nayax execution, first show the exact case, action, amount/decision, and transaction evidence; then require a fresh six-digit authenticator code. Cancellation, bad/expired code, stale or same-second AMR, refresh-only token, replay, a second intent using the same proof, and case/mapping/evidence change take no official action and force a fresh review/challenge.
