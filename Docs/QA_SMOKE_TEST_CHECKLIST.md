...
- [ ] Run `npm run refunds:validate-gmail-intake-shadow-runner` and the owner-only intake-shadow dry-run from `REFUND_GMAIL_INTAKE_SHADOW_RUNBOOK.md`. Require the separate shadow-label digest, exact owner Auth-email digest, exact two-message ordered shape, cap one, retention/schedules/delivery/official/Nayax hard-off posture, one POST/no retry, conclusive two-attempt maximum safe-close, and aggregate-only `no_effect|complete_exact|partial_incident|outcome_unknown` evidence. Any live run is separately authorized and never replayed. `complete_exact` must prove zero Gmail/Resend/customer/manager/provider/refund attempt, one owner-manageable draft/action case, and the assigned earliest/latest retention cleanup obligation; partial/unknown remains a durable incident after gate closure.
