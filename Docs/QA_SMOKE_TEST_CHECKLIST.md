...
- [ ] Run `npm run refunds:validate-gmail-intake-shadow-runner`, the one-time closed-state initialization when secrets are absent, and the owner-only intake-shadow dry-run from `REFUND_GMAIL_INTAKE_SHADOW_RUNBOOK.md`. Require the separate shadow-label digest, exact owner Auth-email digest, exactly one fresh matching thread in the five-minute window, exact two-message ordered shape, cap one, retention/schedules/delivery/official/Nayax hard-off posture, one POST/no retry, conclusive two-attempt maximum safe-close, more-than-400-second exact-run reconciliation, and aggregate-only `no_effect|complete_exact|partial_incident|outcome_unknown` effects evidence plus a separate final gate state. Any live run is separately authorized and never replayed. `complete_exact` is successful only with conclusively closed gates and must prove zero Gmail/Resend/customer/manager/provider/refund attempt, one owner-manageable draft/action case, and the exact run-bound assigned earliest/latest retention cleanup obligation; partial/unknown or unverified gates remain a durable HOLD.
