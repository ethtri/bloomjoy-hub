...
- [ ] Guarded card success shows a confirmation receipt plus customer- and manager-email states; blocked execution leaves the case open with `Refund not sent`; an unconfirmed provider outcome warns the manager not to retry until Nayax is reconciled.
...
- [ ] Approved card path uses the matched transaction amount, requires one explicit mapped Machine Manager confirmation, calls guarded in-app `nayax-card-refund`, leaves the case open without a customer-facing success message when execution is blocked, and completes only after a successful execution response without showing raw provider IDs in the UI.
- [ ] Confirmed Nayax success creates exactly one completion transition, one reporting adjustment, and one customer-facing completion operation with the send-time active mapped-manager CC set. Repeated manager clicks, function retries, browser refreshes, and replayed provider responses create no duplicate refund, record, or email.
