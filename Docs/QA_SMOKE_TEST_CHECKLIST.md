...
- [ ] A resolved-machine customer message CCs only currently active, non-revoked mapped Machine Managers. A manager Reply All is classified as manager correspondence, not customer evidence; a zero-manager, malformed, revoked, duplicate, mailbox, or unrelated fallback address is excluded safely.
...
- [ ] Waiting-on-customer scheduling uses two business days and manager escalation occurs before the five-business-day target; displayed wording, scheduler calculations, configured contact window, and holiday/weekend fixtures agree.
...
- [ ] Every official approve, decline, cash-completion, and Nayax action is blocked until the mapped Machine Manager completes a fresh per-action TOTP challenge after reviewing the frozen case/action/evidence context. Missing, stale, wrong-method, cancelled, replayed, mapping-changed, case-changed, evidence-changed, shared-session, and agent-session fixtures all fail closed without storing the code, factor ID, secret, QR material, or raw JWT.
- [ ] First-factor TOTP enrollment is available only through the approved owner-controlled pilot window in a human-only session. An ordinary AAL1/shared/agent session cannot silently bootstrap payment authority; QR/secret material is absent from screenshots and evidence, and enrollment is closed or explicitly governed after the approved managers are verified.
