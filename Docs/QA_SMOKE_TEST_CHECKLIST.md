...
- [ ] Choosing a possible transaction exposes only `Confirm this transaction`; its confirmation says no refund, approval, or customer email will occur. Confirming keeps the case in review with no decision, no approval/completion message, and no refund call. The payment badge remains `Payment: Not issued` until verified payment success.
...
- [ ] Ambiguous, no-match, lookup-failed, setup-needed, unresolved wallet/manual, and hard-blocked card states never expose an enabled refund action; each shows one plain-language recovery action and keeps alternate transactions behind progressive disclosure. A `unique_qr_time` result becomes eligible only under the versioned `#674` rule after one plausible transaction remains and the manager confirms it.
