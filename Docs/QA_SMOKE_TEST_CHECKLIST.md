...
- [ ] Submit a hosted refund request with a structured reader interaction, customer time confidence, issue category, amount, permitted last 4, optional product text, and optional machine/product photo; the form says wallet/device digits may differ from the physical card, warns against card/wallet screenshots, and logs the customer confirmation in `refund_case_messages`.
