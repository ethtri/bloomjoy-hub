...
- [ ] Manually dispatch one approved synthetic scheduled window with a new safe `run_key`, then dispatch again with that exact key; the second workflow returns `duplicate_suppressed`, and `refund_automation_runs`/`refund_automation_actions` keep one row each with no duplicate customer message, state change, escalation event, or audit event.
- [ ] Force one synthetic failure-test alert with a new safe `run_key`; the designated operations recipients receive a useful health alert with no customer name/email, payment data, complaint text, or provider payload. Dispatching again with the exact same failure-test key returns `duplicate_suppressed` and sends no second alert.
...
- [ ] Quick disable: set GitHub variable `REFUND_AUTOMATION_SWEEP_ENABLED=false` and Edge secret `REFUND_AUTOMATION_ENABLED=false`; a manual run with a new safe `run_key` returns `disabled`, while refund intake, queue review, manual manager actions, and customer-email retry remain available.
