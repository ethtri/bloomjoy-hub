...
- [ ] Confirmed Nayax success creates exactly one completion transition, one reporting adjustment, one customer confirmation, and one manager confirmation. Repeated manager clicks, function retries, browser refreshes, and replayed provider responses create no duplicate refund, record, or email.
