...
- [ ] Run `npm run refunds:validate-public-options-smoke`, then post-deploy run `npm run refunds:smoke-public-options -- --project-ref <project-ref> --confirm-project-ref <project-ref>`; public option count equals the active eligible Commercial/Mini portfolio count, missing portfolio options is zero, the aggregate contains no internal `Unmapped`/`Unknown` label or duplicate machine/display row, includes Atlanta, DC, and Seattle, and prints no identifiers.
