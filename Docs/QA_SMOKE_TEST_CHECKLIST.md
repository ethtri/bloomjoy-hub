...
- [ ] Authorized managers see one concise automation health banner on `/refunds` with healthy, stale, failing, paused, or waiting state plus the last successful time; raw run/action ledgers remain inaccessible to browser roles.
