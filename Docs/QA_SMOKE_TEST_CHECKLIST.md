...
- [ ] Run `npm run refunds:validate-gmail`; the designated-mailbox check, label-only intake, minimal Gmail endpoints, idempotency, redaction, private quarantine, fail-closed attachment deletion and retention, aggregate health, default-off schedule, and original-thread reply guards pass.
...
- [ ] With Gmail provider sync disabled, keep `REFUND_GMAIL_RETENTION_ENABLED=true` and run the workflow in `retention` mode. It returns only aggregate deleted/purged counts, does not refresh a Google token or call Gmail, and continues to enforce the approved 180-day local-copy deletion policy.
