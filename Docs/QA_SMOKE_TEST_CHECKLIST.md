...
- [ ] The PR's `Refund UAT Evidence` check passes and its 30-day artifact contains the 18 synthetic, hashed desktop/mobile screenshots plus `refund-uat-evidence.json`; the manifest source commit matches the reviewed PR head and says `containsProductionData: false`.
...
- [ ] Ambiguous, no-match, lookup-failed, setup-needed, wallet/manual, and hard-blocked card states never expose an enabled refund action; each shows one plain-language recovery action and keeps alternate transactions behind progressive disclosure.
...
- [ ] Portal Reports > Operator view shows visible, keyboard-operable Daily, Weekly, and Monthly Breakdown choices plus Today and Last 7 days presets without removing custom dates
- [ ] Portal Reports > Operator view Daily breakdown shows exactly one row/card per selected calendar date, including `$0.00` dates, and daily net, gross, refund, and transaction totals reconcile at exact-cent precision with the KPI summary and detailed machine/payment rows
- [ ] Portal Reports > Operator view machine and payment filters update KPIs, daily totals, trend, detailed rows, and export scope together; loading and failed states do not present missing data as confirmed zero sales
...
- [ ] Portal Reports > Operator view daily controls and rows remain readable, focus-visible, touch-friendly, and free of page-level horizontal overflow at `360x800`, `390x844`, `414x896`, and a representative desktop width
