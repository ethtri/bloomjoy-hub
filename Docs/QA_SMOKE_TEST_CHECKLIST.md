...
- [ ] Run `npm run refunds:validate-sms-responder-audit`; the sanitized provider findings, RED launch gate, owner/backup requirement, 11-column Form contract, one-response target, STOP/HELP/loop/delivery checks, independent disable, rollback timing, and evidence redaction checks pass.
- [ ] Follow `Docs/REFUND_SMS_RESPONDER_AUDIT.md` in an owner-supervised window using an approved synthetic handset only. One first inbound receives one humble current-Form response within 60 seconds; two ordinary follow-ups, provider redelivery, and an automated sender receive no additional Form response inside the 24-hour suppression target.
- [ ] Provider-supported STOP, HELP, delivery-failure, and dead-link tests produce the documented safe behavior and PII-free owner/backup alert. Disabling only SMS leaves the email responder, hosted Form, Hub queue, and default-off Sheet bridge available; restoring the known-good version repeats the baseline within the same day.
