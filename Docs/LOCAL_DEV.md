...
- `SUNZE_REPORTING_TIMEZONE` controls browser/report date selection only; it does not prove the exported `Payment time` basis. Keep `SUNZE_PAYMENT_TIME_SEMANTICS_STATUS=unvalidated`, `SUNZE_PAYMENT_TIME_PROOF_SCOPE=unvalidated`, and `SUNZE_PAYMENT_TIME_TIMEZONE` blank until the independent known-order/DST evidence in `Docs/SUNZE_SALES_DISCOVERY.md` proves an account-wide IANA basis. Only then set `validated`, `account`, and that proved IANA zone; per-location assumptions are not accepted by this worker.
...
| Changed product surface | During implementation, using the verification profile from `npm run agent:context` | the existing focused validator for that surface, such as quote intake, mobile content, reporting, or commerce | PR author |
...
| Production drift | Daily schedule or explicit `main` dispatch | least-privilege credential probe and live production comparison with the sealed manifest | Technical owner |

`npm test` is intentionally the quick, universal unit profile. Run
