...
6) Run the mocked refund-only portal QA harness against the running app:
   ...
   - The high-confidence card fixture should auto-select the recommended transaction, show one approval confirmation, call only `nayax-card-refund`, and never issue a browser-side completed-case save. The success fixture confirms both customer and approving-manager delivery states; the disabled fixture saves approval but claims no success.
   - Run `npm run refunds:validate-nayax-execution`, `deno check --no-lock supabase/functions/nayax-card-refund/index.ts supabase/functions/refund-case-automation-sweep/index.ts`, and `npm run db:validate-migrations` after changing this path.
