...
**Current operational truth:** the reviewed repository target covers all ten manifest-tracked Refund Operations functions and all 72 required refund/Nayax migrations. The first nine mapping repairs are deployed and both affected Great Mall cases now complete transaction search safely. The follow-up reconciliation accounts for the remaining 22 active provider rows as 16 customer-safe published mappings, two explicit setup rows, and four explicit test/invalid exclusions. No payment flag changes. Production deployment and live aggregate/form QA remain required for that follow-up slice.
...
|---|---|---|
| Integrated release | Immutable `main` commit, 10-function/72-migration manifest, full verification, reviewed migration dry run, restore source, and clean postdeploy drift | Release and technical owners |
...
- Local release alignment: ten manifest-tracked functions and 72 required refund/Nayax migrations.
...
4. Confirm the manifest covers all ten manifest-tracked Refund Operations functions and all 72 required refund/Nayax migrations, with the exact canonical 51-migration predeployment bridge preserved only as historical restore evidence.
