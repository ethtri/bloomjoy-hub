...
**Current operational truth:** the reviewed repository target covers all ten manifest-tracked Refund Operations functions and all 84 required refund/Nayax migrations. The first nine mapping repairs are deployed and both affected Great Mall cases now complete transaction search safely. The follow-up reconciliation accounts for the remaining 22 active provider rows as 16 customer-safe published mappings, two explicit setup rows, and four explicit test/invalid exclusions. The 75th migration adds the provider-free temporary workflow for the twelve exact Adam-managed NC-account machines that do not yet have Bloomjoy API identities. The 76th migration stores and enforces their exact per-machine Eastern/Central timezone. The 77th migration makes manager transaction confirmation replay-safe and returns a server-owned next-action readiness contract without enabling a machine or making a provider call. The 78th migration makes machine refund readiness truthful, records why card refunds are paused, and provides reviewed single-machine and qualified-fleet activation boundaries with the standard $50 cap. The 79th migration keeps the provider-attempt ledger private while exposing only service-role aggregate UTC-day count and amount totals required for live cap readiness. The 80th migration adds server-derived, replay-safe manual external completion for active cash cases without weakening the Nayax card path. The 81st migration removes the obsolete payout-contact requirement and exposes only the exact machine choice needed for cash at the one grouped location. The 82nd migration makes the database authoritative for request-to-approval transitions, adds version negotiation and account-level circuit breaking, and exposes privacy-safe reliability health. The 83rd migration accepts current 10-digit Nayax DTM transaction references while preserving the provider-free, privacy-safe reconciliation boundary. The 84th migration adds a provider-free, no-retry evidence-only lane for refunds completed in Nayax before Bloomjoy recorded an attempt. Production deployment, controlled non-customer canary, and owner UAT remain required before broad card-refund reopening.
...
- Target local release alignment: ten manifest-tracked functions and 84 required refund/Nayax migrations.
...
4. Confirm the manifest covers all ten manifest-tracked Refund Operations functions and all 84 required refund/Nayax migrations, with the exact canonical 51-migration predeployment bridge preserved only as historical restore evidence.
