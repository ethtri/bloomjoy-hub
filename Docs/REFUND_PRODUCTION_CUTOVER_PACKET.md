...
2. Capture and independently review the timestamped production function receipt. Update the manifest through a reviewed manifest-only change only for a `new_bundle_candidate`; retain the sealed manifest for a `same_bundle_later_revision`. Then require the standard production drift check to pass for all eleven functions, including the two inert HTTP 410 cutover tombstones.
