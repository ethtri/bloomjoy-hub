...
- The integrated production set is live through `20260823110000_refund_public_location_selections.sql`. For this repair, `supabase db push --dry-run --linked` must show only `20260823203839_repair_refund_placeholder_location_selections.sql` as pending. Any other result stops deployment. Deploy only that reviewed repair migration and make no database write during the dry run.
