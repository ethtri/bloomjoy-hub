...
2. Review the current integrated release candidate in PR `#760`.
...
4. Confirm the reviewed manifest covers all ten manifest-tracked Refund Operations functions and all 41 current required refund/Nayax migrations.
...
The historical `#629/#716` five-migration bridge does not apply to PR `#760`. The current release must use its own 41-migration manifest, reviewed dry run, backup, migration-before-function order, source capture, and post-deployment drift proof. Any unexpected migration, source digest, function version, switch state, or health result stops the release.
...
3. Run the aggregate-only Nayax mapping smoke and manager-readiness audit. Require one non-duplicate mapping and one to three active managers per pilot machine, assigned-only access, and Admin denial.
...
7. Only in an explicitly approved production-email window, use the owner-controlled synthetic intake smoke and verify the original-thread customer/manager recipient rules. This is not approval to process a real customer case.
...
10. Enable and test automation only after its manual-run evidence passes; keep the quick-disable sequence ready.
11. Enable Gmail only after `#634` approvals, synthetic thread evidence, and legacy-responder cutover proof pass.
...
13. Start live Nayax execution only after the separate `#430` provider-contract decision and a controlled low-value test. Use the approved cohort, allowlist, per-refund cap, and UTC daily count/amount caps.
