...
The owner-only completion function targets only that exact cleanup task handle and proves both messages have the canonical deleted content, identifiers, sender name, provider/reference headers, CC addresses/count, and linked thread subject before atomically completing the run-bound task. It returns exact-task status plus aggregate overdue and all-assigned outstanding counts. A missing/wrong/prior handle, an assigned current task, or any other assigned task (including one not yet due) fails closed; a previously completed handle cannot hide newer work. Remove the private packet and revoke temporary owner credentials using the established owner procedure only after `assignedOutstanding=0`.
