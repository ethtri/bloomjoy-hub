...
- **Machine timestamp source correction (`#990`, September 3):** the original Last Sales response and rejected request prove a local field substitution: `AuthorizationDateTimeGMT` was copied into the refund identity while `MachineAuthorizationTime` had different seconds/fractions. Nayax [documents them as separate fields](https://devzone.nayax.com/reference/lynx/machines/get-last-sales-for-machine-by-machineid). Recommendations now keep GMT for scoring, derive machine time separately, and retain the exact bounded machine string in private candidate evidence. Missing/malformed machine source cannot create a selectable candidate; ambiguous machine time is review-only. No historical attempt is rewritten. This is source preservation, not a verified raw-to-`MachineAuTime` payment transport: that serialization, immutable selected/attempt binding and remaining-refundable preflight remain #990 work. The provider adapter and immutable direct-execution block are unchanged. The generic access-or-transaction-credentials rejection does not establish its sole cause.

- **Capability is proved:** at least one historical Tulsa `$10.90` refund that began through Bloomjoy's Nayax API path was later authoritatively confirmed as a real provider refund. The Lynx write API can refund; the present blocker is not API unavailability.
- **Direct automation is not yet proved:** Bloomjoy has zero production proofs in which an account-confirmed request response advanced to an account-confirmed approval response and automatic case/reporting/customer finalization without DTM or Support reconciliation. The current hold protects that response-interpretation and final-observability gap. See `Docs/NAYAX_REFUND_PRODUCTION_RCA.md` for the detailed timeline and root cause analysis.
...
- The current blocker is the literal, case-sensitive accepted/rejected `Result`/`Status` contract for both request and approval, plus Nayax's exact reason for the later business rejection, followed by one fresh end-to-end proof. It is not evidence that the active user lacks the provider-confirmed roles. Change roles or tokens only if Nayax evidence identifies a scope problem.
...
- Normal execution still requires separate account-scoped request and approval credentials, the exact response contract, exact rollout scope, and every existing manager/evidence/cap/idempotency/circuit-breaker gate. Account-level roles are provider-confirmed; token changes require evidence of a token-specific scope failure.
- Deployment alone does not reopen card refunds. Provider-free reconciliation of the historical attempts is complete and no provider write is in flight; the current hold is the exact response contract, the provider's rejection explanation, and one fresh direct proof. SQS/SFTP readback and registration of the invited read/report user are optional P1 reporting work, not dependencies for ordinary refund execution.
