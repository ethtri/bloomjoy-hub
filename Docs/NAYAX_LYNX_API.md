...
- **Historical offset diagnostic:** Nayax's request schema types `MachineAuTime` as `date-time`, and Bloomjoy tested a default-off mode that appended an offset derived from the selected instant. That experiment returned a combined access-or-transaction-credentials error despite working permissions. Current production uses `exact_source`: preserve the selected raw `MachineAuthorizationTime` wall clock and fractional digits unchanged, never substitute `AuthorizationDateTimeGMT`, and never append `Z` or an offset. The same exact wire value remains bound to request and approval.
...
- Normal execution requires the paired account-scoped write credentials, one manager or Super-admin approval, the exact selected original and amount, duplicate protection and a durable generation-scoped journal. Unknown outcomes hold the same attempt for verification. Only exact authoritative DTM or support proof that no refund occurred can advance that same attempt under its original approval; a rejected response label is not proof. When API search cannot find a match, the Nayax portal is for read-only transaction research only; never issue or record a refund there.
...
Nayax defines `IsRefundedExternally=true` only for a refund the customer's billing provider already handled; that path requires the provider's refund document URL. An ordinary Nayax refund uses `IsRefundedExternally=false` and omits `RefundDocumentUrl`. The enabled production path uses that shape, and September 8 verified its actual business outcome through normally approved customer operations.
