...
- Normal execution requires the paired account-scoped write credentials, manager or Super-admin approval, the exact selected original and amount, duplicate protection and a durable journal. There is no first-proof cohort or quota. Unknown outcomes permanently hold the same attempt. When API search cannot find a match, the Nayax portal is for read-only transaction research only; never issue or record a refund there.
