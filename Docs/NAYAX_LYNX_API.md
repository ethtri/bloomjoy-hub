...
- The case is card-only, manager-approved in the same server request, `card_refund_pending`, matched to the current sanitized high-confidence recommendation, and has no prior settlement adjustment.

First automated execution remains full-refund only, USD only, and manager initiated for a case that manager is authorized to handle. A wallet transaction may become eligible after the customer supplies the virtual/device last four or when verified machine QR, exact amount, and timing leave exactly one plausible sale. Wallet use by itself is not a block; ambiguous evidence still is.

The adapter records the manager's one approval, then obtains a database-atomic single-use claim before contacting Nayax. The claim rejects any case, transaction, amount, machine, or account-key change between the initial read and the database lock, then returns the exact frozen server-only evidence used for both provider calls. It serializes daily cap checks and prevents double clicks, parallel servers, timeouts, and alternate idempotency keys from creating a second provider attempt for the same case.
...
Bloomjoy's production Core account was inspected read-only on 2026-07-27 without revealing token values, changing configuration, or submitting a refund. It established:

- The account has production User Tokens of type `LYNX`, but Core does not display endpoint-level scopes or write authority. A token's presence or recent use therefore does not prove refund permission.
- Dynamic Transactions Monitor exposes `Site ID`, `Transaction ID`, authorization value, machine authorization time, refund request date, refund amount, requester, and the `Refund Requested`, `Refunded`, and `Refund Failed` states. This confirms that Bloomjoy's Core data has the identifiers needed for reconciliation even though a previously captured Last Sales response omitted `SiteID`.
- Core's refund form accepts a two-decimal currency value with a minimum of `0.01` and sends that decimal value as the refund amount. Together with the Lynx `double` contract and decimal Last Sales values, this confirms major currency units for the adapter: a $7.00 refund is `7.00`, not `700`.
- Core's own refund workflow is transaction-gated. It exposes refund actions only when transaction/provider flags indicate refund support and the transaction is otherwise eligible. This means the adapter must continue to fail closed for ineligible, already-refunded, non-positive, or unsupported records instead of assuming that every settled card sale is refundable.
- Core's internal handler recognizes a success result and a failed status, but that UI path calls a Core-internal service rather than the public Lynx endpoint. It is supporting evidence only; it must not be used as the raw Lynx `Result`/`Status` contract.
- No Bloomjoy QA account, QA token, or test-transaction surface was identified in Core.

`SiteID` remains required on both write calls. The normal automated source is the matched Last Sales record, as Nayax's refund guide explicitly directs. If Bloomjoy's production Last Sales response continues to omit it, execution must remain ineligible until a server-side, transaction-safe source is added and verified; a value copied manually from Core is acceptable for investigation but not for automatic execution.
...
Do not contact Nayax as the default next step. The Core inspection above resolved the production host, authentication mode, amount unit, presence of reconciliation fields, and transaction-level refund-support behavior. Because Core exposed no QA route, the next conclusive step is a separately approved, allowlisted, capped low-value production pilot on a known eligible transaction. Record only sanitized HTTP status plus normalized `Result`/`Status` values, confirm the pending and final states in Dynamic Transactions Monitor, and stop immediately on any unknown result. Contact Nayax only if this controlled route cannot establish permission or response behavior.
